...
public class kinematic_variables {
    ...
    double y(LorentzVector lv_beam,
        ...
        double p_dot_q = Lorentz_vector_inner_product(lv_target, lv_q);
        double p_dot_k = Lorentz_vector_inner_product(lv_target, lv_beam);

        return p_dot_q / p_dot_k;
    }

    double gamma(double Q2, double x) {
        return 2 * particle_mass(2212) * x / Math.sqrt(Q2);
    }
    ...
    double Depolarization_A(double gamma, double y) {
        return 1 / (1 + gamma * gamma)
                * (1 - y + y * y / 2 + y * y * gamma * gamma / 4);
    }

    double Depolarization_B(double gamma, double y) {
        return 1 / (1 + gamma * gamma)
                * (1 - y - y * y * gamma * gamma / 4);
    }

    double Depolarization_C(double gamma, double y) {
        return (y / Math.sqrt(1 + gamma * gamma))
                * (1 - y / 2);
    }

    double Depolarization_V(double gamma, double y) {
        return (2 - y) / (1 + gamma * gamma)
