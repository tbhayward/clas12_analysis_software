...
public class proton_energy_loss_corrections_fitter extends GenericKinematicFitter {

    protected final Double mybeam;

    public proton_energy_loss_corrections_fitter(double beam) {
        super(beam);
        mybeam = beam;
    } 
...
//        if (runnum < 10000) {
        ...
        // RGB winter 2020 // (also using this for RGB fall 2019, but it should be updated)
...
//            && nphe_cut(current_Part, cc_Bank) // legacy cut used in the analysis note to check the effect
    ...
    public boolean pion_test(int current_Part, int pid, float vz, double trigger_electron_vz, HipoDataBank rec_Bank, 
        ...
        return true
            && p > 1.20
            && p < 4.00 // this wasn't used in the dihadron publication but was used in the submitted single pion
            && forward_detector_cut(current_Part, rec_Bank)
...
//                if (pid == -321) { pid = 211; }
            ...
            for (int current_Part = 0; current_Part < rec_Bank.rows(); current_Part++) {
                ...
                if (run_Bank.getFloat("torus", 0)<0) { // inbending
                    if (theta < 27) {
                        dp = Math.exp(-2.739-3.932*p)+0.002907;
                    } else {
                        dp = Math.exp(-1.2-4.228*p)+0.007502;
                    }
                } else { // outbending
                    if (theta < 27) {
                       dp = Math.exp(-2.739-3.932*p)+0.002907; 
                    } else {
                       dp = Math.exp(-1.871-3.063*p)+0.007517; 
                    }
                }
                double fe = (dp+p)/p;
                ...
                if (particle_test(current_Part, rec_Bank) 
                   ...
                   Particle part = new Particle(pid,fe*px,fe*py,fe*pz,vx,vy,vz); // momenta scaled for energy loss
                   physEvent.addParticle(part);   
               }
            }
