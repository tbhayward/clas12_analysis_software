...
public class fiducial_cuts {
    ...
    public boolean pcal_fiducial_cut(
        ...
        // Do not apply localized dead-channel exclusions at strictness 1.
        if (strictness < 2) {
            return true;
        }
    ...
    public boolean pass1_pcal_fiducial_cut(int particle_Index, int strictness,
        ...
        if (calIndex == -1) {
            ...
            if (pid == 11) {
                return false;
            } // calorimeter hit required for electron identification
            else {
                return true;
            } // neutrals do not require FD, (probably photon in FT)
        }
        ...
        for (int current_Row = 0; current_Row < traj_Bank.rows(); current_Row++) {
            ...
            if (particle_Index == traj_Bank.getInt("pindex", current_Row)) {
                ...
                if (region == 0 || region == 1 || region == 2) {
                    ...
                    int sector = 0;
                    if (phi_DCr_raw < 30 && phi_DCr_raw >= -30) {
                        sector = 0;
                    } // note this is CLAS sector-1
                    else if (phi_DCr_raw < 90 && phi_DCr_raw >= 30) {
                        sector = 1;
                    } // because of Java indexing reasons
                    else if (phi_DCr_raw < 150 && phi_DCr_raw >= 90) {
                        sector = 2;
                    } else if (phi_DCr_raw >= 150 || phi_DCr_raw < -150) {
                        sector = 3;
                    } else if (phi_DCr_raw < -90 && phi_DCr_raw >= -150) {
                        sector = 4;
                    } else if (phi_DCr_raw < -30 && phi_DCr_raw >= -90) {
                        sector = 5;
                    }
                    double phi_DCr = 5000;
                    if (sector == 0) {
                    ...
                    if (run_Bank.getFloat("torus", 0) < 0 && rec_Bank.getInt("pid", particle_Index) != 11) {
                        // inbending hadrons
