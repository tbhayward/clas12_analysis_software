...
void dc_fiducial_determination(TTreeReader& dataReader, TTreeReader* mcReader = nullptr) {
    ...
    for (const auto& particle_type : particle_types) {
        ...
        for (const auto& region : regions) {
            ...
            TCanvas* c_edge = new TCanvas(("c_" + particle_name + "_" + region_name + "_edge").c_str(), ("c_" + particle_name + " chi^{2}/ndf vs traj_edge").c_str(), 1800, 1200);
            c_edge->Divide(3, 2);
