...
def fill_hist_from_tree(tree, hist, branch_name):
    ...
    # Set up a reference to the branch to avoid repeated lookups
...
def main():
    ...
    h_gen_born = ROOT.TH1D("h_gen_born", "Generated MC; -t1 (GeV^{2}); Counts", nbins, x_min, x_max)
    h_gen_rad  = ROOT.TH1D("h_gen_rad",  "Generated MC; -t1 (GeV^{2}); Counts", nbins, x_min, x_max)

    # Reconstructed MC histograms
    h_rec_born = ROOT.TH1D("h_rec_born", "Reconstructed MC; -t1 (GeV^{2}); Counts", nbins, x_min, x_max)
    h_rec_rad  = ROOT.TH1D("h_rec_rad",  "Reconstructed MC; -t1 (GeV^{2}); Counts", nbins, x_min, x_max)
    ...
    # Fill histograms
    fill_hist_from_tree(t_gen_born, h_gen_born, "t1")
    fill_hist_from_tree(t_gen_rad,  h_gen_rad,  "t1")
    fill_hist_from_tree(t_rec_born, h_rec_born, "t1")
    fill_hist_from_tree(t_rec_rad,  h_rec_rad,  "t1")
