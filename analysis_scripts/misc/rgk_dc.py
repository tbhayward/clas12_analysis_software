...
# Fixed e_theta bins: 8° to 35°, 5 equal bins
edges_deg = np.linspace(8, 35, 6)
...
for i, (low_deg, high_deg) in enumerate(bins_deg):
    ...
    # Fit window around proton peak
    fit_mask = (centers >= M_p2 - 0.1) & (centers <= M_p2 + 0.1)
    # Initial guesses: [A, mu, sigma, m, c]
    p0_11 = [counts11[fit_mask].max(), M_p2, 0.02, 0, np.median(counts11)]
    p0_13 = [counts13[fit_mask].max(), M_p2, 0.02, 0, np.median(counts13)]
    # Perform fits
    try:
        popt11, _ = curve_fit(gauss_lin, centers[fit_mask], counts11[fit_mask], p0=p0_11)
    except:
        popt11 = [np.nan]*5
    try:
        popt13, _ = curve_fit(gauss_lin, centers[fit_mask], counts13[fit_mask], p0=p0_13)
    except:
        popt13 = [np.nan]*5
    # Extract parameters
    ...
    ax.plot(centers, gauss_lin(centers, *popt11), '--', color='blue')
    ax.plot(centers, gauss_lin(centers, *popt13), '--', color='red')
    # Labels and title
    ax.set_xlabel(r'$M_{x}^{2}$ (GeV$^{2}$)')
    ax.set_ylabel('counts')
    ax.set_title(f'$e_θ$ ∈ [{low_deg:.0f}°, {high_deg:.0f}°]')
    ax.legend()

# Final subplot: μ vs e_theta (deg) with σ as error bars
ax = axes_flat[5]
