...
void compareTrees(const char* file1, const char* file2, const char* output,
    ...
    // // electrons
    // std::vector<double> binEdges = {6.0,7.5,7.75,8,8.25,8.5,8.67,9,9.15,9.3,10.6};
    ...
    for (int i = 0; i < nBins; ++i) {
        ...
        sprintf(entry1, "Uncorrected, #mu = %.4f #pm %.4f", mean1, meanError1);
        sprintf(entry2, "Corrected, #mu = %.4f #pm %.4f", mean2, meanError2);
