...
int main(int argc, char** argv) {
    ...
    for(auto& sp: species){
        int pidVal=sp.first; std::string label=sp.second;

        // Default palette and stats off
        gStyle->SetOptStat(0);
        // Book histos
        ...
        for(Long64_t ev=0;ev<nD;ev++){
            dataCh.GetEntry(ev); if(pid!=pidVal) continue;
            double xs[3]={cal_x_1,cal_x_4,cal_x_7}, ys[3]={cal_y_1,cal_y_4,cal_y_7};
        ...
        for(Long64_t ev=0;ev<nM;ev++){
            mcCh.GetEntry(ev); if(pid!=pidVal) continue;
            double xs[3]={cal_x_1,cal_x_4,cal_x_7}, ys[3]={cal_y_1,cal_y_4,cal_y_7};
            for(int i=0;i<3;i++) if(xs[i]!=-9999&&ys[i]!=-9999) hM[i]->Fill(xs[i],ys[i]);
        }
        // Unnormalized Data/MC
        TCanvas c1("c_data_uncut","Data Uncut",1800,600); c1.Divide(3,1);
        SetSame2DScale(hD[0],hD[1],hD[2]);
        for(int i=0;i<3;i++){c1.cd(i+1);gPad->SetLeftMargin(.15);gPad->SetRightMargin(.15);gPad->SetLogz();hD[i]->Draw("COLZ");}
        ...
        for(int i=0;i<3;i++){c2.cd(i+1);gPad->SetLeftMargin(.15);gPad->SetRightMargin(.15);gPad->SetLogz();hM[i]->Draw("COLZ");}
        c2.SaveAs(Form("output/cal/mc_uncut_%s.png",label.c_str()));
        // Normalize & ratio
        for(int i=0;i<3;i++){double id=hD[i]->Integral(),im=hM[i]->Integral();if(id>0)hD[i]->Scale(1/id);if(im>0)hM[i]->Scale(1/im);
            hR[i]=(TH2D*)hD[i]->Clone(Form("hR_%s_%s",label.c_str(),layers[i].c_str()));hR[i]->Divide(hM[i]);}
        // Stats
        std::vector<double> mu(3),sigma(3);
        for(int i=0;i<3;i++){int cnt=0;double s=0,s2=0;
            for(int ix=1;ix<=NB;ix++)for(int iy=1;iy<=NB;iy++){double v=hR[i]->GetBinContent(ix,iy);if(v<=0)continue;s+=v;s2+=v*v;cnt++;}
            mu[i]=cnt? s/cnt:0; sigma[i]=cnt?sqrt(s2/cnt-mu[i]*mu[i]):0;}
        // Ratio plot
        TCanvas c3("c_ratio","Data/MC Ratio",1800,600);c3.Divide(3,1);
        SetSame2DScale(hR[0],hR[1],hR[2]);
        for(int i=0;i<3;i++){c3.cd(i+1);gPad->SetLeftMargin(.15);gPad->SetRightMargin(.15);gPad->SetLogz();hR[i]->Draw("COLZ");
            TLegend leg(0.6,0.7,0.9,0.9);leg.SetFillColor(kWhite);leg.SetBorderSize(1);leg.SetTextSize(0.03);
            leg.AddEntry((TObject*)0,Form("Mean=%.3f",mu[i]),"");leg.AddEntry((TObject*)0,Form("StdDev=%.3f",sigma[i]),"");leg.Draw();}
        c3.SaveAs(Form("output/cal/ratio_%s.png",label.c_str()));
        // Outliers map
        TCanvas c4("c_outliers","Outliers (red/blue)",1800,600);c4.Divide(3,1);
        for(int i=0;i<3;i++){c4.cd(i+1);gPad->SetLeftMargin(.15);gPad->SetRightMargin(.15);gPad->SetLogz(0);
            hR[i]->Draw("axis");
            for(int ix=1;ix<=NB;ix++){double x1=hR[i]->GetXaxis()->GetBinLowEdge(ix),x2=hR[i]->GetXaxis()->GetBinUpEdge(ix);
                for(int iy=1;iy<=NB;iy++){double v=hR[i]->GetBinContent(ix,iy);if(v<=0)continue;
                    double y1=hR[i]->GetYaxis()->GetBinLowEdge(iy),y2=hR[i]->GetYaxis()->GetBinUpEdge(iy);
                    TBox box(x1,y1,x2,y2);box.SetFillColor((v<0.5||v>2.0)?kRed:kBlue);box.SetLineColor(0);box.SetFillStyle(1001);box.Draw("same");}}
        }
