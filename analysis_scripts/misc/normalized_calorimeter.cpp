 ...
 *   6) Creates a second 3-panel "ratio_outliers" plot by filling each bin area:
 *      red if 0.5≤ratio≤2 (blue otherwise).
...
int main(int argc, char** argv) {
    ...
    for (auto& sp : species) {
        ...
        for(int i=0;i<3;i++){
            c3.cd(i+1); gPad->SetLeftMargin(.15); gPad->SetRightMargin(.15); gPad->SetLogz(); hR[i]->Draw("COLZ");
        ...
        // Outliers: fill bins red/blue
        TCanvas c4("c_outliers","Outliers (red/blue)",1800,600); c4.Divide(3,1);
        for(int i=0;i<3;i++){
            c4.cd(i+1); gPad->SetLeftMargin(.15); gPad->SetRightMargin(.15); gPad->SetLogz(0);
            hR[i]->Draw("axis");
            for(int ix=1;ix<=NB;ix++){
                double x1=hR[i]->GetXaxis()->GetBinLowEdge(ix), x2=hR[i]->GetXaxis()->GetBinUpEdge(ix);
                for(int iy=1;iy<=NB;iy++){
                    ...
                    if(v<0.5||v>2.0) box.SetFillColor(kRed);
                    else             box.SetFillColor(kBlue);
                    box.SetLineColor(0); box.SetFillStyle(1001);
                    box.Draw("same");
                }
            }
        }
