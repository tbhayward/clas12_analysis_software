...
int main(int argc, char** argv) {
    ...
    // Loop over species: photons and electrons
    ...
    for (auto& sp : species) {
        ...
        // Reset to default ROOT palette for regular plots
        gStyle->SetOptStat(0);
        gStyle->SetPalette(1);
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            mu[i]    = count ? sum/count : 0;
            sigma[i] = count ? std::sqrt(sum2/count - mu[i]*mu[i]) : 0;
        }
        ...
        // Draw outliers map: two-color fill
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            hMap->Reset();
            for (int ix = 1; ix <= NB; ++ix) {
                double x1 = hMap->GetXaxis()->GetBinLowEdge(ix);
                double x2 = hMap->GetXaxis()->GetBinUpEdge(ix);
                for (int iy = 1; iy <= NB; ++iy) {
                    double v = hR[i]->GetBinContent(ix, iy);
                    if (v == 0) continue;
                    double y1 = hMap->GetYaxis()->GetBinLowEdge(iy);
                    double y2 = hMap->GetYaxis()->GetBinUpEdge(iy);
