...
table_data = []
# first row with units
table_data.append([f"{y0:.1f} events/nC"])
...
# Plot the fit line from 0 to 10 nA
x_fit = np.linspace(0, 10, 100)
y_fit = m * x_fit + b
ax.plot(x_fit, y_fit, '-', label=f'Fit: y = {m:.2f} x + {b:.2f}')

# Set x-axis from 0 to 10
ax.set_xlim(0, 10)

# Add a small table like a second legend in the upper right corner
table = ax.table(
    cellText=table_data,
    ...
    rowLoc='center',
    loc='upper right',
    bbox=[0.65, 0.70, 0.30, 0.25]  # [left, bottom, width, height] in axes fraction
)
