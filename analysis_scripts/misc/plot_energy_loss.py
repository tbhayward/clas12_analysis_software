...
def plot_electron_comparison(out_dir):
    ...
    for ax, p in zip(axs, [2.0, 4.0, 6.0]):
        ...
        ax.legend(loc='upper right', frameon=True)
    fig.suptitle('Electron Corrections: Java Fall 2018 Pass 2 vs jpsi inbending')
    fig.tight_layout(rect=[0,0,1,0.95])
    fig.savefig(f'{out_dir}/electron_corrections_comparison.png')
    plt.close(fig)
...
def main():
    ...
    plot_electron_comparison(out_dir)
