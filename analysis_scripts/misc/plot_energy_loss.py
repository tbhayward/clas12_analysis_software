...
def stefan_pi_plus_fd(theta, p):
    if theta < 27 and p < 2.3:
        ...
        dp = 0.00959007 - 0.0139218*1.9 + 0.0122966*1.9**2 - 0.0034012*1.9**3 + 0*p**4
...
def java_el_fall2018_pass2(phi_local, p, sec):
    """
    Electron momentum correction from first Java function:
    Fall 2018 Pass 2 (corEl == 3), as a function of local phi and momentum.
    """
...
def plot_run_periods(out_dir):
    ...
    for ax, p in zip(axs, [0.75, 1.75, 2.75]):
        ...
        ax.plot(theta_fd, [timothy_fd_fa18_out(t, p) for t in theta_fd],
...
def plot_pion_comparison(out_dir):
    ...
    for ax, p in zip(axs, [1.0, 2.0, 3.0]):
        ax.plot(theta_fd, [stefan_pi_plus_fd(t, p) for t in theta_fd], label='Stefan π⁺', linewidth=2)
        ax.plot(theta_fd, [krishna_pi_minus_fd(t, p) for t in theta_fd], label='Krishna π⁻', linestyle='--', linewidth=2)
        ax.axhline(0, linestyle='--', color='gray', linewidth=1)
        ax.set_xlim(5, 39)
        ax.set_ylim(-0.16, 0.16)
        ax.set_title(f'p = {p:.1f} GeV')
        ax.set_xlabel(r'$\theta$ (deg)')
        if ax is axs[0]:
            ax.set_ylabel(r'$\Delta p$ (GeV)')
        ax.legend(loc='lower left', frameon=True, fontsize='small')
...
def plot_electron_comparison(out_dir, bending):
    """Plot electron corrections for inbending or outbending."""
    phi_local = np.linspace(0, 60, 500)
    dp_func = jpsi_inb if bending == 'inbending' else jpsi_outb
    fig, axs = plt.subplots(1, 3, figsize=(12,4), sharey=True, gridspec_kw={'wspace':0})
    for ax, p in zip(axs, [2.0, 4.0, 6.0]):
        ...
        ax.plot(phi_local, [dp_func(p)]*len(phi_local),
        ...
        if ax is axs[0]:
            ax.set_ylabel(r'$\Delta p$ (GeV)')
        ax.legend(loc='upper right', frameon=True, fontsize='small')
    fig.suptitle(f'Electron Corrections: Momentum Taskforce vs J/#psi group {bending}')
    fig.tight_layout(rect=[0,0,1,0.95])
    fig.savefig(f'{out_dir}/electron_corrections_comparison_{bending}.png')
    plt.close(fig)
...
def main():
    ...
    plot_electron_comparison(out_dir, 'inbending')
    plot_electron_comparison(out_dir, 'outbending')
