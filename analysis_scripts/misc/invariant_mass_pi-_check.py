...
for ep -> enpi+ candidates, separately for RGC Su22, RGC Fa22, and RGC Sp23.
"""
import os
import sys
import numpy as np
...
def analyze_period(period_name, file_path, branches, edges):
    ...
    plt.close(fig)
    print(f"Saved plot for {period_name} to {out_path}")
...
def main():
    ...
    for name, path in periods:
        analyze_period(name, path, branches, edges)
