...
1) 2D histograms (4x3 grid), saved to:
   ...
   (These use the standard exclusivity window 0.81 < Mx2 < 1.00.)
...
3) NEW: Single 4x6 canvas of Mx2 distributions by (x_B slice × -t bin), saved to:
   ...
   For each subplot:
     ...
     • y-axis label: "counts"
   IMPORTANT: the Mx2 canvas uses NO Mx2 cut (only Q2/W/y + fiducial + x_B + -t).
"""
...
# Optional SciPy for non-linear least squares (will fall back gracefully if unavailable)
try:
...
def load_arrays(path, include_mx2_cut=True):
    """
    Read branches, compute -t, wrap phi.
    Always apply: Q2 > 1, W > 2, y < 0.75.
    If include_mx2_cut is True, also apply 0.81 < Mx2 < 1.00.
    """
    try:
    ...
    base_mask = (Q2 > 1.0) & (W > 2.0) & (y < 0.75)
    if include_mx2_cut:
        base_mask = base_mask & (Mx2 > 0.81) & (Mx2 < 1.00)
    #endif
...
# ─────────────────────────────────────────────────────────────────────
# Figure 3: Mx2 distributions by (x_B slice × -t bin), with fits (NO Mx2 cut)
# ─────────────────────────────────────────────────────────────────────
...
def _fit_gauss_plus_quad(xc, y, yerr, mu0=M_P2, sigma0=0.03):
    ...
    Uses SciPy if available; otherwise a grid search over (mu, sigma)
...
def make_mx2_canvas(data_no_mx2cut, outdir):
    """
    Build a 4x6 canvas of Mx2 distributions (NO Mx2 cut in the input data):
    ...
    for r, sname in enumerate(slice_order):
        ...
        base_mask = xb_slice_mask_for_mx2(data_no_mx2cut, xb_range)
        for c in range(nbins):
            tmin, tmax = edges[c], edges[c+1]
            ax = axes[r, c]

            # mask for this cell
            mask = base_mask & tbin_mask(data_no_mx2cut, tmin, tmax)
            vals = data_no_mx2cut["Mx2"][mask]
...
def main():
    ...
    # For figures 1 and 2, keep the standard Mx2 exclusivity window
    data_with_mx2cut = load_arrays(infile, include_mx2_cut=True)

    # For the Mx2 canvas, load WITHOUT the Mx2 cut
    data_no_mx2cut = load_arrays(infile, include_mx2_cut=False)

    make_2d_canvases(data_with_mx2cut, outdir)
    make_binned_canvas_4x6(data_with_mx2cut, outdir)
    make_mx2_canvas(data_no_mx2cut, outdir)
#endfor
