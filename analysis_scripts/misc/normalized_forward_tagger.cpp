...
int main(int argc, char** argv) {
    ...
    for (auto& sp: species) {
        ...
        for(Long64_t e=0;e<nD;++e){ dataCh.GetEntry(e); if(pid!=pidVal) continue;
        ...
        for(Long64_t e=0;e<nM;++e){ mcCh.GetEntry(e); if(pid!=pidVal) continue;
            double xs[3]={x1,x4,x7}, ys[3]={y1,y4,y7};
            for(int i=0;i<3;i++) if(xs[i]!=-9999 && ys[i]!=-9999) hM[i]->Fill(xs[i],ys[i]);
        }
        gStyle->SetOptStat(0);
        // Unnormalized
        TCanvas c1("c1","DataUncut",1800,600); c1.Divide(3,1);
        ...
        TCanvas c2("c2","MCUncut",1800,600); c2.Divide(3,1);
        ...
        for(int i=0;i<3;i++){
            ...
            for(int ix=1;ix<=NB;ix++) for(int iy=1;iy<=NB;iy++){ double v=hR[i]->GetBinContent(ix,iy); if(v==0) continue; sum+=v; sum2+=v*v; cnt++; }
            mu[i]=cnt?sum/cnt:0; sigma[i]=cnt?sqrt(sum2/cnt-mu[i]*mu[i]):0;
        }
        TCanvas c3("c3","Ratio",1800,600); c3.Divide(3,1);
        ...
        // Scatter outliers
        TCanvas c4("c4","Outliers",1800,600); c4.Divide(3,1);
        for(int i=0;i<3;i++){
            c4.cd(i+1);
            // draw axes
            hR[i]->Draw("axis");
            ...
            for(int ix=1; ix<=NB; ix++){
                double x = hR[i]->GetXaxis()->GetBinCenter(ix);
                for(int iy=1; iy<=NB; iy++){
                    double y = hR[i]->GetYaxis()->GetBinCenter(iy);
                    double v = hR[i]->GetBinContent(ix,iy);
                    if(v==0) continue;
                    if(v>2.0||v<0.5) gOut.SetPoint(gOut.GetN(), x,y);
                    else             gIn.SetPoint(gIn.GetN(),  x,y);
                }
            }
            gIn.Draw("P"); gOut.Draw("P same");
        }
