...
Cut definition (THIS VERSION):
  - Enforced cut window is (left@LEVEL, right@LEVEL) for EACH histogram (data and mc separately).
  - LEVEL is now 0.2% = 0.0020 in normalized units (requested).
...
# Target normalized level for "0.2%"
TARGET_LEVEL = 0.002  # 0.2% in unit-normalized histogram units
...
def validate_inputs() -> None:
    ...
    #endif
    if TARGET_LEVEL >= 1.0:
        fatal(f"Invalid TARGET_LEVEL={TARGET_LEVEL}, must be < 1")
    #endif
    ...
    try:
        with uproot.open(root_path) as f:
            ...
            h = Hist1D(
                ...
                left_level_vz=float(left_vz),
                left_level_y=float(left_y),
                right_level_vz=float(right_vz),
                right_level_y=float(right_y),
...
def plot_2x3_canvas(
    ...
    for period_label, (r, c) in PANEL_POS.items():
        ...
        # Level-based cut lines: DATA (black), MC (red)
...
def print_summary(pid: int, pid_label: str, data_hists: Dict[str, Hist1D], mc_hists: Dict[str, Hist1D]) -> None:
    ...
    print(f"  Histogram range = ({VZ_MIN:.1f}, {VZ_MAX:.1f}) (cm), N_BINS={N_BINS}, target level = {TARGET_LEVEL:.4f} ({100.0*TARGET_LEVEL:.2f}%)")
    print("  Enforced cut window is (left@level_vz, right@level_vz) for EACH histogram (data and mc separately).")
    print("------------------------------------------------------------")
    ...
    for period_label in periods:
        ...
        print(f"        left@level: vz={dh.left_level_vz:.4f} y={dh.left_level_y:.6f}   right@level: vz={dh.right_level_vz:.4f} y={dh.right_level_y:.6f}")
        ...
        print(f"        left@level: vz={mh.left_level_vz:.4f} y={mh.left_level_y:.6f}   right@level: vz={mh.right_level_vz:.4f} y={mh.right_level_y:.6f}")
    #endfor
...
def main() -> None:
    ...
    for pid, pid_label, fname in particles:
        ...
        title = f"Vertex z comparison: {pid_label} (pid={pid}) [unit-normalized, log-y, {100.0*TARGET_LEVEL:.1f}% cuts]"
