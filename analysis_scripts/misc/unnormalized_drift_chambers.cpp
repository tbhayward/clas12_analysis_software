/*****************************************************************************
 * File: separate_drift_chambers.cpp
 *
 * Compile with:
 *   g++ separate_drift_chambers.cpp -o separate_drift_chambers \
 *       $(root-config --cflags --libs)
 *
 * Run with:
 *   ./separate_drift_chambers [NeventsSIDISDVCS] [dataFile] [mcFile]
 *
 * Same arguments as normalized_drift_chambers.cpp, but this version:
 *   • Fills and normalizes data and MC histograms separately.
 *   • Does **not** compute any data/MC ratios.
 *   • Produces canvases for data and MC in parallel.
 *   • Saves all images under output/normalization/.
 *****************************************************************************/
...
// Helper: unify 2D color scale across three histograms
...
void SetSame1DScale(TH1D* a, TH1D* b, TH1D* c) {
    double mn = 1e9, mx = -1e9;
    for (auto* h : {a,b,c}) {
        mn = std::min(mn, h->GetMinimum());
        mx = std::max(mx, h->GetMaximum());
...
int main(int argc, char** argv) {
    ...
    if (useDataFile) dataCh.Add(dataFile.c_str());
        ...
        dataCh.Add(".../sidisdvcs_0.root");
        dataCh.Add(".../sidisdvcs_1.root");
        dataCh.Add(".../sidisdvcs_2.root");
    }
    if (useMCFile) mcCh.Add(mcFile.c_str());
    else mcCh.Add(".../clasdis.root");

    // 3) branch addresses
    Int_t    pid;
    Double_t x6,y6,x18,y18,x36,y36, e6,e18,e36;
    ...
    const double R1=140, R2=215, R3=290;
    ...
    TH1D *d_e1=new TH1D("d_e1","Data edge R1;cm",nb1,0,125),
         ...
         *m_e1=new TH1D("m_e1","MC   edge R1;cm",nb1,0,125),
    ...
    // 7) normalize each histogram individually
    auto norm = [](TH1* h){ double I=h->Integral(); if (I>0) h->Scale(1/I); };
    for (auto* h : {d_r1,d_r2,d_r3,d_r1c,d_r2c,d_r3c,d_e1,d_e2,d_e3,d_rp1,d_rp2,d_rp3})
        norm(h);
    for (auto* h : {m_r1,m_r2,m_r3,m_r1c,m_r2c,m_r3c,m_e1,m_e2,m_e3,m_rp1,m_rp2,m_rp3})
        norm(h);
    ...
    // 8) draw & save data canvases
    {
        ...
        c.cd(1); d_r1->Draw("COLZ");
        c.cd(2); d_r2->Draw("COLZ");
        c.cd(3); d_r3->Draw("COLZ");
        c.SaveAs("output/normalization/data_uncut.png");
    }
    {
        TCanvas c("data_circle","Data Circle Overlay",1800,600);
        c.Divide(3,1);
        for (int i=0;i<3;i++){ c.cd(i+1); gPad->SetRightMargin(0.15); gPad->SetLeftMargin(0.15); gPad->SetLogz(); }
        SetSame2DScale(d_r1,d_r2,d_r3);
        for (int i=0;i<3;i++){
            c.cd(i+1);
            TH2D* h = (i==0?d_r1:(i==1?d_r2:d_r3));
    ...
    }
    {
        ...
        c.cd(1); d_r1c->Draw("COLZ");
        c.cd(2); d_r2c->Draw("COLZ");
        c.cd(3); d_r3c->Draw("COLZ");
    ...
    }
    {
        ...
        c.cd(1); d_e1->Draw("HIST");
        c.cd(2); d_e2->Draw("HIST");
        c.cd(3); d_e3->Draw("HIST");
    ...
    }
    {
        ...
        c.cd(1); gPad->SetLogz(); d_rp1->Draw("COLZ");
        c.cd(2); gPad->SetLogz(); d_rp2->Draw("COLZ");
        c.cd(3); gPad->SetLogz(); d_rp3->Draw("COLZ");
        c.SaveAs("output/normalization/data_radphi.png");
    }

    // 9) draw & save MC canvases (mirror of above)
    {
        TCanvas c("mc_uncut","MC Uncut",1800,600);
        c.Divide(3,1);
        for (int i=0;i<3;i++){ c.cd(i+1); gPad->SetLogz(); gPad->SetLeftMargin(0.15); gPad->SetRightMargin(0.15); }
        SetSame2DScale(m_r1,m_r2,m_r3);
        c.cd(1); m_r1->Draw("COLZ");
        c.cd(2); m_r2->Draw("COLZ");
        c.cd(3); m_r3->Draw("COLZ");
    ...
    }
    {
        ...
        c.cd(1); m_r1c->Draw("COLZ");
        c.cd(2); m_r2c->Draw("COLZ");
        c.cd(3); m_r3c->Draw("COLZ");
    ...
    }
    {
        ...
        c.cd(1); m_e1->Draw("HIST");
        c.cd(2); m_e2->Draw("HIST");
        c.cd(3); m_e3->Draw("HIST");
    ...
    }
    {
        ...
        c.cd(1); gPad->SetLogz(); m_rp1->Draw("COLZ");
        c.cd(2); gPad->SetLogz(); m_rp2->Draw("COLZ");
        c.cd(3); gPad->SetLogz(); m_rp3->Draw("COLZ");
