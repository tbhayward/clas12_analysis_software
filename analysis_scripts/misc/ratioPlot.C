...
void ratioPlot() {
    ...
    TH1F *h1 = new TH1F("h1", "pi- counts", 50, 1, 6);
    TH1F *h2 = new TH1F("h2", "k- counts", 50, 1, 6);
    ...
    // Style the plot
    gStyle->SetOptStat(0);
    hRatio->SetTitle("Positives Outbending;p (GeV);#pi^{+} /k^{+}");
    ...
    c1->SaveAs("output/positive_out_ratio.png");
