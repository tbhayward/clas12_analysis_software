...
int main() {
    ...
    for (int bin = 0; bin < 17; ++bin) { 
        ...
        for (int i = 0; i < num_pT_bins[bin] * num_z_bins[bin]; ++i) {
            hData[bin].push_back(new TH1F(Form("hData_bin%d_%d", bin+1, i), ";#phi;Normalized Counts", 36, 0, 2*TMath::Pi()));
            hMCReco[bin].push_back(new TH1F(Form("hMCReco_bin%d_%d", bin+1, i), ";#phi;Normalized Counts", 36, 0, 2*TMath::Pi()));
            hMCGene[bin].push_back(new TH1F(Form("hMCGene_bin%d_%d", bin+1, i), ";#phi;Normalized Counts", 36, 0, 2*TMath::Pi()));
        }
    }
