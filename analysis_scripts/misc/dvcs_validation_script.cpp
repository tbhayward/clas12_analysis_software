...
void plot_elastic_energy_loss_validation(const char* file1, const char* file2, const char* titleSuffix) {
    ...
    // Create histograms for each theta bin with 50 bins
    h1[0] = new TH1D("h1_integrated", Form("Integrated #theta [5, 65] %s", titleSuffix), 30, -0.15, 0.15);
    h2[0] = new TH1D("h2_integrated", Form("Integrated #theta [5, 65] %s", titleSuffix), 30, -0.15, 0.15);

    for (int i = 0; i < nBins; ++i) {
        h1[i + 1] = new TH1D(Form("h1_%d", i), Form("#theta [%.0f, %.0f] %s", thetaBins[i], thetaBins[i + 1], titleSuffix), 30, -0.1, 0.15);
        h2[i + 1] = new TH1D(Form("h2_%d", i), Form("#theta [%.0f, %.0f] %s", thetaBins[i], thetaBins[i + 1], titleSuffix), 30, -0.1, 0.15);
    }
