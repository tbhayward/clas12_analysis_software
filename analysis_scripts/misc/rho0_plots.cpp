...
void plotCombinationDependence(
    ...
    for (size_t i = 0; i < suffixes.size(); ++i) {
        ...
        if (it0 != asymmetryData.end() && it1 != asymmetryData.end() && it2 != asymmetryData.end()) {
            ...
            graph1 = createTGraphErrors(x1, y1, y1Err, 21, 1.0, kGreen+1);  // Red circles
            graph2 = createTGraphErrors(x2, y2, y2Err, 22, 1.0, kBlue); // Blue circles
            ...
            if (entry0) entry0->SetTextColor(kRed);  // Set color of the first entry to green
            ...
            if (entry1) entry1->SetTextColor(kGreen+1);  // Set color of the second entry to red
...
int main(int argc, char *argv[]) {
    ...
    plotCombinationDependence(asymmetryData, 
        ...
        {0.0, 0.04},  // y range for ALUsinphi
        {0.1, 0.45},  // y range for ALL
        "output/rho0_plots/PT_epipluspiminus_combination_dependence_plots_test.png", 
        {
            "",  // Baseline (black)
            "e'#pi^{+}#pi^{-}X, M_{x (#pi^{+})} > 1.5 (GeV)",  // Dataset 1 (red)
            "e'#pi^{+}#pi^{-}X, M_{x (#pi^{+})} > 1.5 (GeV), M_{x (#pi^{+}#pi^{-})} > 1.5 (GeV)"  // Dataset 2 (blue)
