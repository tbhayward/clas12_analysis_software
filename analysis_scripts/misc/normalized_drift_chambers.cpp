 ...
 *   6) Draws “COLZ” canvases for Data, MC, and two ratio versions
 *      (original and strict) with survival-percentage values embedded in the pad title.
...
bool dc_fiducial_cut_strict(int torus, int pid, double theta_deg,
    ...
    double r6=std::hypot(x6,y6), r18=std::hypot(x18,y18), r36=std::hypot(x36,y36);
    if (torus<0 && pid==11) {
        bool pass=(theta_deg<10 ? (edge1>14&&edge2>14&&edge3>14)
                                : (edge1>6 && edge2>6 && edge3>14));
        if (!pass||r6<50||r18<70||r36<70||r36>260) return false;
        return true;
    }
    if (torus<0 && pid==2212) {
        bool pass=(theta_deg<10 ? (edge1>10&&edge2>10&&edge3>10)
                                : (edge1>3 && edge2>3 && edge3>10));
        if (!pass||r6<60||r6>150||r18<120||r18>210||r36<280||r36>380) return false;
        return true;
    }
    if (torus>0 && pid==11) {
        bool pass=edge1>3&&edge2>3&&edge3>14;
        if (!pass||r6<35||r6>60||r18<78||r18>98||r36<100||r36>180) return false;
        return true;
    }
    if (torus>0 && pid==2212) {
        bool pass=edge1>3&&edge2>3&&edge3>14;
        if (!pass||r6<85||r6>145||r18<120||r18>190||r36<90||r36>250) return false;
        return true;
    }
    return false;
}
...
void SetSame2DScale(TH2D* a, TH2D* b, TH2D* c) {
    ...
    for (auto*h:{a,b,c}) { mn = std::min(mn,h->GetMinimum()); mx = std::max(mx,h->GetMaximum()); }
    for (auto*h:{a,b,c}) { h->SetMinimum(mn); h->SetMaximum(mx); }
}

int main(int argc, char** argv) {
    if (argc<5) {
        std::cerr<<"Usage: "<<argv[0]<<" [Nevents] [dataFile] [mcFile] [torus]\n";
        return 1;
    }
    gSystem->mkdir("output",kTRUE);
    gSystem->mkdir("output/dc",kTRUE);

    Long64_t maxE=std::stoll(argv[1]); if(maxE==0) maxE=-1;
    ...
    // booking histograms
    std::vector<std::vector<TH2D*>> Dorig(species.size()), Morig(species.size()), Rorig(species.size()),
                                     Dstr (species.size()), Mstr (species.size()), Rstr (species.size());
    for (size_t s=0; s<species.size(); ++s) {
        auto& lab=species[s].second;
        for (int r=0; r<3; ++r) {
            Dorig[s].push_back(new TH2D(Form("Dorig_%s_r%d",lab.c_str(),r+1),
                                        Form("%s Data Orig R%d; x; y",lab.c_str(),r+1),
                                        NB2,xmins[r],xmaxs[r],NB2,xmins[r],xmaxs[r]));
            Morig[s].push_back((TH2D*)Dorig[s][r]->Clone(Form("Morig_%s_r%d",lab.c_str(),r+1)));
            Dstr [s].push_back(new TH2D(Form("Dstr_%s_r%d",lab.c_str(),r+1),
                                        Form("%s Data Strict R%d; x; y",lab.c_str(),r+1),
                                        NB2,xmins[r],xmaxs[r],NB2,xmins[r],xmaxs[r]));
            Mstr [s].push_back((TH2D*)Dstr[s][r]->Clone(Form("Mstr_%s_r%d",lab.c_str(),r+1)));
        }
    }

    // counters
    std::vector<std::vector<Long64_t>> nNo(species.size(),std::vector<Long64_t>(3,0)),
                                    nOr(species.size(),std::vector<Long64_t>(3,0)),
                                    nSt(species.size(),std::vector<Long64_t>(3,0));

    // fill data
    Long64_t nD=dataCh.GetEntries(); if(maxE>0&&maxE<nD) nD=maxE;
    ...
    // print survival rates to console
    ...
    for (size_t s=0; s<species.size(); ++s) {
        ...
        std::cout << lab
                  << ": original fiducial survival = " << pOr << "%, "
                  << "strict fiducial survival = " << pSt << "%\n";
    }
    std::cout << "=============================\n";

    // fill MC
    Long64_t nM=mcCh.GetEntries(); if(maxE>0&&maxE<nM) nM=maxE;
    ...
    for (size_t s=0; s<species.size(); ++s) {
        for (int r=0; r<3; ++r) {
            ...
            Rorig[s].push_back((TH2D*)Dorig[s][r]->Clone(Form("Rorig_%s_r%d",species[s].second.c_str(),r+1)));
            ...
            if (cnt>0) { mO[s][r]=sum/cnt; sO[s][r]=std::sqrt(sum2/cnt-mO[s][r]*mO[s][r]); }

            double dI2=Dstr[s][r]->Integral();
            if (dI2>0) Dstr[s][r]->Scale(1/dI2);
            double mI2=Mstr[s][r]->Integral();
            if (mI2>0) Mstr[s][r]->Scale(1/mI2);
            Rstr[s].push_back((TH2D*)Dstr[s][r]->Clone(Form("Rstr_%s_r%d",species[s].second.c_str(),r+1)));
            ...
            if (cnt>0) { mS[s][r]=sum/cnt; sS[s][r]=std::sqrt(sum2/cnt-mS[s][r]*mS[s][r]); }
        }
    }

    gStyle->SetOptStat(0);

    // draw original ratio with survival % in pad title
    for (size_t s=0; s<species.size(); ++s) {
        auto &lab=species[s].second;
        TCanvas c(Form("RatioOrig_%s",lab.c_str()),Form("%s Ratio Orig",lab.c_str()),1800,600);
        c.Divide(3,1);
        SetSame2DScale(Rorig[s][0],Rorig[s][1],Rorig[s][2]);
        for (int r=0; r<3; ++r) {
            c.cd(r+1);
            double pO = nNo[s][r]?100.*nOr[s][r]/nNo[s][r]:0;
            double pS = nNo[s][r]?100.*nSt[s][r]/nNo[s][r]:0;
            Rorig[s][r]->SetTitle(Form("R%d: Orig: %.1f%%, Strict: %.1f%%; x; y",
                                       r+1,pO,pS));
    ...
    // draw strict ratio likewise
    for (size_t s=0; s<species.size(); ++s) {
        auto &lab=species[s].second;
        TCanvas c(Form("RatioStr_%s",lab.c_str()),Form("%s Ratio Strict",lab.c_str()),1800,600);
        c.Divide(3,1);
        ...
        for (int r=0; r<3; ++r) {
            ...
            double pO = nNo[s][r]?100.*nOr[s][r]/nNo[s][r]:0;
            double pS = nNo[s][r]?100.*nSt[s][r]/nNo[s][r]:0;
            Rstr[s][r]->SetTitle(Form("R%d: Orig: %.1f%%, Strict: %.1f%%; x; y",
                                      r+1,pO,pS));
            gPad->SetLeftMargin(.15);
            gPad->SetRightMargin(.15);
            gPad->SetLogz();
    ...
    for (size_t s=0; s<species.size(); ++s) {
        auto &lab=species[s].second;
        TCanvas c(Form("OutlOrig_%s",lab.c_str()),Form("%s Outliers Orig",lab.c_str()),1800,600);
        c.Divide(3,1);
        for (int r=0; r<3; ++r) {
            c.cd(r+1);
            gPad->SetLeftMargin(.15);
    ...
    // draw outliers strict
    for (size_t s=0; s<species.size(); ++s) {
        auto &lab=species[s].second;
        TCanvas c(Form("OutlStr_%s",lab.c_str()),Form("%s Outliers Strict",lab.c_str()),1800,600);
        c.Divide(3,1);
        for (int r=0; r<3; ++r) {
            c.cd(r+1);
            gPad->SetLeftMargin(.15);
