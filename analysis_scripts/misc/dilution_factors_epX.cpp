...
void dilution_factors_epX(const char* nh3_file, const char* c_file) {
    ...
    TH1D *h_xF_nh3 = new TH1D("h_xF_nh3", "M_{x} Distribution; M_{x} (GeV); Counts", 100, -2, 3);
    TH1D *h_xF_carbon = new TH1D("h_xF_carbon", "M_{x} Distribution; M_{x} (GeV); Counts", 100, -2, 3);

    // Fill the histograms
    tree_nh3->Draw("Mx>>h_xF_nh3");
    tree_carbon->Draw("Mx>>h_xF_carbon");

    // First panel: plot xF histograms
    c1->cd(1);
    ...
    for (int i = 1; i <= h_xF_nh3->GetNbinsX(); ++i) {
        ...
        if (c_counts > 0) {
            ...
            gr_ratio->SetPoint(i - 1, h_xF_nh3->GetBinCenter(i), ratio);
