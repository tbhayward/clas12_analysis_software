...
std::pair<double, double> scale_normalization(const char* nh3_file, const char* c_file) {
    ...
    //
    TH1D *h_xF_nh3 = 
        new TH1D("h_xF_nh3", "x_{F} Distribution; x_{F} (GeV); Counts", 50, -3.5, 1);
    TH1D *h_xF_carbon = 
        new TH1D("h_xF_carbon", "x_{F} Distribution; x_{F} (GeV); Counts", 50, -3.5, 1);
    // Fill the histograms
    ...
    // Set x-axis range
    gr_ratio_xF->GetXaxis()->SetLimits(-3.5, 1);
    ...
    // Fit the data from -2.5 to -1 to a constant
    TF1 *fit_const_xF = new TF1("fit_const", "[0]", -3.5, -1.5);
