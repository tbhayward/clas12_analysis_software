...
def diagnostic_file_check(args, n_data_entries, n_reco_mc_entries, n_gen_from_file, n_gen):
    status("DIAGNOSTIC FILE CHECK:")
    status("  data_root    = {}".format(args.data_root))
    status("  reco_mc_root = {}".format(args.reco_mc_root))
    status("  gen_mc_root  = {}".format(args.gen_mc_root))
    status("  data entries = {}".format(n_data_entries))
    status("  reco entries = {}".format(n_reco_mc_entries))
    status("  gen entries  = {}".format(n_gen_from_file))
    status("  N_GEN used   = {:.12g}".format(n_gen))
#enddef


def summarize_counts_for_variable(result, variable_key):
    if variable_key not in result["counts_by_variable"]:
        return 0.0, []
    #endif
    ...
    for panel_counts in result["counts_by_variable"][variable_key]:
        panel_total = sum(panel_counts)
        panel_totals.append(panel_total)
        total += panel_total
    #endfor

    return total, panel_totals
#enddef


def print_data_mc_integral_diagnostics(data_result, mc_result):
    status("DIAGNOSTIC DATA/MC HISTOGRAM INTEGRALS:")

    for variable_key in get_variable_keys():
        data_total, data_panels = summarize_counts_for_variable(data_result, variable_key)
        mc_total, mc_panels = summarize_counts_for_variable(mc_result, variable_key)
        ...
        status("  {} total: data = {:.12g}, MC = {:.12g}, data/MC = {:.12g}".format(
            variable_key,
            data_total,
            mc_total,
            ratio
        ...
        status("  {} panels data = {}".format(
            variable_key,
            ", ".join("{:.12g}".format(x) for x in data_panels)
        ))
        status("  {} panels MC   = {}".format(
            variable_key,
            ", ".join("{:.12g}".format(x) for x in mc_panels)
        ))
    #endfor
#enddef
...
def main():
    ...
    diagnostic_file_check(args, n_data_entries, n_reco_mc_entries, n_gen_from_file, n_gen)
    ...
    status("DIAGNOSTIC MC WEIGHT CHECK:")
    status("  raw_weight_sum over any-filled entries = {:.12g}".format(mc_result["raw_weight_sum"]))
    status("  scaled_weight_sum over any-filled entries = {:.12g}".format(mc_result["scaled_weight_sum"]))
    status("  max_raw_weight = {:.12g}".format(mc_result["max_raw_weight"]))

    print_data_mc_integral_diagnostics(data_result, mc_result)
