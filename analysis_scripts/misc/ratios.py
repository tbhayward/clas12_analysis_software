...
def draw_ratio(h_num, h_den, n_bins, x_min, x_max, xlabel):
    ...
    global _ratio_count
    name = f"h_ratio_{_ratio_count}"
    _ratio_count += 1

    hr = ROOT.TH1F(name, f"Ratio; {xlabel}; ratio", n_bins, x_min, x_max)
...
def make_canvas(p_branch, theta_branch, p_label, theta_label,
    ...
    """
    Create a 2×2 canvas:
      top-left: momentum histograms (range p_min→p_max),
      top-right: momentum ratio,
      bottom-left: theta histograms (range t_min→t_max),
    ...
    # Top-right: momentum ratio
    c.cd(2)
    draw_ratio(h_p13, h_p11, p_bins, p_min, p_max, f"{p_label} (GeV)").Draw("E1")
    ...
    # Bottom-right: theta ratio
    c.cd(4)
    draw_ratio(h_t13, h_t11, t_bins, t_min, t_max, f"{theta_label} (deg)").Draw("E1")

    c.SaveAs(out_pdf)

# Electron canvas (1→4.5 GeV, 5→35°)
make_canvas(
    p_branch="e_p",
