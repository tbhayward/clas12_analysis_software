...
#   (3) PDF plots (same style as your prior migration script):
...
def main():
    ...
    ap.add_argument("fit_out_name", help="Output file name only (no directory). Saved under output/rgc_enpi+_mx2_fermi_migration_study/")
    ...
    out_dir = "output/rgc_enpi+_mx2_fermi_migration_study"
    ...
    # Plotting: inject sys into the baseline map (so points are baseline, sys rectangles are migration sys)
