...
Cuts (applied to all events before counting):
  fiducial_status >= 111
  0.81 < Mx2 < 1.00
  xB in the slice range (see X_SLICES below)
...
# Try to import uproot / matplotlib with a friendly message if missing
try:
    import uproot
except Exception:
...
def slice_index_from_x(x):
    ...
    or -1 if x is outside all slice ranges.
    """
    idx = np.full(x.shape, -1, dtype=np.int16)
    for k, name in enumerate(SLICE_ORDER):
        xa, xb = X_SLICES[name]
        m = (x > xa) & (x < xb)   # open intervals like your usual cuts
        idx[m] = k
    return idx
...
def main():
    ...
    branches = [
        "fiducial_status", "x", "t", "Mx2",
        "matching_e_pid", "matching_p1_pid",
    ]

    # Accumulators: totals and mis-id counts per (xB slice, -t bin)
    totals = np.zeros((4, NTBINS), dtype=np.int64)
    bads   = np.zeros((4, NTBINS), dtype=np.int64)

    # Iterate in chunks to be memory-friendly
    step = "200 MB"
    try:
    ...
    fig.suptitle(r"Misidentification rate vs $-t$"
                 "\n" r"(percentage with $matching\_e\_pid=-211$ and $matching\_{p1}\_pid=321$; "
                 r"$0.81<M_{x}^{2}<1.00$, $fid\ge 111$)", fontsize=13, y=0.98)
