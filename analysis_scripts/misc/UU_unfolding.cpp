...
int main() {
    ...
    for (Long64_t i = 0; i < nDataEntries; ++i) {
        ...
        if (binIndex >= 0) {
            ...
            if (pT_bin != -1 && z_bin != -1) {
                int histIndex = (num_z_bins[binIndex]-z_bin+1) * num_pT_bins[binIndex] + pT_bin - 1;
                std::cout << zData << " " << pTData << std::endl;
                std::cout << (z_bin) << " " << num_pT_bins[binIndex] << " " << pT_bin << " " << (num_z_bins[binIndex]-z_bin) * num_pT_bins[binIndex] + pT_bin - 1 << std::endl << std::endl;
