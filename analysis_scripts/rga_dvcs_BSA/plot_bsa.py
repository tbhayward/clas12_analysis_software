...
def plot_a1_vs_t_by_Q2(binning_csv, final_dir="final_results", output_dir="bsa_plots/a1_vs_t_by_Q2",
    ...
    Only the far-left subplot displays the y-axis label and tick marks.
    Each subplot shows its own x-axis label and tick marks.
    The x-axis is forced to range from 0 to 1.
    The legend shows the average xB value for each curve.
    Additionally, set the y-axis of each subplot to range from -0.2 to 0.6 and add a light
    dashed gray line at y = 0.
    The far right subplot displays y-axis tick labels on the right.
    """
    plt.style.use(PLOT_STYLE)
    binning = load_binning_scheme(binning_csv)
    global_means = load_global_bin_means(global_means_file)
    ...
    for idx, i_Q2 in enumerate(sorted(Q2_indices)):
        ...
        ax.set_ylim(-0.2, 0.6)
        # Add horizontal dashed gray line at y = 0.
        ...
        ax.grid(True, alpha=0.3)
        ax.set_xlabel("-t")
        # Only the far-left subplot gets the y-axis label and tick labels.
        if idx == 0:
