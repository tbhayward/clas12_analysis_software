...
For each unique xB bin we produce a canvas (figure) whose rows are Q² bins and columns are t bins.
...
# Import the binning scheme loader (assumes you have load_binning_scheme.py)
from load_binning_scheme import load_binning_scheme

# Global constants: 12 phi bins with edges from 0 to 2pi
N_PHI_BINS = 12
...
def plot_contamination_for_run(run_period,
    ...
    Each plot is a grid with rows = unique Q² bins and columns = unique t bins.
    In each cell we plot the contamination (with error bars) vs. φ (in degrees).
    """
    # Load contamination data from JSON
    contamination = load_contamination(run_period, contamination_dir)
    
    # Load the binning scheme from the CSV
    binning_scheme = load_binning_scheme(binning_csv)
    
    # Compute the unique xB, Q², and t bin boundaries from the binning scheme.
    ...
    # Loop over each unique xB bin. (There should be about 8.)
    for i_xB, (xB_min, xB_max) in enumerate(unique_xB_bins):
        ...
        # For each Q² and t bin, plot the 12 phi bins
        for i_Q2, (Q2_min, Q2_max) in enumerate(unique_Q2_bins):
            ...
            for i_t, (t_min, t_max) in enumerate(unique_t_bins):
                ...
                # Add a title with the average Q² and t for the cell.
        ...
        fig.suptitle(rf"$x_B={xB_avg:.3f}$", fontsize=12)
