...
for cff, Im_fit in Im_funcs.items():
    ...
    for idx,(ax,tval) in enumerate(zip(axes1,t_fixed)):
        ...
        if idx==2:
            ax.legend(["Original Parameters","RGA pass-1"],
    ...
    # hide zero tick labels
    for lbl in axes1[0].get_yticklabels():
        if lbl.get_text()=='0': lbl.set_visible(False)
    for ax in (axes1[4],axes1[5]):
        for lbl in ax.get_xticklabels():
            if lbl.get_text()=='0.0': lbl.set_visible(False)

    # add title
    fig1.suptitle(fr"$\mathrm{{Im}}\,{cff}$ vs $\xi$", fontsize=16, y=0.96)
    fig1.subplots_adjust(left=0.10,right=0.98,bottom=0.08,top=0.90,wspace=0,hspace=0)
    ...
    fig2.suptitle(fr"$\mathrm{{Im}}\,{cff}$ vs $-t$", fontsize=16, y=0.96)
    fig2.subplots_adjust(left=0.10,right=0.98,bottom=0.08,top=0.90,wspace=0,hspace=0)
