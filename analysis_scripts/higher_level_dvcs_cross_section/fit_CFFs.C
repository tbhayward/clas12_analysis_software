...
void parse_args(int argc, char** argv){
    ...
    int c;
    while((c=getopt_long(argc,argv,"s:h:t:e:x:",opts,0))!=-1){
        switch(c){
          ...
          default:
            std::cerr<<"Usage: "<<argv[0]<<" --strategy <1|2> -H<E> -Ht<E> -E<E> -Et<E>\n";
            std::exit(1);
        }
    }
...
int main(int argc, char** argv){
...

    {
      ...
      for(int i=0;i<nim;++i){
        ...
        if(nm=="n_H")       init=1.35;
        if(nm=="n_Ht")      init=0.6;
        if(nm=="alpha0_E"||nm=="alpha0_Et") init=0.43;
        if(nm=="alpha1_E"||nm=="alpha1_Et") init=0.85;
