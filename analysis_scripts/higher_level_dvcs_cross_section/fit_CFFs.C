 ...
 *   constraint=1 : drop points with -t/Q2 >= 0.2 or -t >= 1,
 *                  and drop any bin with χ²_red>=2 or |B|>=0.9
 *
 * Compile:
...
// Bin & sinφ fit → bin_A,bin_dA,bin_chi2; then drop bad bins if constraint==1
void BinBsaData(){
    ...
    for(size_t i=1;i<=bsaData.size();++i){
        ...
        start = i;
        int M = pts.size();
        if(M<3) continue;  // need >3 points for sinφ/(1+Bcosφ) fit

        // compute A_bin,dA_bin, χ2 for sinφ fit
        double SwA=0, Sw2=0;
        for(auto &d: pts){
        ...
        // quick B fit + red χ2,bin
        TF1 ftmp("ftmp","[0]*sin(x*TMath::Pi()/180.)/(1+[1]*cos(x*TMath::Pi()/180.))",0,360);
        ftmp.SetParameters(A_bin,0.);
        TGraphErrors gr(M);
        for(int j=0;j<M;++j){ gr.SetPoint(j, pts[j].phi, pts[j].A ); gr.SetPointError(j,0,pts[j].sigA); }
        gr.Fit(&ftmp,"Q0R");
        double Bfit     = ftmp.GetParameter(1);
        double redChi2b = chi2 / (M-1);

        if(gConstraint==1){
          if(redChi2b>=2.0 || std::fabs(Bfit)>=0.9) continue;
        }
        ...
        for(auto &d: pts){ double w = 1./(d.sigA*d.sigA); sumw+=w; Sx+=w*d.xB; Sq+=w*d.Q2; St+=w*d.t; Se+=w*d.Eb; }
    ...
    Nbins = bin_A.size();
    // average chi2 per bin-fit
    double totChi2 = std::accumulate(bin_chi2.begin(), bin_chi2.end(), 0.0);
    reducedAmpChi2 = Nbins>0 ? totChi2/Nbins : 0.0;
}

static std::vector<std::string> parNamesIm;
void build_par_list(){
    parNamesIm.clear();
    if(hasH )  parNamesIm.insert(parNamesIm.end(), {"r_H","alpha0_H","alpha1_H","b_H","M2_H","P_H"});
    if(hasHt)  parNamesIm.insert(parNamesIm.end(), {"r_Ht","alpha0_Ht","alpha1_Ht","b_Ht","M2_Ht","P_Ht"});
    if(hasE )  parNamesIm.insert(parNamesIm.end(), {"r_E","alpha0_E","alpha1_E","b_E","M2_E","P_E"});
    if(hasEt)  parNamesIm.insert(parNamesIm.end(), {"r_Et","alpha0_Et","alpha1_Et","b_Et","M2_Et","P_Et"});
}

void fcn(int&, double*, double &f, double *par, int){
    int ip=0;
    if(gStage==1){
        if(hasH){ r_H=par[ip++]; alpha0_H=par[ip++]; alpha1_H=par[ip++]; b_H=par[ip++]; M2_H=par[ip++]; P_H=par[ip++]; }
        if(hasHt){ r_Ht=par[ip++]; alpha0_Ht=par[ip++]; alpha1_Ht=par[ip++]; b_Ht=par[ip++]; M2_Ht=par[ip++]; P_Ht=par[ip++]; }
        if(hasE){ r_E=par[ip++]; alpha0_E=par[ip++]; alpha1_E=par[ip++]; b_E=par[ip++]; M2_E=par[ip++]; P_E=par[ip++]; }
        if(hasEt){ r_Et=par[ip++]; alpha0_Et=par[ip++]; alpha1_Et=par[ip++]; b_Et=par[ip++]; M2_Et=par[ip++]; P_Et=par[ip++]; }
        double chi2=0;
        ...
        renormReal = par[ip++];
        double chi2=0;
        for(auto &d: xsData){ BMK_DVCS dvcs(-1,0,0,d.Eb,d.xB,d.Q2,d.t,d.phi);
...
int main(int argc, char** argv) {
    ...
    std::cout<<"\n BSA bins="<<Nbins
             <<" (raw="<<bsaData.size()<<")\n"
             <<" Average χ² per bin-fit = "<<reducedAmpChi2<<"\n\n";
    ...
    {
      TMinuit minu(nim); minu.SetPrintLevel(1); minu.SetFCN(fcn);
      for(int i=0;i<nim;++i){ const auto &nm = parNamesIm[i];
        double init=0, lo=0, hi=1e6, step=0.01;
        if(nm=="r_H") init=r_H;
        else if(nm=="alpha0_H") init=alpha0_H, lo=0, hi=1;
    ...
    if(gStrategy==2){ gStage=2;
      ...
      m2.Migrad(); m2.Command("HESSE"); double chi2_re, edm2, errdef2; int nv2,nx2,ic2,ndf_re;
      m2.mnstat(chi2_re,edm2,errdef2,nv2,nx2,ic2);
      double reVal,reErr; m2.GetParameter(0,reVal,reErr); ndf_re = xsData.size()-1;
    ...
    std::cout<<"\n--- Fit Results ---\n";
    for(auto &n: outNames){ std::cout<<" "<<n<<" = "<<valMap[n]<<" ± "<<errMap[n]<<"\n"; }
    std::cout<<" χ²/ndf = "<<chi2_im<<"/"<<ndf_im<<" = "<<(chi2_im/ndf_im)<<"\n";
    std::cout<<" Average χ² per bin-fit = "<<reducedAmpChi2<<"\n";
...
void PlotBinFit(int ibin, const std::string &ts) {
    ...
    size_t start=0;
    for(size_t i=1;i<=bsaData.size();++i){ bool newbin=(i==bsaData.size()||bsaData[i].phi<bsaData[i-1].phi);
        if(newbin){ bins.emplace_back(bsaData.begin()+start,bsaData.begin()+i); start=i; }
    }
    if(ibin<0||ibin>=int(bins.size())) return;
    auto &dp=bins[ibin]; int n=dp.size(); if(n<2) return;
    system("mkdir -p output/plots/binned_fits"); gStyle->SetOptStat(0);
    TGraphErrors *gr=new TGraphErrors(n);
    for(int i=0;i<n;++i){ gr->SetPoint(i, dp[i].phi, dp[i].A); gr->SetPointError(i,0,dp[i].sigA); }
    gr->SetMarkerStyle(20);
    TF1 *f1=new TF1(Form("f_bin%d",ibin),
      "[0] + [1]*sin(x*TMath::Pi()/180.)/(1+[2]*cos(x*TMath::Pi()/180.))",0,360);
    f1->SetParameters(0, bin_A[ibin], 0);
    f1->SetParLimits(1,-1,1); f1->SetParLimits(2,-1,1);
    ...
    frame->GetXaxis()->SetTitle("#phi (deg)"); frame->GetYaxis()->SetTitle("A_{LU}");
    frame->Draw("AXIS"); gr->Draw("P same"); f1->Draw("L same");
