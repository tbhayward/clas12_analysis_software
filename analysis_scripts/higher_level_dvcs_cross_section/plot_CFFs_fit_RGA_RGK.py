...
def parse_fit_results(fname):
    ...
    for i,l in enumerate(L):
        if l.startswith("# parameters"):
            pnames=L[i].split()[2:]; break
    vals=errs=None; chi2=ndf=None
    for i,l in enumerate(L):
...
def idx(pn,name): return pn.index(name)
...
def make_Im_func(cff,pars,ren):
    ...
    def Im(xi,t):
        r    = pars.get("r",d["r"])
        a0,a1= pars.get("alpha0",d["alpha0"]), pars.get("alpha1",d["alpha1"])
        nval = pars.get("n",d["n"])
        bval = pars.get("b",d["b"])
        M2   = pars.get("M2",d["M2"])
        Pval = pars.get("P",d["P"])
...
def band(cff,xi_vals,t_vals,ren,cent,err,n=200):
    ...
    for i in range(n):
        ...
        A.append(f(xi_b,t_b))
    M=np.array(A)
    return np.median(M,0),np.percentile(M,2.5,0),np.percentile(M,97.5,0)
