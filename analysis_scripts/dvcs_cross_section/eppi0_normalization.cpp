...
static std::string tree_file_name(TTree* tree) {
    if (!tree) {
...
static double sum_weight_branch(TTree* tree) {
    if (!tree) {
        return 0.0;
    }
...
static void fill_eppi0_mc_hists_python_like(TTree* tree,
    ...
    const Long64_t N = tree->GetEntries();
    long long n_filled = 0;
    double sum_raw_weight = 0.0;
    double sum_scaled_weight = 0.0;
    double max_raw_weight = 0.0;

    for (Long64_t i = 0; i < N; ++i) {
        tree->GetEntry(i);
        const double raw_weight = b.weight;
        const double w = event_norm * raw_weight;
        fill_hist_set(hists, b, w);
        sum_raw_weight += raw_weight;
        sum_scaled_weight += w;
        if (n_filled == 0 || raw_weight > max_raw_weight) max_raw_weight = raw_weight;
        ++n_filled;
    }

    std::cout << "[eppi0_norm] python-like MC entries=" << (long long)N
              << " filled=" << n_filled
              << " raw_weight_sum=" << std::setprecision(12) << sum_raw_weight
              << " scaled_weight_sum=" << std::setprecision(12) << sum_scaled_weight
              << " max_raw_weight=" << std::setprecision(12) << max_raw_weight
              << " event_norm=" << std::setprecision(12) << event_norm
              << std::endl;
}
...
static void fill_eppi0_mc_hists_analysis(const ChannelConfig& cfg,
    ...
    for (Long64_t i = 0; i < N; ++i) {
        ...
        ++n_pass;
        const double raw_weight = b.weight;
        const double w = event_norm * raw_weight / current_factor;
        fill_hist_set(hists, b, w);
        sum_raw_weight += raw_weight;
        sum_scaled_weight += w;
        if (n_pass == 1 || raw_weight > max_raw_weight) max_raw_weight = raw_weight;
    }
    ...
    std::cout << "[eppi0_norm] analysis MC " << tags.display
              ...
              << " max_raw_weight=" << std::setprecision(12) << max_raw_weight
              << " event_norm=" << std::setprecision(12) << event_norm
              << " current_factor=" << std::setprecision(12) << current_factor
              << std::endl;
}
...
static PeriodNormalization run_period_normalization(const std::string& period,
    ...
    const double n_gen_from_tree = (double)generated_entries(gen_tree);
    const double n_gen = (n_gen_override > 0.0) ? n_gen_override : n_gen_from_tree;

    const Long64_t n_data_entries = data_tree ? data_tree->GetEntries() : 0;
    const Long64_t n_rec_entries = rec_tree ? rec_tree->GetEntries() : 0;
    const Long64_t n_gen_entries = gen_tree ? gen_tree->GetEntries() : 0;
    const double rec_raw_weight_sum_all_entries = sum_weight_branch(rec_tree);

    std::cout << "\n[eppi0_norm][INPUT CHECK] period=" << period << "\n"
              << "  data file = " << tree_file_name(data_tree) << "\n"
              << "  rec  file = " << tree_file_name(rec_tree) << "\n"
              << "  gen  file = " << tree_file_name(gen_tree) << "\n"
              << "  data entries = " << (long long)n_data_entries << "\n"
              << "  rec entries  = " << (long long)n_rec_entries << "\n"
              << "  gen entries  = " << (long long)n_gen_entries << "\n"
              << "  N_gen used = " << std::setprecision(12) << n_gen
              << ((n_gen_override > 0.0) ? " (override)" : " (tree entries)") << "\n"
              << "  rec raw weight sum over all entries = " << std::setprecision(12) << rec_raw_weight_sum_all_entries << "\n"
              << "  data charge raw nC = " << std::setprecision(12) << q_raw << "\n"
              << "  L_int pb^-1 = " << std::setprecision(12) << lint << "\n"
              << "  event_norm = L_int / N_gen = " << std::setprecision(12) << ((n_gen > 0.0) ? (lint / n_gen) : 0.0) << "\n"
              << std::endl;
    ...
    if (dbg.is_open()) {
        dbg << "period " << period << "\n";
        dbg << "data_file " << tree_file_name(data_tree) << "\n";
        dbg << "rec_file " << tree_file_name(rec_tree) << "\n";
        dbg << "gen_file " << tree_file_name(gen_tree) << "\n";
        dbg << "data_entries " << (long long)n_data_entries << "\n";
        dbg << "rec_entries " << (long long)n_rec_entries << "\n";
        dbg << "gen_entries " << (long long)n_gen_entries << "\n";
        dbg << "n_gen_from_tree " << std::setprecision(12) << n_gen_from_tree << "\n";
        dbg << "n_gen_override " << std::setprecision(12) << n_gen_override << "\n";
        dbg << "n_gen_source " << ((n_gen_override > 0.0) ? "override" : "tree_entries") << "\n";
        dbg << "rec_raw_weight_sum_all_entries " << std::setprecision(12) << rec_raw_weight_sum_all_entries << "\n";
        ...
        if (options.override_to_unity) {
            ...
            for (const std::string& period : WORK_PERIOD_ORDER) {
                ...
                PeriodNormalization n = run_period_normalization(period,
                                                                 ...
                                                                 options.output_dir,
                                                                 [&]() {
                                                                     auto it_override = options.n_gen_override_by_period.find(period);
                                                                     if (it_override == options.n_gen_override_by_period.end()) {
                                                                         return 0.0;
                                                                     }
