...
static int findRangeIndexRobust(const std::vector<std::pair<double,double>>& ranges,
    ...
    // pass 1: try exact edge match (with tolerance)
    for (int i = 0; i < (int)ranges.size(); ++i) {
        if (eq(ranges[i].first, want.first) && eq(ranges[i].second, want.second)) return i;
    }
    // pass 2: fall back to midpoint containment
    const double mid = 0.5 * (want.first + want.second);
    for (int i = 0; i < (int)ranges.size(); ++i) {
        const double lo = ranges[i].first, hi = ranges[i].second;
        if (mid >= lo - 10*atol && mid <= hi + 10*atol) return i;
    }
    return -1;
}
    ...
    const std::string& output_dir)
{
    ...
    // ---------- helper: per-subplot phi-averaged ratio ----------
    ...
    // ---------- SINGLE LARGE PLOT: specific bin ----------
    {
        ...
        const int ix = findRangeIndexRobust(xB_bins, xb_sel);
        const int iQ = findRangeIndexRobust(Q2_bins, Q2_sel);
        const int it = findRangeIndexRobust(t_bins,  t_sel);

        if (ix < 0 || iQ < 0 || it < 0) {
            std::cerr << "[compare][single-bin] Requested bin not found with robust matching.\n";
            std::cerr << "  Wanted xB=[" << xb_sel.first << "," << xb_sel.second << "], "
                      << "Q2=[" << Q2_sel.first << "," << Q2_sel.second << "], "
                      << "t=["  << t_sel.first  << "," << t_sel.second  << "]\n";
            std::cerr << "  Available xB bins:"; for (const auto& b : xB_bins) std::cerr << " [" << b.first << "," << b.second << "]";
            std::cerr << "\n  Available Q2 bins:"; for (const auto& b : Q2_bins) std::cerr << " [" << b.first << "," << b.second << "]";
    ...
    // ---------- SINGLE LARGE PLOT: 10.60 GeV helicity (+, -, unpolarized) in the SAME bin ----------
    {
        const std::pair<double,double> xb_sel = {0.09, 0.12};
        const std::pair<double,double> Q2_sel = {1.456, 1.912};
        const std::pair<double,double> t_sel  = {0.25,  0.40};
        ...
        if (ix < 0 || iQ < 0 || it < 0) {
            std::cerr << "[compare][single-bin 10.60 helicities] Requested bin not found.\n";
        } else {
            const std::string path_xsec_106 = (fs::path(output_dir) / "jsons" / "uncorrected_xsec_10.60.json").string();
            const json jx_106 = load_json(path_xsec_106);
            if (jx_106.empty() || !jx_106.contains("bins")) {
                std::cerr << "[compare][single-bin 10.60 helicities] Missing JSON " << path_xsec_106 << " or malformed.\n";
            } else {
                const std::string bkey = "(" + std::to_string(ix) + "," + std::to_string(iQ) + "," + std::to_string(it) + ")";
                if (!jx_106["bins"].contains(bkey)) {
                    std::cerr << "[compare][single-bin 10.60 helicities] Bin " << bkey << " not found in 10.60 JSON.\n";
                } else {
                    const json& jb = jx_106["bins"][bkey];

                    auto fetchNode = [&](const char* node)->const json* {
                        if (!jb.contains(node)) return nullptr;
                        const json& h = jb[node];
                        if (!h.contains("phi") || !h.contains("xsec") || !h.contains("xsec_err")) return nullptr;
                        if ((int)h["phi"].size()!=N_PHI_BINS || (int)h["xsec"].size()!=N_PHI_BINS || (int)h["xsec_err"].size()!=N_PHI_BINS) return nullptr;
                        return &h;
                    };

                    const json* hplus  = fetchNode("helicity_plus");
                    const json* hminus = fetchNode("helicity_minus");
                    const json* hunpol = fetchNode("unpolarized");

                    if (!hplus && !hminus && !hunpol) {
                        std::cerr << "[compare][single-bin 10.60 helicities] No helicity nodes present for bin " << bkey << ".\n";
                    } else {
                        const int W = 900, H = 560;
                        TCanvas* c = new TCanvas("c_single_bin_106_helicity","c_single_bin_106_helicity", W, H);
                        ...
                        // Same y-range styling as your single-bin plot (1e-1 to 1e1)
                        TH1* fr = gPad->DrawFrame(0.0, 1e-1, 360.0, 1e1);
                        fr->GetXaxis()->SetLabelSize(0.0001);
                        fr->GetXaxis()->SetTitle("#phi (deg)");
                        fr->GetYaxis()->SetTitle("d#sigma/d#phi (nb/GeV^{4})");
                        fr->GetXaxis()->CenterTitle();
                        fr->GetYaxis()->CenterTitle();
                        fr->GetXaxis()->SetNdivisions(505);
                        fr->GetXaxis()->SetTitleSize(0.060);
                        fr->GetYaxis()->SetTitleSize(0.060);
                        fr->GetYaxis()->SetLabelSize(0.048);
                        fr->GetXaxis()->SetTitleOffset(1.25);
                        fr->GetYaxis()->SetTitleOffset(1.42);

                        drawDegreeTicks(gPad->GetUxmin(), gPad->GetUymin(), gPad->GetUxmax(), 0.048);

                        auto drawSet = [&](const json* node, int mstyle, int color)->TGraphErrors* {
                            if (!node) return (TGraphErrors*)nullptr;
                            std::vector<double> x(N_PHI_BINS), y(N_PHI_BINS), e(N_PHI_BINS);
                            for (int i=0;i<N_PHI_BINS;++i){
                                x[i] = (*node)["phi"][i].get<double>();
                                y[i] = (*node)["xsec"][i].get<double>();
                                e[i] = std::max(1e-12, (*node)["xsec_err"][i].get<double>());
                            }
                            auto* gr = new TGraphErrors(N_PHI_BINS, x.data(), y.data(), nullptr, e.data());
                            gr->SetMarkerStyle(mstyle);
                            gr->SetMarkerSize(1.0);
                        ...
                        TGraphErrors* g_plus  = drawSet(hplus,  20, kBlue+1);
                        TGraphErrors* g_minus = drawSet(hminus, 25, kRed+1);
                        TGraphErrors* g_unpol = drawSet(hunpol, 22, kGreen+2);
                        ...
                        if (g_plus || g_minus || g_unpol) {
                            TLegend* leg = new TLegend(0.64, 0.18, 0.90, 0.36);
                            leg->SetBorderSize(1);
                            leg->SetLineColor(kBlack);
                            leg->SetFillColor(kWhite);
                            leg->SetFillStyle(1001);
                        ...
                        const std::string out_single_106 =
                            (fs::path(output_dir) / "plots_compare" /
                             ("uncorr_xsec_single_10p60_helicity_xB_"+std::to_string(ix)+
                              "_Q2_"+std::to_string(iQ)+"_t_"+std::to_string(it)+".png")).string();
                        c->SaveAs(out_single_106.c_str());
                        delete c;
                        std::cout << "[compare][single-bin 10.60 helicities] Wrote " << out_single_106 << "\n";
                    }
                }
            }
        }
    ...
    if (sumB > 0.0) integrated_ratio_pct = 100.0 * (sumA / sumB);
