...
This always produces the KM15/BH part of the study and exports the exact
native+10.6-GeV kinematics needed by PARTONS.

On the JLab farm, add --run-partons to obtain/reuse GK16 predictions through the
existing PARTONS driver and complete the transport-model uncertainty:

  python compare_world_dvcs_cross_sections.py --workers 8 --run-partons

If a completed PARTONS table already exists, it is reused automatically.  A
specific table can be supplied with --partons-table.

The script is intended to live beside extract_emff_from_dvcs_bh.py and
 evaluate_bh_model_selection.py in external_scripts/.
"""
...
def print_summary(dataset_summary: pd.DataFrame, model_scores: pd.DataFrame, pair_summary: pd.DataFrame, have_gk16: bool) -> None:
    ...
    if not have_gk16:
        print("\n[NOTE] GK16 was not merged in this run.  The script still produced the complete")
        print("       KM15/BH native-kinematics study and the PARTONS native+10.6 cache.")
        print("       Run with --run-partons on the farm to add GK16 transport uncertainties.")
    #endif
#enddef
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--partons-evaluator", default=str(here / "evaluate_bh_model_selection.py"))
    ...
    p.add_argument("--run-partons", action="store_true", help="Invoke/reuse the existing PARTONS evaluator to obtain GK16 native+target predictions")
    p.add_argument("--partons-table", default=None, help="Optional completed bh_model_selection_all_points.csv to merge instead of the default output location")
    p.add_argument("--partons-workers", type=int, default=4)
    p.add_argument("--partons-chunk-size", type=int, default=200)
    p.add_argument("--force-partons", action="store_true")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # ---------------------------------------------------------------------
    # 3. Export native+target kinematics and optionally run/reuse PARTONS.
    # ---------------------------------------------------------------------
    partons_input = outdir / "cache" / "partons_native_and_target_kinematics.csv"
    write_partons_kinematics(world, partons_input, float(args.target_ebeam))

    partons_outdir = outdir / "partons_predictions"
    default_partons_table = partons_outdir / "bh_model_selection_all_points.csv"

    if args.run_partons:
        run_partons_driver(args, partons_input, partons_outdir)
    #endif

    partons_table = Path(args.partons_table).expanduser().resolve() if args.partons_table else default_partons_table
    world, have_gk16 = merge_gk16_predictions(world, partons_table)
