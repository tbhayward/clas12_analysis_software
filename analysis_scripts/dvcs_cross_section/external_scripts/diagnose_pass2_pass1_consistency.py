...
Diagnostics included:
  ...
  3. Histograms:
       ...
       pass2_point_by_point_half_width_distribution_ge2_good_periods.png
...
def find_column_fuzzy(fieldnames: Sequence[str], candidates: Sequence[str]) -> Optional[str]:
    exact = find_column(fieldnames, candidates, required=False)
    ...
    for name in fieldnames:
        normalized_map[normalize_column_name(name)] = name
    #endfor
...
def detect_theta_columns(fieldnames: Sequence[str], label: str) -> Dict[str, Optional[str]]:
...
def row_optional_float(row: Dict[str, str], col: Optional[str]) -> float:
    if col is None:
        return float("nan")
    #endif

    return to_float(row.get(col, ""), default=float("nan"))
...
def load_pass2_points(
    ...
    with path.open("r", newline="") as handle:
        ...
        theta_cols = detect_theta_columns(fieldnames, "Pass-2")
        ...
        for row in reader:
            ...
            local_points.append(
                LocalScalePoint(
                    key=key,
                    phi=phi,
                    e_theta=row_optional_float(row, theta_cols["e_theta"]),
                    p_theta=row_optional_float(row, theta_cols["p_theta"]),
                    g_theta=row_optional_float(row, theta_cols["g_theta"]),
...
def write_local_scale_points_csv(output_dir: Path, points: Sequence[LocalScalePoint]) -> None:
    ...
    with path.open("w", newline="") as handle:
        handle.write(
            ...
            "xB_center,Q2_center,t_abs_center,phi,e_theta,p_theta,g_theta,"
            "pass2_xs,pass2_stat,"
        ...
        for p in points:
            ...
            handle.write(
                ...
                f"{format_float(p.e_theta)},"
                f"{format_float(p.p_theta)},"
                f"{format_float(p.g_theta)},"
...
def point_variable_value(point: LocalScalePoint, variable: str) -> float:
    if variable == "phi":
        return point.phi
    #endif

    if variable == "xB":
        return key_center_xb(point.key)
    #endif

    if variable == "Q2":
        return key_center_q2(point.key)
    #endif

    if variable == "t":
        return key_center_t(point.key)
    #endif

    if variable == "e_theta":
        return point.e_theta
    #endif

    if variable == "p_theta":
        return point.p_theta
    #endif

    if variable == "g_theta":
        return point.g_theta
    #endif
...
def write_kinematic_summary_by_bin(output_dir: Path, points: Sequence[LocalScalePoint], min_good_periods: int) -> None:
    ...
    with path.open("w", newline="") as handle:
        handle.write(
            ...
            "mean_s_obs,mean_s_stat,mean_e_theta,mean_p_theta,mean_g_theta\n"
        )
        ...
        for key in sorted(bins.keys(), key=lambda k: (k.xb_min, k.q2_min, k.t_min)):
            ...
            s_obs = [p.local.s_obs for p in selected if math.isfinite(p.local.s_obs)]
            s_stat = [p.local.s_stat_exp for p in selected if math.isfinite(p.local.s_stat_exp)]
            e_theta = [p.e_theta for p in selected if math.isfinite(p.e_theta)]
            p_theta = [p.p_theta for p in selected if math.isfinite(p.p_theta)]
            g_theta = [p.g_theta for p in selected if math.isfinite(p.g_theta)]
            ...
            handle.write(
                ...
                f"{format_float(mean(s_obs))},"
                f"{format_float(mean(s_stat))},"
                f"{format_float(mean(e_theta))},"
                f"{format_float(mean(p_theta))},"
                f"{format_float(mean(g_theta))}\n"
            )
        #endfor
    #endwith
...
def draw_kinematic_diagnostics(
    ...
    draw_s_comb_vs_variable(output_dir, points, "e_theta", r"$\theta_e$ (deg)", "pass2_local_s_comb_vs_e_theta.png", min_good_periods)
    draw_s_comb_vs_variable(output_dir, points, "p_theta", r"$\theta_p$ (deg)", "pass2_local_s_comb_vs_p_theta.png", min_good_periods)
    draw_s_comb_vs_variable(output_dir, points, "g_theta", r"$\theta_\gamma$ (deg)", "pass2_local_s_comb_vs_g_theta.png", min_good_periods)
...
def main() -> int:
    ...
    s_comb_ge2 = values_for(local_points, "s_comb", 2, None)
    ...
    run_summary = {
        ...
        "mean_s_comb_ge2": mean(s_comb_ge2),
