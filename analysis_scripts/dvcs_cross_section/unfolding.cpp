...
//   - Period names are NOT transformed; they must match group keys in the master JSON exactly.
    ...
    while (true) {
        ...
        while (true) {
            ...
            auto extract_block = [&](const std::string& src, const char* label)->std::string {
                ...
                size_t br2 = src.find('{', p);
                if (br2 == std::string::npos) return std::string();
                int dd=0; size_t ii=br2; for (; ii<src.size(); ++ii){ if(src[ii]=='{') dd++; else if(src[ii]=='}'){ dd--; if(!dd){ ++ii; break; } } }
                return src.substr(br2, ii-br2);
            };
            ...
            // obj contains: "helicity":{ "+1":{value,err}, "-1":{value,err} }, "total":{...}
            size_t hpos = obj.find("\"helicity\"");
            if (hpos == std::string::npos) { bkey = jj; continue; }
            size_t hbr = obj.find('{', hpos); if (hbr == std::string::npos) { bkey = jj; continue; }
...
void compute_and_plot_unfolding(
    const std::vector<std::string>& periods,           // exact group keys to process
    const std::vector<Binning>& binning_scheme,
    ...
    const std::string& out_root_dir)
{
    ...
    // Load corrected master (value, err per helicity)
    GroupHelMap groups;
    ...
    for (const auto& group : periods) {
        ...
        // acceptance JSON for this group
        const fs::path acc_path = fs::path(out_root_dir)/"jsons"/("acceptance_"+group+".json");
        AccMap3 accCells;
        if (!load_acceptance_json(acc_path.string(), accCells)) {
            std::cerr<<"[unf][WARN] Missing/invalid acceptance for "<<group<<" — skipping.\n";
            continue;
        }
