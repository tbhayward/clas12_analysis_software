// bin_centering_cross_check.cpp
// -----------------------------------------------------------------------------
// Cross-check of bin-centering corrections (Hayward pass-2 vs Lee pass-1)
// using *only* CSVs.
...
//
//   Lee:     "Fbin"                (single value, no error)
//   Hayward: "Fbin, 10.6 GeV" (three-tuple: value, stat_err, syst_err)
//
...
//   bin_center_counts_xB_<ix>.png
//   bin_center_ratio_xB_<ix>.png
//
// -----------------------------------------------------------------------------
...
static inline void info_bc(const std::string& s) {
    std::cout << "[bcent] " << s << std::endl;
}

static inline void warn_bc(const std::string& s) {
    std::cout << "[bcent][warn] " << s << std::endl;
}

static inline void fatal_bc(const std::string& s) {
    std::cerr << "[bcent][FATAL] " << s << std::endl;
    throw std::runtime_error(s);
}

static inline std::string slower_bc(std::string s) {
    for (auto& c : s) {
...
static inline std::string trim_bc(const std::string& s) {
    size_t i0 = 0;
...
static std::vector<std::string> split_csv_line_bc(const std::string& line) {
...
build_header_index_bc(const std::vector<std::string>& header) {
...
static const std::string& get_col_ref_bc(const std::vector<std::string>& row,
...
static double ToDouble_bc(const std::string& s) {
...
static int ToInt_bc(const std::string& s) {
    if (s.empty()) return 0;
    char* endp = nullptr;
...
static void require_columns_bc(const std::unordered_map<std::string,int>& idx,
...
static BinVolValue parse_binvol_triplet(const std::string& s) {
    BinVolValue bv;
    ...
    if (parts.size() >= 2) {
        bv.value    = ToDouble_bc(parts[0]);
        bv.stat_err = ToDouble_bc(parts[1]);
        bv.valid    = true;
    }
...
static int find_col_alias_bc(const std::vector<std::string>& header,
                             const std::vector<std::string>& names) {
    for (size_t i = 0; i < header.size(); ++i) {
        std::string h = slower_bc(trim_bc(header[i]));
        for (const auto& raw_name : names) {
            std::string n = slower_bc(trim_bc(raw_name));
            if (!n.empty() && h == n) {
                return (int)i;
            }
        }
...
static LeeCsvCols_bc detect_lee_columns_bc(const std::vector<std::string>& header) {
    if (header.empty()) {
        fatal_bc("Empty header row in Lee CSV");
    }
    ...
    cols.c_bin = find_col_alias_bc(header, { "bin index", "bin", "idx" });
    if (cols.c_bin < 0) {
        std::string h0 = trim_bc(header[0]);
        if (h0.empty()) {
            cols.c_bin = 0;
        } else {
            std::ostringstream oss;
    ...
    // 2) xB, Q2, phi
    cols.c_xb_min  = find_col_alias_bc(header, { "xBmin", "xbmin", "xB_min", "xb_min" });
    cols.c_xb_max  = find_col_alias_bc(header, { "xBmax", "xbmax", "xB_max", "xb_max" });
    cols.c_q2_min  = find_col_alias_bc(header, { "Q2min", "q2min", "Q2_min", "q2_min" });
    cols.c_q2_max  = find_col_alias_bc(header, { "Q2max", "q2max", "Q2_max", "q2_max" });
    cols.c_phi_avg = find_col_alias_bc(header, { "phiavg", "phi_avg", "phi_average" });

    // 3) |t| min and max
    cols.c_t_min = find_col_alias_bc(header, { "t_abs_min", "tmin", "t_min" });
    cols.c_t_max = find_col_alias_bc(header, { "t_abs_max", "tmax", "t_max" });
...
struct BinRow_bc {
    ...
    double lee_fbin    = 0.0;  // Lee Fbin (single value, no error)
    double my_binvol   = 0.0;  // Hayward bin_volume, 10.6 GeV (value)
    double my_binvol_err = 0.0;  // Hayward bin_volume stat error
};

static AxisSets_bc build_axes_from_rows_bc(const std::vector<BinRow_bc>& rows) {
...
static inline int find_index_bc(const std::pair<double,double>& r,
...
struct PerPanel_bc {
    // key = (ix, iQ, it)
    std::map<std::tuple<int,int,int>, PanelData_bc> lee;
    std::map<std::tuple<int,int,int>, PanelData_bc> hayward;
};

static PerPanel_bc map_to_panels_bc(const std::vector<BinRow_bc>& rows,
                                    const AxisSets_bc& ax) {
    PerPanel_bc pp;

    for (const auto& r : rows) {
        const auto xb = std::make_pair(r.xBmin, r.xBmax);
        const int ix  = find_index_bc(xb, ax.xB);
        if (ix < 0) continue;

        const auto& Q2s = ax.Q2_by_ix.at(ix);
        const auto& Ts  = ax.t_by_ix.at(ix);

        const int iQ = find_index_bc({r.Q2min, r.Q2max}, Q2s);
        const int it = find_index_bc({r.tmin,   r.tmax},  Ts);
        ...
        if (r.lee_fbin > 0.0) {
            ...
            pp.lee[key].val.push_back(r.lee_fbin);
    ...
    auto sort_panel = [](PanelData_bc& pd) {
...
static inline void degreeTicks_bc(double xmin, double ymin, double xmax, double labelSize) {
...
static void draw_degree_ticks_here_bc(double labelSize) {
    degreeTicks_bc(gPad->GetUxmin(), gPad->GetUymin(), gPad->GetUxmax(), labelSize);
}

// Markers + error bars
static TGraphErrors* graph_pe1_bc(const std::vector<double>& X,
...
static std::string safe_canvas_name_bc(const std::string& out_png) {
    return fs::path(out_png).filename().string();
}

static void draw_one_canvas_bc(const std::string& title,
                               const std::vector<std::pair<double,double>>& Q2s,
                               const std::vector<std::pair<double,double>>& Ts,
                               const std::function<void(int,int,PanelData_bc&,PanelData_bc&)>& fetchBoth,
    ...
    const std::string cname = safe_canvas_name_bc(out_png);
    ...
    for (int r = 0; r < nrows; ++r) {
        for (int ccol = 0; ccol < ncols; ++ccol) {
            ...
            PanelData_bc hayward, lee;
            fetchBoth(ccol, r, hayward, lee);

            // Fixed y-axis range. Bin-centering factors are often O(1),
            // but bin_volume may have a different scale. If needed,
            ...
            draw_degree_ticks_here_bc(0.050);
            ...
            if (draw_ratio_only) {
                ...
                // Plot both series
                graph_pe1_bc(hayward.phi, hayward.val, hayward.err, 20, black);  // Hayward with errors
                graph_pe1_bc(lee.phi, lee.val, lee.err, 24, orange);             // Lee (errors are 0)
            }
        }
    }
...
static std::vector<BinRow_bc> load_lee_rows_bc(const std::string& lee_csv_path,
                                               std::unordered_map<int,size_t>& bin_to_index) {
    std::ifstream fin(lee_csv_path);
    if (!fin.is_open()) {
        fatal_bc("Cannot open Lee CSV: " + lee_csv_path);
    }

    std::string header_line;
    if (!std::getline(fin, header_line)) {
        fatal_bc("Lee CSV appears empty: " + lee_csv_path);
    }
    std::vector<std::string> header = split_csv_line_bc(header_line);

    LeeCsvCols_bc cols_lee = detect_lee_columns_bc(header);
    auto H = build_header_index_bc(header);

    const std::vector<std::string> required = {
        "valid bin",
        "Fbin"
    };
    require_columns_bc(H, required, "Lee CSV");

    std::vector<BinRow_bc> rows;
    rows.reserve(3000);
    ...
    while (std::getline(fin, line)) {
        ...
        std::vector<std::string> cols = split_csv_line_bc(line);

        const std::string& valid_s = get_col_ref_bc(cols, H, "valid bin");
        int valid = ToInt_bc(valid_s);
        if (valid != 1) continue;

        if ((int)cols.size() <= cols_lee.c_phi_avg) {
            fatal_bc("Row in Lee CSV has fewer columns than expected based on header detection.");
        }

        BinRow_bc r;
        r.bin_index   = ToInt_bc(cols[cols_lee.c_bin]);
        r.xBmin       = ToDouble_bc(cols[cols_lee.c_xb_min]);
        r.xBmax       = ToDouble_bc(cols[cols_lee.c_xb_max]);
        r.Q2min       = ToDouble_bc(cols[cols_lee.c_q2_min]);
        r.Q2max       = ToDouble_bc(cols[cols_lee.c_q2_max]);
        r.tmin        = ToDouble_bc(cols[cols_lee.c_t_min]);
        r.tmax        = ToDouble_bc(cols[cols_lee.c_t_max]);
        r.phiavg      = ToDouble_bc(cols[cols_lee.c_phi_avg]);

        double lee_raw = ToDouble_bc(get_col_ref_bc(cols, H, "Fbin"));
        r.lee_fbin     = lee_raw;
        r.my_binvol    = 0.0;
        r.my_binvol_err = 0.0;

        if (bin_to_index.find(r.bin_index) != bin_to_index.end()) {
            fatal_bc("Duplicate bin index in Lee CSV: " + std::to_string(r.bin_index));
        }
    ...
    info_bc("Lee CSV rows read: " + std::to_string(input_rows));
    info_bc("Lee valid rows kept (valid bin == 1): " + std::to_string(kept_rows));
    return rows;
}

static void fill_hayward_binvol_bc(const std::string& hayward_csv_path,
                                   const std::unordered_map<int,size_t>& bin_to_index,
                                   std::vector<BinRow_bc>& rows) {
    std::ifstream fin(hayward_csv_path);
    if (!fin.is_open()) {
        fatal_bc("Cannot open Hayward CSV: " + hayward_csv_path);
    }

    std::string header_line;
    if (!std::getline(fin, header_line)) {
        fatal_bc("Hayward CSV appears empty: " + hayward_csv_path);
    }
    std::vector<std::string> header = split_csv_line_bc(header_line);
    auto H = build_header_index_bc(header);
    ...
    require_columns_bc(H, required, "Hayward CSV");
    ...
    while (std::getline(fin, line)) {
        ...
        int bin_index = ToInt_bc(get_col_ref_bc(cols, H, "bin index"));
        auto it = bin_to_index.find(bin_index);
        if (it == bin_to_index.end()) {
            continue;
        }

        BinVolValue bv = parse_binvol_triplet(get_col_ref_bc(cols, H, "Fbin, 10.6 GeV"));

        BinRow_bc& r = rows[it->second];
        if (bv.valid) {
    ...
    info_bc("Hayward CSV rows read: " + std::to_string(input_rows));
    info_bc("Hayward valid rows matched to Lee bins: " + std::to_string(matched));
}
...
void plot_bin_centering_cross_checks(const std::string& lee_csv_path,
    ...
    // 1) Load Lee CSV
    std::unordered_map<int,size_t> bin_to_index;
    auto rows = load_lee_rows_bc(lee_csv_path, bin_to_index);

    // 2) Load Hayward CSV
    fill_hayward_binvol_bc(hayward_csv_path, bin_to_index, rows);

    // 3) Build axis sets and per-panel maps
    AxisSets_bc ax = build_axes_from_rows_bc(rows);
    PerPanel_bc pp = map_to_panels_bc(rows, ax);

    info_bc("Axis xB bins: " + std::to_string(ax.xB.size()));

    auto make_fetchBoth = [&](int ix) {
        return [&, ix](int iQcol, int irow,
                       PanelData_bc& hayward,
                       PanelData_bc& lee) {
    ...
    for (int ix = 0; ix < (int)ax.xB.size(); ++ix) {
        ...
        const std::string title_counts =
            Form("Bin-centering corrections: 10.6 GeV   x_{B} #in [%.3g, %.3g]", xb_lo, xb_hi);
        const std::string title_ratio  =
            Form("Bin-centering ratio (Hayward/Lee): 10.6 GeV   x_{B} #in [%.3g, %.3g]",
                 xb_lo, xb_hi);

        auto fetchBoth = make_fetchBoth(ix);

        const std::string f_counts =
            (fs::path(output_base_dir) / Form("bin_center_counts_xB_%d.png", ix)).string();
        const std::string f_ratio  =
            (fs::path(output_base_dir) / Form("bin_center_ratio_xB_%d.png",  ix)).string();

        draw_one_canvas_bc(title_counts, Q2s, Ts, fetchBoth, f_counts, /*draw_ratio_only=*/false);
        draw_one_canvas_bc(title_ratio,  Q2s, Ts, fetchBoth, f_ratio,   /*draw_ratio_only=*/true);

        info_bc("Saved: " + f_counts);
        info_bc("Saved: " + f_ratio);
    }
}