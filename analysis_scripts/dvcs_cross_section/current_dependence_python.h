 ...
 *
 * Reconstructed DATA and MC samples are selected with the canonical Python-
 * optimized nominal production exclusivity cuts from combined_cuts_json.
 * Supported logical variables are Delta_phi, theta, theta_gamma_gamma,
 * pTmiss, Emiss2, Mx2 and Mx2_2. For direct ep->eppi0 trees, the logical
 * theta_gamma_gamma cut is evaluated using the physical theta_pi0_pi0 branch.
 * Mx2 is mandatory because it is the forced first cut in the optimizer;
 * remaining variables are applied only when present in a topology block.
 *
