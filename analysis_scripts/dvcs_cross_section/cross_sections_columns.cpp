...
enum class UnpolarizedChargeMode {
    Column2,
    Columns3Plus4,
    ScaledColumns3To5
};

static const char* unpolarized_charge_mode_name(UnpolarizedChargeMode mode) {
    switch (mode) {
        case UnpolarizedChargeMode::Column2:
            return "column 2";
        case UnpolarizedChargeMode::Columns3Plus4:
            return "columns 3+4";
        case UnpolarizedChargeMode::ScaledColumns3To5:
            return "scaled columns 3+4+5";
    }
...
static Triple load_rga_lumi_file(const std::string &path,
                                 UnpolarizedChargeMode unpolarized_charge_mode,
    ...
    while (std::getline(ifs, line)) {
        ...
        const size_t required_fields =
            (unpolarized_charge_mode == UnpolarizedChargeMode::ScaledColumns3To5) ? 5U : 4U;

        if (fields.size() < required_fields) {
            std::cerr << "[cross_sections] WARNING: lumi file " << path
                      << " has a line with fewer than " << required_fields
                      << " columns, skipping: " << s << "\n";
            continue;
        }
        ...
        const double col5      = (fields.size() >= 5U) ? std::atof(fields[4].c_str()) : 0.0;
    ...
    if (unpolarized_charge_mode == UnpolarizedChargeMode::Column2) {
        final_total = sum_total_col;
    } else if (unpolarized_charge_mode == UnpolarizedChargeMode::Columns3Plus4) {
        final_total = sum_pos + sum_neg;
    } else if (unpolarized_charge_mode == UnpolarizedChargeMode::ScaledColumns3To5) {
    ...
    std::cout << "[cross_sections] Loaded lumi from " << path
              ...
              << " col5=" << sum_col5
              << "  [unpolarized source: "
              << unpolarized_charge_mode_name(unpolarized_charge_mode);

    if (unpolarized_charge_mode == UnpolarizedChargeMode::ScaledColumns3To5) {
...
LumiMap build_lumi_map(const LumiBuildOptions &options) {
    ...
    if (options.use_columns_3_to_5_charge_sum_scaled_for_unpolarized) {
        std::cout << "scaled columns-3-to-5 mode: unpolarized luminosity = "
    ...
    // This controls only Triple.value, i.e. the unpolarized luminosity.
    ...
    UnpolarizedChargeMode fa18_unpol_mode = UnpolarizedChargeMode::Column2;
    UnpolarizedChargeMode sp18_unpol_mode = UnpolarizedChargeMode::Column2;
    UnpolarizedChargeMode sp19_unpol_mode = UnpolarizedChargeMode::Column2;

    if (options.use_columns_3_to_5_charge_sum_scaled_for_unpolarized) {
        fa18_unpol_mode = UnpolarizedChargeMode::ScaledColumns3To5;
        sp18_unpol_mode = UnpolarizedChargeMode::ScaledColumns3To5;
        sp19_unpol_mode = UnpolarizedChargeMode::ScaledColumns3To5;
    } else if (!use_col2_for_all) {
        fa18_unpol_mode = UnpolarizedChargeMode::Columns3Plus4;
        sp18_unpol_mode = UnpolarizedChargeMode::Column2;
        sp19_unpol_mode = UnpolarizedChargeMode::Columns3Plus4;
    }
