...
static void write_csv_or_throw(const std::string& path, const CsvTable& table) {
    ...
    if (!fout.is_open()) throw std::runtime_error("Could not open temporary CSV for writing: " + tmp_path);

    fout << join_csv_row(table.header) << "\n";
    for (const auto& row : table.rows) {
        if (row.size() != table.header.size()) throw std::runtime_error("Internal CSV row/header size mismatch.");
        fout << join_csv_row(row) << "\n";
    }
    fout.close();
    if (!fout) throw std::runtime_error("Failed while writing temporary CSV: " + tmp_path);
...
static void require_columns(const CsvTable& table,
    ...
    for (const auto& col : required) {
        if (table.index.find(col) == table.index.end()) missing.push_back(col);
    }
    if (!missing.empty()) {
        std::ostringstream msg;
...
static TupleValue get_input_value(const CsvTable& table,
    ...
    for (const auto& col : input.columns) values.push_back(get_tuple(table, row, col));
    TupleValue out;
...
static std::vector<CombinationCase> combination_cases() {
    return {
        ...
        },
        {
            ...
            false,
            {
                input_single("Fa18", cross_section_column("Fa18", "unpol")),
                input_single("Sp18", cross_section_column("Sp18", "unpol"))
            }
        },
        {
            "Inb vs Out unpol",
            ...
            false,
            {
                input_group("Inb", {cross_section_column("Fa18 Inb", "unpol"), cross_section_column("Sp18 Inb", "unpol")}),
                input_group("Out", {cross_section_column("Fa18 Out", "unpol"), cross_section_column("Sp18 Out", "unpol")})
            }
        }
    };
}
...
static void validate_schema(const CsvTable& table,
    ...
    for (const auto& c : cases) {
        ...
        if (c.fill_output) required.push_back(c.output_column);
        for (const auto& input : c.inputs) {
            for (const auto& col : input.columns) required.push_back(col);
        }
    }
    for (const auto& col : sp19_proxy_output_columns()) required.push_back(col);
...
static std::string csv_escape_field(const std::string& s) {
    const bool need_quotes = s.find(',') != std::string::npos ||
                             s.find('"') != std::string::npos ||
                             s.find('\n') != std::string::npos ||
                             s.find('\r') != std::string::npos;
...
static CombinationResult evaluate_case(const CsvTable& table,
    ...
    result.reference_column = c.reference_column;
    result.output_column = c.output_column;

    std::map<std::string, PeriodRatioAccumulator> acc_by_period;
    for (const auto& input : c.inputs) acc_by_period[input.period] = PeriodRatioAccumulator();
    ...
    for (const auto& row : table.rows) {
        ...
        for (const auto& input : c.inputs) {
            const TupleValue v = get_input_value(table, row, input);
            if (!v.ok || !std::isfinite(v.value) || v.stat <= 0.0) continue;
        ...
        if (n_valid_in_bin >= 2) ++result.valid_bins;
    }
    ...
    for (const auto& input : c.inputs) {
        ...
        if (acc.sum_w <= 0.0 || acc.n <= 0) continue;
        ...
        if (!std::isfinite(p.mean_ratio) || !std::isfinite(p.mean_ratio_stat) || p.mean_ratio_stat <= 0.0) continue;
    ...
    if (n_period > 0) {
        ...
        result.s_comb = std::sqrt(std::max(0.0,
            result.s_obs * result.s_obs - result.s_stat_exp * result.s_stat_exp));
    }
...
static void fill_output_column(CsvTable& table,
                               ...
                               double value) {
    const auto it = table.index.find(column);
    if (it == table.index.end()) throw std::runtime_error("Missing output column: " + column);
    const std::string value_string = format_double(value);
    const size_t icol = (size_t)it->second;
    for (auto& row : table.rows) row[icol] = value_string;
}

static void write_summary_csv(const std::string& path,
                              const std::vector<CombinationResult>& results) {
    std::ofstream fout(path);
    if (!fout.is_open()) throw std::runtime_error("Could not open summary CSV: " + path);

    fout << "case,reference column,output column,valid bins,ratio points,s_obs_period,s_stat_period,s_comb,s_comb percent,period,period ratio points,period mean ratio,period mean ratio stat\n";
    for (const auto& r : results) {
        if (r.period_summaries.empty()) {
            fout << csv_escape_field(r.label) << ","
                 << csv_escape_field(r.reference_column) << ","
                 << csv_escape_field(r.output_column) << ","
                 << r.valid_bins << "," << r.ratio_points << ","
    ...
    fout.close();
    if (!fout) throw std::runtime_error("Failed while writing summary CSV: " + path);
}
...
static void print_summary_table(const std::vector<CombinationResult>& results) {
    ...
    for (const auto& r : results) {
        std::cout << std::left << std::setw(30) << r.label
                  << std::right << std::setw(10) << r.valid_bins
...
bool combination_systematics(const std::string& csv_path,
    ...
    try {
        ...
        std::cout << "[combination-systematics] CSV rows loaded: " << table.rows.size() << "\n";
        std::cout << "[combination-systematics] Output directory: " << out_dir << "\n";

        std::vector<CombinationResult> results;
        results.reserve(cases.size() + 3);

        double ten6_unpol_s_comb = std::numeric_limits<double>::quiet_NaN();
        for (const auto& c : cases) {
            CombinationResult result = evaluate_case(table, c);
            if (c.fill_output) fill_output_column(table, result.output_column, result.s_comb);
            if (c.label == "10.6 GeV unpol") ten6_unpol_s_comb = result.s_comb;
        ...
        std::cout << "[combination-systematics] Wrote summary CSV: " << summary_csv << "\n";
        std::cout << "[combination-systematics] Updated CSV: " << csv_path << "\n";
        return true;
