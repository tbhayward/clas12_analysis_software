...
static std::vector<std::string> combined_members_for(const std::string &L) {
    if (L == "Fa18") {
        return {"Fa18 Inb", "Fa18 Out"};
    }
    if (L == "Sp18") {
        return {"Sp18 Inb", "Sp18 Out"};
    }
    if (L == "10.6 GeV") {
...
bool compute_cross_sections(const std::string &csv_main,
    ...
    try {
        for (const auto &p : base_periods_with_acceptance) {
            std::string col = "acceptance, " + p;
            int idx = find_col(header, col);
            acc_col_idx[p] = idx;
        }
    } catch (const std::exception &e) {
    ...
    auto period_has_acceptance = [&](const std::string &period,
        ...
        if (it == acc_col_idx.end()) {
            return false;
        }
        const std::string &cell = fields[it->second];
        Triple a = parse_tuple3(cell);
        return (a.value > 0.0);
    };
    ...
    for (size_t row = 1; row < lines.size(); ++row) {
        ...
        for (const auto &L : labels) {
            ...
            Triple LumiRow = lumi_for_label_row(L, fields);
