...
void plot_dvcs_data_mc_comparison(const std::string& output_dir, 
    ...
    // For MC-generated data, detector information may not be available
    ...
    auto process_events = [&](TTreeReader& reader,
                              ...
                              TTreeReaderValue<int>* detector1,
                              TTreeReaderValue<int>* detector2,
        ...
        while (reader.Next()) {
            ...
            if (detector1 && detector2) {
                if (*detector1 == 1 && *detector2 == 1) {
                    topology = "(FD,FD)";
                } else if (*detector1 == 2 && *detector2 == 1) {
                    topology = "(CD,FD)";
                } else if (*detector1 == 2 && *detector2 == 0) {
                    topology = "(CD,FT)";
                } else {
    ...
    // Process data events
    process_events(data_reader, phi_data, xB_data, Q2_data, t1_data, open_angle_ep2_data, theta_neutral_neutral_data, Emiss2_data, Mx2_data, Mx2_1_data, Mx2_2_data, pTmiss_data, xF_data, &detector1_data, &detector2_data, "data");

    // Process MC-reconstructed events
    process_events(mc_rec_reader, phi_mc_rec, xB_mc_rec, Q2_mc_rec, t1_mc_rec, open_angle_ep2_mc_rec, theta_neutral_neutral_mc_rec, Emiss2_mc_rec, Mx2_mc_rec, Mx2_1_mc_rec, Mx2_2_mc_rec, pTmiss_mc_rec, xF_mc_rec, &detector1_mc_rec, &detector2_mc_rec, "mc_rec");

    // Process MC-generated events (without detector info, default topology)
    process_events(mc_gen_reader, phi_mc_gen, xB_mc_gen, Q2_mc_gen, t1_mc_gen, open_angle_ep2_mc_gen, theta_neutral_neutral_mc_gen, Emiss2_mc_gen, Mx2_mc_gen, Mx2_1_mc_gen, Mx2_2_mc_gen, pTmiss_mc_gen, xF_mc_gen, nullptr, nullptr, "mc_gen");
    ...
    for (const auto& group : bin_groups) {
        ...
        legend->AddEntry(graph_mc_gen, "Generated MC", "lep");
