...
void plot_dvcs_data_mc_comparison(const std::string& output_dir, int xB_bin, const std::vector<BinBoundary>& bin_boundaries, TTreeReader& data_reader, TTreeReader& mc_gen_reader, TTreeReader& mc_rec_reader) {
    ...
    for (int idx = 0; idx < n_Q2t_bins; ++idx) {
        ...
        h_data_histograms[idx]->GetXaxis()->SetLabelSize(0.07);
        h_data_histograms[idx]->GetYaxis()->SetLabelSize(0.07);

        h_mc_gen_histograms[idx]->GetXaxis()->SetLabelSize(0.07);
        h_mc_gen_histograms[idx]->GetYaxis()->SetLabelSize(0.07);

        h_mc_rec_histograms[idx]->GetXaxis()->SetLabelSize(0.07);
        h_mc_rec_histograms[idx]->GetYaxis()->SetLabelSize(0.07);
