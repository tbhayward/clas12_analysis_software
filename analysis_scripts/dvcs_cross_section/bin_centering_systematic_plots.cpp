...
void style(TCanvas&c){c.SetLeftMargin(.125);c.SetRightMargin(.035);c.SetBottomMargin(.13);c.SetTopMargin(.15);c.SetTicks(1,1);}
}
bool make_bin_centering_systematic_analysis_note_plots(const std::string& sys_csv,const std::string& corr_csv,const std::string& out){
 try{
  ...
  {TCanvas cv("c_fbin_sys_dist","",1050,740);style(cv);TH1D h("h_fbin_sys_dist","",45,0,std::max(3.0,std::ceil(qtile(rel,.99)+.5)));for(double x:rel)h.Fill(x);if(h.Integral()>0)h.Scale(1/h.Integral());h.SetLineColor(kMagenta+2);h.SetLineWidth(3);h.SetFillColorAlpha(kMagenta+1,.18);h.GetXaxis()->SetTitle("Relative bin-centering systematic uncertainty (%)");h.GetYaxis()->SetTitle("Fraction of analysis bins");h.GetXaxis()->SetTitleSize(.045);h.GetYaxis()->SetTitleSize(.045);h.GetXaxis()->SetLabelSize(.039);h.GetYaxis()->SetLabelSize(.039);h.GetYaxis()->SetTitleOffset(1.25);h.SetMaximum(1.2*h.GetMaximum());h.Draw("HIST");TLine m(qtile(rel,.5),0,qtile(rel,.5),h.GetMaximum());m.SetLineStyle(7);m.SetLineWidth(2);m.SetLineColor(kMagenta+3);m.Draw();TLatex t;t.SetNDC();t.SetTextFont(42);t.SetTextSize(.042);t.DrawLatex(.125,.94,"Bin-centering model uncertainty");TLegend l(.66,.78,.93,.88);l.SetBorderSize(0);l.SetFillStyle(0);std::ostringstream ss;ss<<"Median = "<<std::fixed<<std::setprecision(2)<<qtile(rel,.5)<<"%";l.AddEntry(&m,ss.str().c_str(),"l");l.Draw();cv.SaveAs((fs::path(out)/"bin_centering_systematic_distribution.png").string().c_str());}
  {std::vector<std::pair<double,double>> edges;for(auto&r:s.r){double a=num(r[sx0]),b=num(r[sx1]);if(std::isfinite(a)&&std::isfinite(b)&&std::find(edges.begin(),edges.end(),std::make_pair(a,b))==edges.end())edges.push_back({a,b});}std::sort(edges.begin(),edges.end());TCanvas cv("c_fbin_sys_xb","",1050,740);style(cv);cv.SetGridy();TH1F fr("h_fbin_sys_xb","",100,.05,.60);fr.SetMinimum(0);fr.SetMaximum(std::max(2.5,1.15*qtile(rel,.95)));fr.GetXaxis()->SetTitle("x_{B}");fr.GetYaxis()->SetTitle("Relative bin-centering systematic uncertainty (%)");fr.GetXaxis()->SetTitleSize(.045);fr.GetYaxis()->SetTitleSize(.045);fr.GetYaxis()->SetTitleOffset(1.25);fr.Draw();TGraphAsymmErrors g;g.SetMarkerStyle(20);g.SetMarkerColor(kMagenta+2);g.SetLineColor(kMagenta+2);int n=0;for(auto&e:edges){std::vector<double>v;for(auto&r:s.r)if(fabs(num(r[sx0])-e.first)<1e-8&&fabs(num(r[sx1])-e.second)<1e-8){double xs=num(r[sval]),u=num(r[sf]);if(std::isfinite(xs)&&fabs(xs)>0&&std::isfinite(u))v.push_back(100*u/fabs(xs));}if(v.empty())continue;double md=qtile(v,.5),lo=qtile(v,.16),hi=qtile(v,.84);g.SetPoint(n,.5*(e.first+e.second),md);g.SetPointError(n,0,0,md-lo,hi-md);++n;}g.Draw("PE SAME");TLatex t;t.SetNDC();t.SetTextFont(42);t.SetTextSize(.042);t.DrawLatex(.125,.94,"Kinematic dependence of the bin-centering model uncertainty");TLatex nte;nte.SetNDC();nte.SetTextFont(42);nte.SetTextSize(.026);nte.DrawLatex(.135,.865,"Points show the median; bars show the central 68% interval within each x_{B} bin");cv.SaveAs((fs::path(out)/"bin_centering_systematic_vs_xB.png").string().c_str());}
  ...
  if(n>0){TCanvas cv("c_fbin_sys_phi","",1050,740);style(cv);cv.SetGridy();TH1F fr("h_fbin_sys_phi","",100,0,360);fr.SetMinimum(std::min(.94,ymin-.015));fr.SetMaximum(std::max(1.04,ymax+.015));fr.GetXaxis()->SetTitle("#phi (deg)");fr.GetYaxis()->SetTitle("F_{bin}");fr.GetXaxis()->SetTitleSize(.045);fr.GetYaxis()->SetTitleSize(.045);fr.GetYaxis()->SetTitleOffset(1.15);fr.Draw();g.Draw("PE SAME");TLine one(0,1,360,1);one.SetLineStyle(2);one.SetLineColor(kGray+2);one.Draw();TLatex t;t.SetNDC();t.SetTextFont(42);t.SetTextSize(.042);t.DrawLatex(.125,.94,"Representative bin-centering correction and model uncertainty");TLatex a;a.SetNDC();a.SetTextFont(42);a.SetTextSize(.025);a.DrawLatex(.135,.865,"0.204 < x_{B} < 0.268, 1.912 < Q^{2} < 2.510 GeV^{2}, 0.250 < |t| < 0.400 GeV^{2}");cv.SaveAs((fs::path(out)/"bin_centering_systematic_phi_example.png").string().c_str());}
  return true;
