...
static inline bool approx_equal(double a, double b, double abs=1e-6, double rel=1e-4){
...
struct BothSides {
    std::map<Key3, Series> inb;
    std::map<Key3, Series> out;
};
...
// ---------- gather Lee series (xL,yL at Lee’s own phi) ----------
struct LeeSeries { std::vector<double> phi; std::vector<double> val; };
static std::map<Key3, LeeSeries> collect_lee(const std::vector<LeeRow>& rows, const AxisSets& ax){
    ...
    for (const auto& r : rows){
        ...
        if (std::isfinite(r.contam_inb) && r.contam_inb>0.0){
            auto& s = out[{ix,iQ,it}]; s.phi.push_back(phi); s.val.push_back(r.contam_inb);
        }
        if (std::isfinite(r.contam_out) && r.contam_out>0.0){
            // we collect a single map; caller will select inb/out CSV beforehand if needed.
            // If your CSV has both sides, you would keep two maps; here rows already carry side.
        }
    }
    // sort
    for (auto& kv : out){
...
void plot_pi0_contam_cross_checks(const std::vector<LeeRow>& rows,
    ...
    // 1) Build Lee axes and gather Lee series
    AxisSets leeAx = build_axes_from_rows(rows);
    // Split Lee rows by side if you have separate CSVs; if rows already encode side-specific
    // values, you would keep two maps. For simplicity we show a single map here:
    auto lee_all = collect_lee(rows, leeAx);
    ...
    for (int ix=0; ix<(int)leeAx.xB.size(); ++ix){
        ...
        auto fetch_inb = [&](int iQ, int it,
            ...
            // Lee
            {
                auto itL = lee_all.find(k);
                if (itL != lee_all.end()){ xL = itL->second.phi; yL = itL->second.val; }
