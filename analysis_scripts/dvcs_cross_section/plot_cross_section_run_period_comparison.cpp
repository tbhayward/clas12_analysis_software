...
// Function to read the CSV file and retrieve the fall data
CrossSectionData read_fall_csv(const std::string &filename) {
...
// Function to read the CSV file and retrieve the spring data
CrossSectionData read_spring_csv(const std::string &filename) {
    CrossSectionData data;
    std::ifstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Error: Cannot open file " << filename << std::endl;
