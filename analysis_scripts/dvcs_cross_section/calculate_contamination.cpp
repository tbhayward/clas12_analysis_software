...
void calculate_contamination(const std::string& base_output_dir,
    ...
    // Initialize a map to hold all contamination data for each topology
    std::map<std::string, std::vector<ContaminationData>> topology_contamination_data;
    ...
    if (relevant_bins.empty()) {
        ...
        return topology_contamination_data; // Return empty map
    }
    ...
    // Map to hold combined contamination data
    std::vector<UnfoldingData> combined_contamination_data_vec;
