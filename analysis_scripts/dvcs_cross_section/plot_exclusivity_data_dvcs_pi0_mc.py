...
Outputs:
  ...
  * main-suite-compatible combined_cuts JSON files for 90%, 95% and 98%
...
class GlobalCutsConfig:
    analysis_tag: str
    t1_abs_max: float
    open_angle_min_deg: float
    enable_pTmiss_cut: bool
    pTmiss_max: float
    enable_dvcsgen_ycol_cut: bool
    enable_auxiliary_fiducial_cuts: bool
    enable_sp18_out_sector_quality_cuts: bool
    sp18_out_sector_quality_cuts_active: bool
    auxiliary_require_distinct_fd_sectors: bool
    auxiliary_e_theta_min_deg: float
    auxiliary_e_theta_max_deg: float
    auxiliary_fd_proton_theta_min_deg: float
    auxiliary_fd_proton_theta_max_deg: float
    auxiliary_fd_photon_theta_min_deg: float
    auxiliary_fd_photon_theta_max_deg: float
    auxiliary_cd_proton_theta_min_deg: float
    auxiliary_cd_proton_theta_max_deg: float
    auxiliary_ft_photon_p_min_GeV: float
    enable_topology_filter: bool
    required_detector1: int
    required_detector2: int
    enable_electron_fd_sector_filter: bool
    electron_fd_sector: int
    enable_proton_fd_sector_filter: bool
    proton_fd_sector: int
    enable_proton_cd_sector_filter: bool
    proton_cd_sector: int
    enable_photon_fd_sector_filter: bool
    photon_fd_sector: int
    excluded_runs: Tuple[int, ...]


ACTIVE_GLOBAL_CUTS: Optional[GlobalCutsConfig] = None
...
class FitResult:
    ...
    pi0_shift: float = math.nan
    pi0_shift_err: float = math.nan
    pi0_sigma_add: float = math.nan
    pi0_sigma_add_err: float = math.nan
    pi0_sigma_right: float = math.nan
    pi0_sigma_right_err: float = math.nan
    ...
    transformed_pi0_shape: Optional[np.ndarray] = None
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--global-cuts-json",
        required=True,
        help="Global-cut configuration JSON written by the C++ analysis.",
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.98,
        help="Loose iterative signal containment (default: 0.98).",
    )
    parser.add_argument(
        "--tight-containment",
        type=float,
...
def load_global_cuts_config(path: str) -> GlobalCutsConfig:
    config_path = Path(path)
    if not config_path.is_file():
        raise FileNotFoundError(f"Global-cut JSON does not exist: {config_path}")
    with config_path.open("r", encoding="utf-8") as stream:
        raw = json.load(stream)
    required = {
        "analysis_tag", "t1_abs_max", "open_angle_min_deg",
        "enable_pTmiss_cut", "pTmiss_max", "enable_dvcsgen_ycol_cut",
        "enable_auxiliary_fiducial_cuts",
        "enable_sp18_out_sector_quality_cuts",
        "sp18_out_sector_quality_cuts_active",
        "auxiliary_require_distinct_fd_sectors",
        "auxiliary_e_theta_min_deg", "auxiliary_e_theta_max_deg",
        "auxiliary_fd_proton_theta_min_deg", "auxiliary_fd_proton_theta_max_deg",
        "auxiliary_fd_photon_theta_min_deg", "auxiliary_fd_photon_theta_max_deg",
        "auxiliary_cd_proton_theta_min_deg", "auxiliary_cd_proton_theta_max_deg",
        "auxiliary_ft_photon_p_min_GeV", "enable_topology_filter",
        "required_detector1", "required_detector2",
        "enable_electron_fd_sector_filter", "electron_fd_sector",
        "enable_proton_fd_sector_filter", "proton_fd_sector",
        "enable_proton_cd_sector_filter", "proton_cd_sector",
        "enable_photon_fd_sector_filter", "photon_fd_sector", "excluded_runs",
    }
    missing = sorted(required.difference(raw))
    if missing:
        raise KeyError(
            f"Global-cut JSON {config_path} is missing required keys: "
            + ", ".join(missing)
        )
    cfg = GlobalCutsConfig(
        analysis_tag=str(raw["analysis_tag"]),
        t1_abs_max=float(raw["t1_abs_max"]),
        open_angle_min_deg=float(raw["open_angle_min_deg"]),
        enable_pTmiss_cut=bool(raw["enable_pTmiss_cut"]),
        pTmiss_max=float(raw["pTmiss_max"]),
        enable_dvcsgen_ycol_cut=bool(raw["enable_dvcsgen_ycol_cut"]),
        enable_auxiliary_fiducial_cuts=bool(raw["enable_auxiliary_fiducial_cuts"]),
        enable_sp18_out_sector_quality_cuts=bool(raw["enable_sp18_out_sector_quality_cuts"]),
        sp18_out_sector_quality_cuts_active=bool(raw["sp18_out_sector_quality_cuts_active"]),
        auxiliary_require_distinct_fd_sectors=bool(raw["auxiliary_require_distinct_fd_sectors"]),
        auxiliary_e_theta_min_deg=float(raw["auxiliary_e_theta_min_deg"]),
        auxiliary_e_theta_max_deg=float(raw["auxiliary_e_theta_max_deg"]),
        auxiliary_fd_proton_theta_min_deg=float(raw["auxiliary_fd_proton_theta_min_deg"]),
        auxiliary_fd_proton_theta_max_deg=float(raw["auxiliary_fd_proton_theta_max_deg"]),
        auxiliary_fd_photon_theta_min_deg=float(raw["auxiliary_fd_photon_theta_min_deg"]),
        auxiliary_fd_photon_theta_max_deg=float(raw["auxiliary_fd_photon_theta_max_deg"]),
        auxiliary_cd_proton_theta_min_deg=float(raw["auxiliary_cd_proton_theta_min_deg"]),
        auxiliary_cd_proton_theta_max_deg=float(raw["auxiliary_cd_proton_theta_max_deg"]),
        auxiliary_ft_photon_p_min_GeV=float(raw["auxiliary_ft_photon_p_min_GeV"]),
        enable_topology_filter=bool(raw["enable_topology_filter"]),
        required_detector1=int(raw["required_detector1"]),
        required_detector2=int(raw["required_detector2"]),
        enable_electron_fd_sector_filter=bool(raw["enable_electron_fd_sector_filter"]),
        electron_fd_sector=int(raw["electron_fd_sector"]),
        enable_proton_fd_sector_filter=bool(raw["enable_proton_fd_sector_filter"]),
        proton_fd_sector=int(raw["proton_fd_sector"]),
        enable_proton_cd_sector_filter=bool(raw["enable_proton_cd_sector_filter"]),
        proton_cd_sector=int(raw["proton_cd_sector"]),
        enable_photon_fd_sector_filter=bool(raw["enable_photon_fd_sector_filter"]),
        photon_fd_sector=int(raw["photon_fd_sector"]),
        excluded_runs=tuple(int(value) for value in raw["excluded_runs"]),
    )
    if cfg.enable_dvcsgen_ycol_cut:
        raise RuntimeError(
            "FATAL: enable_dvcsgen_ycol_cut is true in global_cuts_config.json. "
            "The Python exclusivity fitter intentionally does not implement this deprecated cut."
        )
    return cfg


def cd_sector_from_phi(phi_rad: np.ndarray) -> np.ndarray:
...
def global_cut_required_branches() -> List[str]:
    cfg = ACTIVE_GLOBAL_CUTS
    if cfg is None:
        raise RuntimeError("Global-cut configuration has not been loaded.")
    branches = [
        "detector1", "detector2", "t1", "open_angle_ep2", "pTmiss",
        "e_phi", "p1_phi", "p2_phi", "runnum",
    ]
    if cfg.enable_auxiliary_fiducial_cuts:
        branches.extend(["e_theta", "p1_theta", "p2_p", "p2_theta"])
    return branches


def global_cut_mask_uproot(
    ...
    opening = np.asarray(arrays["open_angle_ep2"], dtype=np.float64)
    ptmiss = np.asarray(arrays["pTmiss"], dtype=np.float64)
    mask = (
        np.isfinite(t1) & np.isfinite(opening) & np.isfinite(ptmiss)
        & ((-t1) < cfg.t1_abs_max)
        & (opening > cfg.open_angle_min_deg)
    )
    if cfg.enable_pTmiss_cut:
        mask &= ptmiss <= cfg.pTmiss_max
    if cfg.excluded_runs:
        runnum = np.asarray(arrays["runnum"], dtype=np.int64)
        mask &= ~np.isin(runnum, np.asarray(cfg.excluded_runs, dtype=np.int64))
    if cfg.enable_topology_filter:
        mask &= (
            (detector1 == cfg.required_detector1)
            & (detector2 == cfg.required_detector2)
        )

    e_phi = np.asarray(arrays["e_phi"], dtype=np.float64)
    p1_phi = np.asarray(arrays["p1_phi"], dtype=np.float64)
    p2_phi = np.asarray(arrays["p2_phi"], dtype=np.float64)
    e_sector = fd_sector_from_phi(e_phi)
    p_fd_sector = fd_sector_from_phi(p1_phi)
    p_cd_sector = cd_sector_from_phi(p1_phi)
    g_sector = fd_sector_from_phi(p2_phi)

    if cfg.sp18_out_sector_quality_cuts_active and period_key == "sp18_out":
        mask &= e_sector >= 1
        mask &= e_sector != 3
        mask &= ~((e_sector == 5) & (detector2 == 1))
        mask &= ~((e_sector == 5) & (detector1 == 1))
    if cfg.enable_electron_fd_sector_filter:
        mask &= e_sector == cfg.electron_fd_sector
    if cfg.enable_proton_fd_sector_filter:
        mask &= (detector1 == 1) & (p_fd_sector == cfg.proton_fd_sector)
    if cfg.enable_proton_cd_sector_filter:
        mask &= (detector1 == 2) & (p_cd_sector == cfg.proton_cd_sector)
    if cfg.enable_photon_fd_sector_filter:
        mask &= (detector2 == 1) & (g_sector == cfg.photon_fd_sector)

    if cfg.enable_auxiliary_fiducial_cuts:
        e_theta_deg = np.degrees(np.asarray(arrays["e_theta"], dtype=np.float64))
        p1_theta_deg = np.degrees(np.asarray(arrays["p1_theta"], dtype=np.float64))
        p2_theta_deg = np.degrees(np.asarray(arrays["p2_theta"], dtype=np.float64))
        p2_p = np.asarray(arrays["p2_p"], dtype=np.float64)
        mask &= (
            np.isfinite(e_theta_deg) & np.isfinite(p1_theta_deg)
            & np.isfinite(p2_theta_deg) & np.isfinite(p2_p)
        )
        mask &= e_theta_deg > cfg.auxiliary_e_theta_min_deg
        mask &= (detector2 != 1) | (e_theta_deg < cfg.auxiliary_e_theta_max_deg)
        mask &= (
            ((detector1 == 1)
             & (p1_theta_deg > cfg.auxiliary_fd_proton_theta_min_deg)
             & (p1_theta_deg < cfg.auxiliary_fd_proton_theta_max_deg))
            | ((detector1 == 2)
               & (p1_theta_deg > cfg.auxiliary_cd_proton_theta_min_deg)
               & (p1_theta_deg < cfg.auxiliary_cd_proton_theta_max_deg))
        )
        photon_ok = (
            ((detector2 == 1)
             & ((detector1 != 1) | (p2_theta_deg > cfg.auxiliary_fd_photon_theta_min_deg))
             & (p2_theta_deg < cfg.auxiliary_fd_photon_theta_max_deg))
            | ((detector2 == 0) & (p2_p > cfg.auxiliary_ft_photon_p_min_GeV))
        )
        mask &= photon_ok
...
def event_passes_global_cuts(event: object, period_key: str) -> bool:
    cfg = ACTIVE_GLOBAL_CUTS
    if cfg is None:
        raise RuntimeError("Global-cut configuration has not been loaded.")
    ...
    ptmiss = float(getattr(event, "pTmiss"))
    if not (math.isfinite(t1) and math.isfinite(opening) and math.isfinite(ptmiss)):
        return False
    if (-t1) >= cfg.t1_abs_max or opening <= cfg.open_angle_min_deg:
        return False
    if cfg.enable_pTmiss_cut and ptmiss > cfg.pTmiss_max:
        return False
    if cfg.excluded_runs and int(getattr(event, "runnum")) in cfg.excluded_runs:
        return False
    if cfg.enable_topology_filter and (
        detector1 != cfg.required_detector1 or detector2 != cfg.required_detector2
    ):
        return False
    e_phi = float(getattr(event, "e_phi"))
    p1_phi = float(getattr(event, "p1_phi"))
    p2_phi = float(getattr(event, "p2_phi"))
    e_sector = int(fd_sector_from_phi(np.asarray([e_phi]))[0])
    p_fd_sector = int(fd_sector_from_phi(np.asarray([p1_phi]))[0])
    p_cd_sector = int(cd_sector_from_phi(np.asarray([p1_phi]))[0])
    g_sector = int(fd_sector_from_phi(np.asarray([p2_phi]))[0])
    if cfg.sp18_out_sector_quality_cuts_active and period_key == "sp18_out":
        if e_sector < 1 or e_sector == 3:
            return False
        if e_sector == 5 and (detector2 == 1 or detector1 == 1):
            return False
    if cfg.enable_electron_fd_sector_filter and e_sector != cfg.electron_fd_sector:
        return False
    if cfg.enable_proton_fd_sector_filter and (
        detector1 != 1 or p_fd_sector != cfg.proton_fd_sector
    ):
        return False
    if cfg.enable_proton_cd_sector_filter and (
        detector1 != 2 or p_cd_sector != cfg.proton_cd_sector
    ):
        return False
    if cfg.enable_photon_fd_sector_filter and (
        detector2 != 1 or g_sector != cfg.photon_fd_sector
    ):
        return False
    if cfg.enable_auxiliary_fiducial_cuts:
        e_theta_deg = math.degrees(float(getattr(event, "e_theta")))
        p1_theta_deg = math.degrees(float(getattr(event, "p1_theta")))
        p2_theta_deg = math.degrees(float(getattr(event, "p2_theta")))
        p2_p = float(getattr(event, "p2_p"))
        if not all(map(math.isfinite, (e_theta_deg, p1_theta_deg, p2_theta_deg, p2_p))):
            return False
        if not (e_theta_deg > cfg.auxiliary_e_theta_min_deg):
            return False
        if detector2 == 1 and not (e_theta_deg < cfg.auxiliary_e_theta_max_deg):
            return False
        if detector1 == 1:
            if not (cfg.auxiliary_fd_proton_theta_min_deg < p1_theta_deg < cfg.auxiliary_fd_proton_theta_max_deg):
                return False
        elif detector1 == 2:
            if not (cfg.auxiliary_cd_proton_theta_min_deg < p1_theta_deg < cfg.auxiliary_cd_proton_theta_max_deg):
                return False
        else:
            return False
        if detector2 == 1:
            if detector1 == 1 and not (p2_theta_deg > cfg.auxiliary_fd_photon_theta_min_deg):
                return False
            if not (p2_theta_deg < cfg.auxiliary_fd_photon_theta_max_deg):
                return False
        elif detector2 == 0:
            if not (p2_p > cfg.auxiliary_ft_photon_p_min_GeV):
                return False
        else:
            return False
        if cfg.auxiliary_require_distinct_fd_sectors:
            sectors = [e_sector]
            if detector1 == 1:
                sectors.append(p_fd_sector)
            if detector2 == 1:
                sectors.append(g_sector)
...
def fill_histograms_uproot(
    ...
    expressions = sorted(set(global_cut_required_branches() + list(resolved.values())))
    ...
    for arrays in uproot.iterate(
        ...
        base_mask = global_cut_mask_uproot(arrays, period_key, detector1, detector2)
...
def fill_histograms_pyroot(
    ...
    try:
        for event in tree:
            ...
            if not event_passes_global_cuts(event, period_key):
                continue
            # endif
...
def fill_histograms_for_file(
    ...
    if backend == "uproot":
        return fill_histograms_uproot(path, period_key, topologies, step_size, apply_mx2_1_cut, variables)
    # endif

    return fill_histograms_pyroot(path, period_key, topologies, step_size, apply_mx2_1_cut, variables)
...
def fit_shared_two_templates(
    ...
    def single_nuisance_start(variable: VariableConfig) -> np.ndarray:
        center = calibrated_shift_center(variable)
        if is_log_morph_variable(variable):
        ...
        if is_asymmetric_additive_variable(variable):
            if variable.branch == "xF":
                return np.asarray(
                    [center, 1.5 * bin_width, 1.5 * bin_width],
                    dtype=np.float64,
                )
            # endif
            return np.asarray(
    ...
    def single_nuisance_size(variable: VariableConfig) -> int:
        return 3 if is_asymmetric_additive_variable(variable) else 2

    def nuisance_bounds(variable: VariableConfig) -> List[Tuple[float, float]]:
        one_template = single_nuisance_bounds(variable)
        return one_template + one_template

    def nuisance_start(variable: VariableConfig) -> np.ndarray:
        one_template = single_nuisance_start(variable)
        return np.concatenate((one_template, one_template))

    def split_nuisance(
        variable: VariableConfig,
        nuisance: np.ndarray,
    ) -> Tuple[np.ndarray, np.ndarray]:
        size = single_nuisance_size(variable)
        values = np.asarray(nuisance, dtype=np.float64)
        if values.size != 2 * size:
            raise ValueError(
                f"Expected {2 * size} nuisance parameters for "
                f"{variable.branch}, received {values.size}."
            )
        # endif
        return values[:size], values[size:]

    def single_nuisance_penalty(
        variable: VariableConfig,
        nuisance: np.ndarray,
    ) -> float:
    ...
    def nuisance_penalty(
        ...
        dvcs_nuisance, pi0_nuisance = split_nuisance(variable, nuisance)
        return (
            single_nuisance_penalty(variable, dvcs_nuisance)
            + single_nuisance_penalty(variable, pi0_nuisance)
        )
    ...
    def build_variable_model(
        ...
        dvcs_sigma_right = (
            float(dvcs_nuisance[2])
            if is_asymmetric_additive_variable(variable)
            else None
        )
        pi0_sigma_right = (
            float(pi0_nuisance[2])
            if is_asymmetric_additive_variable(variable)
            else None
        )

        transformed_dvcs = transform_template_shape(
            info["dvcs_shape"],
            variable,
            float(dvcs_nuisance[0]),
            float(dvcs_nuisance[1]),
            dvcs_sigma_right,
        )
        transformed_pi0 = transform_template_shape(
            info["pi0_shape"],
            variable,
            float(pi0_nuisance[0]),
            float(pi0_nuisance[1]),
            pi0_sigma_right,
        )
        ...
        dvcs_shape_component = np.clip(
            (1.0 - fraction) * transformed_dvcs,
            0.0,
            None,
        )
        pi0_shape_component = np.clip(
            fraction * transformed_pi0,
            0.0,
            None,
        )
    ...
    if fixed_fraction is None:
        for initial_fraction in (0.10, 0.30, 0.60, 0.85):
            ...
            for iteration in range(15):
                # Resolution step: fit the DVCS and pi0 nuisances in the union support.
    ...
    for variable in VARIABLES:
        ...
        shift = float(dvcs_nuisance[0])
        sigma_add = float(dvcs_nuisance[1])
        sigma_right = (
            float(dvcs_nuisance[2])
            if is_asymmetric_additive_variable(variable)
            else math.nan
        )
        pi0_shift = float(pi0_nuisance[0])
        pi0_sigma_add = float(pi0_nuisance[1])
        pi0_sigma_right = (
            float(pi0_nuisance[2])
            if is_asymmetric_additive_variable(variable)
            else math.nan
        ...
        results[variable.branch] = FitResult(
            ...
            pi0_shift=pi0_shift,
            pi0_sigma_add=pi0_sigma_add,
            pi0_sigma_right=pi0_sigma_right,
            ...
            transformed_dvcs_shape=np.asarray(
                transformed_dvcs,
                dtype=np.float64,
            ).copy(),
            transformed_pi0_shape=np.asarray(
                transformed_pi0,
                dtype=np.float64,
            ).copy(),
...
def draw_fit_canvas(
    ...
    for axis, variable in zip(flat_axes, VARIABLES):
        ...
        if result.success:
            ...
            if is_asymmetric_additive_variable(variable):
                dvcs_morph_text = (
                    rf"DVCS: $\Delta={result.shift:.4g}$, "
                    rf"$\sigma_L={result.sigma_add:.4g}$, "
                    rf"$\sigma_R={result.sigma_right:.4g}$"
                )
                pi0_morph_text = (
                    rf"$\pi^0$: $\Delta={result.pi0_shift:.4g}$, "
                    rf"$\sigma_L={result.pi0_sigma_add:.4g}$, "
                    rf"$\sigma_R={result.pi0_sigma_right:.4g}$"
                )
            else:
                dvcs_morph_text = (
                    rf"DVCS: $\Delta={result.shift:.4g}$, "
                    rf"$\sigma={result.sigma_add:.4g}$"
                )
                pi0_morph_text = (
                    rf"$\pi^0$: $\Delta={result.pi0_shift:.4g}$, "
                    rf"$\sigma={result.pi0_sigma_add:.4g}$"
                )
            # endif
            quality = (
...
def process_period(
    ...
    if fraction_population_arrays is None:
        fraction_population_arrays = {
            "data": load_selected_arrays_for_file(
                period.data_file, period.key, topologies, step_size, DVCS_SHAPE_VARIABLES
            ),
            "signal": load_selected_arrays_for_file(
                period.dvcs_mc_file, period.key, topologies, step_size, DVCS_SHAPE_VARIABLES
            ),
    ...
    for topology in topologies:
        ...
        for variable in VARIABLES:
            ...
            rows.append({
                ...
                "pi0_shift": result.pi0_shift,
                "pi0_shift_err": result.pi0_shift_err,
                "pi0_sigma": result.pi0_sigma_add,
                "pi0_sigma_err": result.pi0_sigma_add_err,
                "pi0_sigma_right": result.pi0_sigma_right,
                "pi0_sigma_right_err": result.pi0_sigma_right_err,
                "pi0_shift_bins": (
                    result.pi0_shift / bin_width
                    if result.success and additive
                    else math.nan
                ),
                "pi0_sigma_bins": (
                    result.pi0_sigma_add / bin_width
                    if result.success and additive
                    else math.nan
                ),
                "sigma_bins": result.sigma_add / bin_width if result.success and additive else math.nan,
...
def load_selected_arrays_for_file(
    ...
    if io_backend() == "uproot":
        return load_selected_arrays_uproot(path, period_key, topologies, step_size, variables)
    # endif
    return load_selected_arrays_pyroot(path, period_key, topologies, step_size, variables)
...
def develop_iterative_cuts_for_period(
    ...
    dvcs_arrays = (
        ...
        if preloaded_dvcs_arrays is not None
            ...
            "data": load_selected_arrays_for_file(
                period.data_file, period.key, topologies, step_size, VARIABLES
            ),
            "signal": load_selected_arrays_for_file(
                period.dvcs_mc_file, period.key, topologies, step_size, VARIABLES
            ),
            "background": load_selected_arrays_for_file(
                period.pi0_as_dvcs_mc_file, period.key, topologies, step_size, VARIABLES
            ),
        }
    )
    pi0_arrays = (
...
def main() -> int:
    global ACTIVE_GLOBAL_CUTS
    args = parse_args()
    ACTIVE_GLOBAL_CUTS = load_global_cuts_config(args.global_cuts_json)
    log(
        f"Loaded global cuts '{ACTIVE_GLOBAL_CUTS.analysis_tag}' from "
        f"{args.global_cuts_json}"
    )
    ...
    # endif
    if not (
        0.50 <= args.tight_containment
        < args.nominal_containment
        < args.loose_containment
        <= 1.0
    ):
        raise ValueError(
    ...
    log(
        ...
        "Both templates are independently morphed: an asymmetric additive core morph is used for theta; ordinary additive core morphs are used for symmetric variables, lower-edge log morphs for pTmiss/theta_gamma_gamma/theta_pi0_pi0 and a bounded logit morph for z2 and an upper-edge log morph for xF2. "
        "DVCS and pi0 template shift priors are centered on the corresponding direct-pi0 core calibrations. "
    ...
    if not args.skip_iterative_cuts:
        ...
        loose_path = json_dir / "combined_cuts_98.json"
