...
// If you have the header with the 3-sigma exclusivity cut, include it.
// We assume it declares: bool passes3sigma(const std::string& period_key,
//                                          const std::string& topo_name,
//                                          double t1, double open_angle_ep2, double pTmiss);
// and (optionally) an enum class Topology { FD_FD, CD_FD, CD_FT } in the global namespace.
...
// If the header did not define Topology, define a local one to compile.
// We will still accept all three topologies for averaging.
#if !HAVE_EXCL_3SIGMA
...
static bool is_10p6_label(const std::string& lbl) {
    return (lbl == "Fa18 Inb" || lbl == "Fa18 Out" || lbl == "Sp18 Inb" || lbl == "Sp18 Out");
}
...
// Always apply BOTH: global cuts AND (if available) 3-sigma exclusivity.
// We keep the call in the global namespace (::passes3sigma) to match your header.
static bool passes_all_cuts(const std::string& period_key,
                            Topology topo,
                            const BranchBinder& b) {
    if (!apply_global_cuts(b.t1, b.open_angle_ep2, b.pTmiss)) return false;
#if HAVE_EXCL_3SIGMA
    const std::string topo_name = topo_to_csv_title(topo);
    if (!::passes3sigma(period_key, topo_name, b.t1, b.open_angle_ep2, b.pTmiss)) return false;
#endif
    return true;
}
...
static bool phi_in_interval(double phi, double a, double b, double period) {
    if (a <= b) return (phi >= a && phi < b);
    return (phi >= a || phi < b); // wrap
}
...
struct Accum {
    ...
    double sum_Q2 = 0.0;
    double sum_t  = 0.0;
    double sum_c  = 0.0; // cos(phi)
    double sum_s  = 0.0; // sin(phi)
    long long n = 0;
    ...
    void mean(double& m_xB, double& m_Q2, double& m_t, double& m_phi, bool phi_out_deg) const {
        ...
        m_t  = sum_t  / (double)n;
        double ang = std::atan2(sum_s, sum_c);
...
struct PeriodResults {
    std::vector<Accum> per_row; // size = number of CSV rows
};
...
static PeriodResults process_period(const std::string& period_key,
    ...
    for (Long64_t i = 0; i < nent; ++i) {
        ...
        // Event must pass BOTH global and 3-sigma cuts for at least one topology.
        bool passes_any_topology = false;
        for (Topology t : all_topos()) {
            if (!event_matches_topology(b.detector1, b.detector2, t)) continue;
            if (passes_all_cuts(period_key, t, b)) { passes_any_topology = true; break; }
        }
        if (!passes_any_topology) continue;

        // Try candidate phi rows in this (xB,Q2,t) cell
        for (int row_idx : it_rows->second) {
            ...
            break; // bins are disjoint in phi, so one match per event
        }
    }
...
static bool parse_double(const std::vector<std::string>& row,
                         const std::unordered_map<std::string,int>& H,
                         const char* name, double& out) {
    auto it = H.find(name);
    if (it == H.end()) return false;
    return as_double(row[it->second], out);
}
...
static GridBuild build_grid_from_csv(const std::vector<std::vector<std::string>>& table,
    ...
    // Detect phi unit (deg if any phimax > 7)
    for (size_t r = 1; r < table.size(); ++r) {
        const auto& row = table[r];
        auto it = H.find("phimax"); if (it == H.end() || it->second >= (int)row.size()) continue;
        double v = 0.0; if (as_double(row[it->second], v) && v > 7.0) { G.csv_phi_is_deg = true; break; }
        G.csv_phi_is_deg = false;
    }

    // Gather valid rows and unique edges
    std::set<std::pair<double,double>> x_ranges, Q_ranges, t_ranges;

    for (size_t r = 1; r < table.size(); ++r) {
        const auto& row = table[r];
        int valid = 0;
        auto itV = H.find("valid bin");
        if (itV != H.end() && itV->second < (int)row.size()) valid = as_int_def(row[itV->second], 0);
        if (valid != 1) continue;

        RowDef rd;
        rd.row_index = (int)r;

        if (!parse_double(row, H, "xBmin", rd.xBmin)) continue;
        if (!parse_double(row, H, "xBmax", rd.xBmax)) continue;
        if (!parse_double(row, H, "Q2min", rd.Q2min)) continue;
        if (!parse_double(row, H, "Q2max", rd.Q2max)) continue;
        if (!parse_double(row, H, "t_abs_min", rd.tmin)) continue;
        if (!parse_double(row, H, "t_abs_max", rd.tmax)) continue;
        if (!parse_double(row, H, "phimin", rd.phimin)) continue;
        if (!parse_double(row, H, "phimax", rd.phimax)) continue;

        // Normalize phi into [0,period)
        const double phi_period = G.csv_phi_is_deg ? 360.0 : (2.0 * M_PI);
        auto wrap = [&](double a)->double { double w = std::fmod(a, phi_period); if (w < 0.0) w += phi_period; return w; };
...
static bool locate_avg_columns(const std::vector<std::string>& header, AvgColIdx& idx) {
    ...
    bool ok = true;
    for (int g = 0; g < 8; ++g) {
        { std::ostringstream n; n << "xBavg, "    << groups[g]; idx.xBavg[g]   = find_header_index(header, n.str()); ok = ok && (idx.xBavg[g]   >= 0); }
        { std::ostringstream n; n << "Q2avg, "    << groups[g]; idx.Q2avg[g]   = find_header_index(header, n.str()); ok = ok && (idx.Q2avg[g]   >= 0); }
        { std::ostringstream n; n << "t_abs_avg, "<< groups[g]; idx.tabsavg[g] = find_header_index(header, n.str()); ok = ok && (idx.tabsavg[g] >= 0); }
        { std::ostringstream n; n << "phiavg, "   << groups[g]; idx.phiavg[g]  = find_header_index(header, n.str()); ok = ok && (idx.phiavg[g]  >= 0); }
    }
    return ok;
}
...
bool update_bin_means_csv(const std::string& csv_path,
    ...
    // 3) Prepare period work items (at most 5)
    struct WorkItem { std::string key; TTree* tree; };
    std::vector<WorkItem> work;
    work.reserve(dataTrees.size());
    for (const auto& kv : dataTrees) if (kv.second) work.push_back({kv.first, kv.second});
    if (max_workers <= 0) max_workers = 1;
    if (max_workers > 5)  max_workers = 5;

    // 4) Launch one async per period (<=5), gather results
    std::vector<std::future<PeriodResults>> futs;
    ...
    std::vector<PeriodResults> results;
    results.reserve(futs.size());
    for (auto& f : futs) results.push_back(f.get());

    // 5) Map labels to fixed slots
    struct Labeled { std::string lbl; PeriodResults R; };
    std::vector<Labeled> ordered(5); // 0..4 = Fa18Inb, Fa18Out, Sp19Inb, Sp18Inb, Sp18Out
    ...
    for (size_t i = 0; i < work.size(); ++i) place(csv_label_for_period_key(work[i].key), std::move(results[i]));

    // 6) Locate output columns
    AvgColIdx Aidx;
    ...
    // 7) Compute per-row means for 5 periods and 3 combined groups
    for (const RowDef& rd : G.rows) {
        Accum per[5]; bool have[5] = {false,false,false,false,false};

        auto pull = [&](int slot, const std::string& label) {
            if (ordered[slot].lbl != label) return;
        ...
        auto combine = [&](const std::vector<int>& idxs) -> Accum {
            Accum a{};
            for (int j : idxs) if (have[j]) {
                a.sum_xB += per[j].sum_xB;
                a.sum_Q2 += per[j].sum_Q2;
                a.sum_t  += per[j].sum_t;
                a.sum_c  += per[j].sum_c;
                a.sum_s  += per[j].sum_s;
                a.n      += per[j].n;
        ...
        Accum g106 = combine({0,1,3,4}); // 10.6 GeV combined

        auto write_group = [&](int gslot, const Accum& acc) {
            double mx=0,mQ=0,mt=0,mphi=0; acc.mean(mx,mQ,mt,mphi, G.csv_phi_is_deg);
        ...
        for (int g = 0; g < 5; ++g) write_group(g, per[g]); // individual periods
        write_group(5, fa18);
        write_group(6, sp18);
        write_group(7, g106);
    }
    ...
    for (size_t r = 1; r < table.size(); ++r) fout << join_csv_row(table[r]) << "\n";
    fout.close();
