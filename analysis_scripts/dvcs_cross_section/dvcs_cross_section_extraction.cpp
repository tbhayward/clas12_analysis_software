...
int main(int argc, char* argv[]) {
    ...
    // Define filenames and check existence
    std::map<std::string, std::vector<std::string>> filenames = define_and_check_files(dir1, dir2, dir3, dir4);

    // Load TTreeReaders
    FileAndReader data = load_trees(filenames["data"]);
    FileAndReader eppi0 = load_trees(filenames["eppi0"]);
    FileAndReader mc_gen_dvcsgen = load_trees(filenames["mc_gen_dvcsgen"]);
    FileAndReader mc_rec_dvcsgen = load_trees(filenames["mc_rec_dvcsgen"]);
    FileAndReader mc_gen_aaogen = load_trees(filenames["mc_gen_aaogen"]);
    FileAndReader mc_rec_aaogen = load_trees(filenames["mc_rec_aaogen"]);
    FileAndReader mc_rec_eppi0_bkg = load_trees(filenames["mc_rec_eppi0_bkg"]);
    ...
    std::cout << "Successfully loaded all data and MC trees and created output directories." << std::endl;
