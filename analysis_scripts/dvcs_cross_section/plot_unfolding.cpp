...
std::map<std::string, std::vector<UnfoldingData>> plot_unfolding(const std::string& output_dir,
                                                                 int xB_bin,
    ...
    for (const auto& topology : topologies) {
        ...
        for (int period = 0; period < n_periods; ++period) {
            ...
            for (size_t group_idx = 0; group_idx < all_unfolding_data.size(); ++group_idx) {
                ...
                // Create canvas
                TCanvas* canvas = new TCanvas(Form("c_unfolding_dvcs_period_%d_group_%zu_%s", period, group_idx, topology.c_str()),
        ...
        for (int period = 0; period < n_periods; ++period) {
            ...
            for (size_t group_idx = 0; group_idx < all_unfolding_data.size(); ++group_idx) {
                ...
                // Create canvas
                TCanvas* canvas = new TCanvas(Form("c_unfolding_eppi0_period_%d_group_%zu_%s", eppi0_period, group_idx, topology.c_str()),
