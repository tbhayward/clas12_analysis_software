...
void plot_unfolding(const std::string& output_dir, 
    ...
    // Now, create canvases for each topology
            ...
            for (int idx = 0; idx < n_Q2t_bins; ++idx) {
                ...
                if ((*xB_data >= bin.xB_low && *xB_data <= bin.xB_high &&
                    *Q2_data >= bin.Q2_low && *Q2_data <= bin.Q2_high &&
                    ...
                    ((topologies[topo_idx] == "(FD,FD)" && *detector1_data == 1 && *detector2_data == 1) ||
                     (topologies[topo_idx] == "(CD,FD)" && *detector1_data == 2 && *detector2_data == 1) ||
                     (topologies[topo_idx] == "(CD,FT)" && *detector1_data == 2 && *detector2_data == 0)) &&
