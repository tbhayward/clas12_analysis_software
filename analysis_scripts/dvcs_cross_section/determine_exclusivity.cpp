...
void determine_exclusivity(TTreeReader& dataReader, TTreeReader& mcReader, const std::string& outputDir, const std::string& plotTitle) {
    ...
    std::vector<std::string> configurations = {"FD_FD", "CD_FD", "CD_FT"};  // Three configurations
    ...
    // Create histograms for each detector configuration
    ...
    while (dataReader.Next()) {
        ...
        // Determine configuration based on detector1 and detector2 values
        if (*detector1_data == 1 && *detector2_data == 1) {
            config_key = "FD_FD";
        } else if (*detector1_data == 2 && *detector2_data == 1) {
            config_key = "CD_FD";
        } else if (*detector1_data == 2 && *detector2_data == 0) {
            config_key = "CD_FT";
        } else {
            continue;  // Ignore any other configurations
        }

        // Normalize for electrons in the specified theta range
        ...
        for (size_t i = 0; i < variables.size(); ++i) {
            if (variables[i] == "placeholder") continue;  // Skip placeholder for now
    ...
    while (mcReader.Next()) {
        ...
        // Determine configuration based on detector1 and detector2 values
        if (*detector1_mc == 1 && *detector2_mc == 1) {
            config_key = "FD_FD";
        } else if (*detector1_mc == 2 && *detector2_mc == 1) {
            config_key = "CD_FD";
        } else if (*detector1_mc == 2 && *detector2_mc == 0) {
            config_key = "CD_FT";
        } else {
            continue;  // Ignore any other configurations
        }
    ...
    // Process each configuration and create plots
    for (const auto& config : configurations) {
        ...
        for (size_t i = 0; i < variables.size(); ++i) {
            if (variables[i] == "placeholder") continue;
            ...
            // Draw histograms
            histograms_data[config][i]->Draw("HIST");
            histograms_mc[config][i]->Draw("HIST SAME");

            // Add a legend
            TLegend* legend = new TLegend(0.375, 0.7, 0.9, 0.9);
            legend->AddEntry(histograms_data[config][i], ("Data (" + std::to_string(static_cast<int>(histograms_data[config][i]->GetEntries())) + " counts)").c_str(), "l");
            legend->AddEntry(histograms_mc[config][i], ("MC (" + std::to_string(static_cast<int>(histograms_mc[config][i]->GetEntries())) + " counts)").c_str(), "l");
            legend->Draw();
