...
void determine_exclusivity(TTreeReader& dataReader, TTreeReader& mcReader, const std::string& outputDir) {
    ...
    for (size_t i = 0; i < variables.size(); ++i) {
        ...
        TH1D* hist_data = new TH1D(hist_data_name.c_str(), hist_data_name.c_str(), config.bins, config.min, config.max);
        TH1D* hist_mc = new TH1D(hist_mc_name.c_str(), hist_mc_name.c_str(), config.bins, config.min, config.max);
