...
def calculate_dilution_error(nNH3, nC, nCH2, nHe, nET, xNH3, xC, xCH2, xHe, xET):
    ...
    term1 = (
        5438.81 * nNH3 * nET * xC**2 * xCH2**2 * xET**2 * xHe**2
        * (-1.0 * nHe * xNH3 + nNH3 * xHe)**2
        * (1.0 * nHe * xC * xCH2 - 1.19655 * nCH2 * xC * xHe + 0.196547 * nC * xCH2 * xHe)**2
    )

    # Second part of the expression (term2)
    term2 = (
        85044.9 * nNH3 * nCH2 * xC**2 * xCH2**2 * xET**2 * xHe**2
        * (-1.0 * nHe * xNH3 + nNH3 * xHe)**2
    ...
    term3 = (
        47470.0 * nNH3 * nC * xC**2 * xCH2**2 * xET**2 * xHe**2
        * (-1.0 * nHe * xNH3 + nNH3 * xHe)**2
    ...
    term5_numerator_part1 = 73.2426 * nHe**2 * xNH3 * xC**2 * xCH2**2 * xET**2

    term5_numerator_part2 = (
        nHe * xC * xCH2 * xET
        * (2.0 * nET * xNH3 * xC * xCH2
           + (-183.185 * nCH2 * xNH3 * xC + 34.6998 * nC * xNH3 * xCH2 + 1.42109e-14 * nNH3 * xC * xCH2) * xET)
    )

    term5_numerator_part3 = (
        -1.97748 * nET**2 * xNH3 * xC**2 * xCH2**2
    ...
    term5_numerator = (
        term5_numerator_part1
        + term5_numerator_part2 * xHe
        + term5_numerator_part3 * xHe**2
    )

    term5 = 0.255725 * nNH3 * nHe * term5_numerator**2
    ...
    sigma_df = 27.3473 * math.sqrt((term1 + term2 + term3 + term4 + term5) / denominNH3tor)
...
def _compute_period(period, trees, xB_bins):
    ...
    for i in range(nbins):
        # mask y
        idx = {
            t: np.digitize(data_x[t][data_y[t] < Y_CUT], xB_bins) - 1
            for t in data_x
        }
        x_mean = (
            np.mean(data_x["NH3"][data_y["NH3"] < Y_CUT][idx["NH3"] == i])
            if np.any(idx["NH3"] == i) else 0.0
        )
...
def calculate_and_save(trees, xB_bins):
    ...
    wsu = 1 / df_su["Df_sigma"]**2
    wfa = 1 / df_fa["Df_sigma"]**2
    df_avg = pd.DataFrame({
        "x_mean": df_su["x_mean"],
    ...
    csv = "output/dilution_factor.csv"
    ...
    plt.tight_layout()
    pdf = "output/dilution_factor.pdf"
    plt.savefig(pdf)
    plt.close()
