...
// hard‐coded NH3 ROOT file paths for each period
static const std::map<std::string,std::string> filePaths = {
  {"RGC_Su22", "/work/clas12/thayward/CLAS12_SIDIS/processed_data/"
                "pass2/data/eX/rgc_su22_inb_NH3_eX.root"},
  {"RGC_Fa22", "/work/clas12/thayward/CLAS12_SIDIS/processed_data/"
                "pass2/data/eX/rgc_fa22_inb_NH3_eX.root"},
  {"RGC_Sp23", "/work/clas12/thayward/CLAS12_SIDIS/processed_data/"
                "pass2/data/eX/rgc_sp23_inb_NH3_eX.root"}
};

// select which periods to run
...
int main() {
  ...
  std::map<int,int>   signMap;
  {
    ...
    while (std::getline(runinfo, line)) {
      ...
      ss >> run >> comma >> ch >> comma >> d1 >> comma
         >> d2 >> comma >> pol_s >> comma >> pol_e;
      chargeMap[run] = ch;
      signMap[run]   = (pol_s > 0 ? +1 : -1);
    }
  }
  std::cout << "[Loaded] " << chargeMap.size() << " runs\n";
  ...
  for (auto& period : periods) {
    ...
    // open NH3 file
    const auto& path = filePaths.at(period);
    TFile f(path.c_str());
