...
std::vector<std::string> selectPeriods() {
  ...
  if (runMode==2) return {"RGC_Fa22"};
  if (runMode==3) return {"RGC_Sp23"};
  // else 0: all
  return {"RGC_Su22","RGC_Fa22","RGC_Sp23"};
}
...
int main() {
  ...
  if(!runinfo){ std::cerr<<"Cannot open "<<RUNINFO<<"\n"; return 1;}
  ...
  std::map<int,int>    signMap;
  {
    ...
    while(std::getline(runinfo,line)){
      ...
      int run; double ch,d1,d2, pol_s, pol_e; char c;
      ss>>run>>c>>ch>>c>>d1>>c>>d2>>c>>pol_s>>c>>pol_e;
      chargeMap[run]=ch;
      signMap[run]=(pol_s>0?+1:-1);
    }
  }
  std::cout<<"[Loaded] "<<chargeMap.size()<<" runs with charges\n";

  // load dilution CSV
  std::ifstream dfcsv("output/dilution_factor.csv");
  std::vector<double> Df, sDf;
  { std::string hdr; std::getline(dfcsv,hdr);
    std::string ln; while(std::getline(dfcsv,ln) && Df.size()<xB_bins.size()-1){
      std::stringstream ss(ln);
      double xm,df,dfs; char c;
      ss>>xm>>c>>df>>c>>dfs;
      Df.push_back(df); sDf.push_back(dfs);
    }
  }
  ...
  for(auto& period:periods){
    ...
    // open each target just to grab the tree (all share same run structure)
    auto &path = filePaths[period]["NH3"];
    TFile f(path.c_str());
    TTree* tree = (TTree*)f.Get("PhysicsEvents");

    // prepare counters per run/bin
    struct Counts{ std::vector<long> Np,Nm; };
    std::map<int,Counts> cnt;
    for(auto& kv:chargeMap) cnt[kv.first] = {std::vector<long>(xB_bins.size()-1,0),
    ...
    for(Long64_t i=0;i<N;++i){
      ...
      if(helicity*sgn>0) it->second.Np[b]++; else it->second.Nm[b]++;
    }
    ...
    for(auto& kv:cnt){
      ...
      for(int b=0;b<(int)C.Np.size();++b){
        long Np=C.Np[b], Nm=C.Nm[b], S=Np+Nm;
        if(S<1) continue;
        ...
        double Pt_g = Δ/(Ag*S), Pt_a = Δ/(Aa*S);
        ...
        eg = std::sqrt(eg*eg + (Pt_g*s_df/df)*(Pt_g*s_df/df)
        ...
        ea = std::sqrt(ea*ea + (Pt_a*s_df/df)*(Pt_a*s_df/df)
      ...
      TF1 f0("f0","[0]",0,1);
      g_grv.Fit(&f0,"Q"); double Pt_grv=f0.GetParameter(0), s_grv=f0.GetParError(0);

      // fit ABD
      TGraphErrors g_abd(xv.size(),&xv[0],&ya[0],nullptr,&ye_ab[0]);
      TF1 f1("f1","[0]",0,1);
      g_abd.Fit(&f1,"Q"); double Pt_abd=f1.GetParameter(0), s_abd=f1.GetParError(0);
