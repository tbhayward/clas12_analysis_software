...
def plot_normalized_yields(trees, xB_bins):
    ...
    for target in ("He", "ET", "CH2"):
        ...
        for ax, p in zip(axes, periods):
            res = results[p]
            # print overall stats
            print(f"[Stats]  Period={p}, Target={target}, Mean Integral={res['mean']:.4f}, Std Integral={res['std']:.4f}")
            for idx, entry in enumerate(res["results"]):
                run = entry["run"]
                norm = entry["norm"]
                style = ["solid","dashed","dashdot","dotted"][idx % 4]
