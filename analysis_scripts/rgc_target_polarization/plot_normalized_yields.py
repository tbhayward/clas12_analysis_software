...
Generates five figures:
  ...
  5) Per-run CH2 normalized yields for each period (1×3 grid)

Handles occasional basket decompression errors gracefully.
"""
...
# Per-run colors (max 5 runs)
PER_RUN_COLORS = ['black','blue','orange','green','red']


def safe_array(tree, branch):
    """
    Try to read `branch` from `tree`; catch decompress errors and return empty array.
    """
    try:
        return tree[branch].array(library="np")
    except AssertionError as e:
        print(f"[Warning] Could not read branch '{branch}' in {tree.name}: {e}")
        return np.empty(0)
    ...
    def per_run_plot(target, fname):
        ...
        for ax,p in zip(axes,periods):
            ...
            for i,run in enumerate(ur):
                ...
                m=rn==run
                cnt,_=np.histogram(xv[m],bins=bins)
                ax.step(centers,cnt/ch,where='mid',color=PER_RUN_COLORS[i],linewidth=1.5,label=str(run))
            ax.set_title(f"{p.replace('RGC_','')} {target}")
            ax.set_xlabel(r'$x_B$'); ax.set_ylabel('counts/nC')
            ax.legend(fontsize='x-small',ncol=2,frameon=False)
        plt.tight_layout(pad=2)
        fig.savefig(f'output/normalized_yields_runs_{target.lower()}.pdf')
        plt.close(fig)
        print(f"[Plot] saved output/normalized_yields_runs_{target.lower()}.pdf")

    # -------- FIG 3,4,5: per-run He, ET, CH2 --------
    per_run_plot('He','runs_he')
    per_run_plot('ET','runs_et')
    per_run_plot('CH2','runs_ch2')
