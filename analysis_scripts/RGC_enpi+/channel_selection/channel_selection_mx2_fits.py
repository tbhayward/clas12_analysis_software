#!/usr/bin/env python3
"""
channel_selection_mx2_fits_v11.py
...
def execute_fit_jobs(
    ...
    print("Running channel_selection_mx2_fits_v11.py", flush=True)
...
def write_latex_nominal_tables(frame: pd.DataFrame, output_path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("% Auto-generated by channel_selection_mx2_fits_v11.py")
...
def plot_carbon_scale_scans(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            fig.tight_layout()
            path = output_dir / (
                f"carbon_phase1_scale_scan_{period}_{stage}_v11.png"
            )
...
def plot_carbon_group_scales(
    ...
    for stage in ("before", "after"):
        ...
        ax.legend()
        fig.tight_layout()
        path = output_dir / f"carbon_phase1_group_scales_{stage}_v11.png"
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
    # endfor
...
def plot_carbon_negative_bin_fraction(
    ...
    for stage in ("before", "after"):
        ...
        fig.tight_layout()
        path = output_dir / (
            f"carbon_phase1_negative_bin_fraction_{stage}_v11.png"
        )
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
...
def plot_problematic_carbon_subtractions(
    ...
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.955))
    path = output_dir / "carbon_phase1_problematic_bins_after_v11.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def write_carbon_phase1_products(
    ...
    """Write Phase-1 carbon tables, JSON, and plots."""
    ensure_directory(table_dir)
    ensure_directory(plot_dir)

    group_csv = table_dir / "carbon_phase1_group_scales_v11.csv"
    scan_csv = table_dir / "carbon_phase1_scale_scan_v11.csv"
    bin_csv = table_dir / "carbon_phase1_bin_diagnostics_v11.csv"
    json_path = table_dir / "carbon_phase1_diagnostics_v11.json"

    pd.DataFrame(diagnostics["group_rows"]).to_csv(group_csv, index=False)
    pd.DataFrame(diagnostics["scan_rows"]).to_csv(scan_csv, index=False)
    pd.DataFrame(diagnostics["bin_rows"]).to_csv(bin_csv, index=False)

    json_payload = {
        "configuration": diagnostics["configuration"],
        "group_scales": diagnostics["group_rows"],
        "scale_scan": diagnostics["scan_rows"],
        "bin_diagnostics": diagnostics["bin_rows"],
    }
    json_path.write_text(
        json.dumps(
            json_safe(json_payload),
            indent=2,
            sort_keys=False,
        )
        + "\n",
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            for mode in ("overlay", "subtraction", "ratio"):
                path = plot_dir / (
                    f"carbon_phase1_{mode}_{period}_{stage}_v11.png"
                )
        ...
        # endfor
    # endfor
    plots.extend(plot_carbon_scale_scans(diagnostics, plot_dir))
    plots.extend(plot_carbon_group_scales(diagnostics, plot_dir))
    plots.extend(plot_carbon_local_scale_closure(diagnostics, plot_dir))
    plots.extend(plot_carbon_negative_bin_fraction(diagnostics, plot_dir))
    plots.extend(plot_carbon_closure_pulls(diagnostics, plot_dir))
    plots.append(plot_problematic_carbon_subtractions(diagnostics, plot_dir))
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="tprime-column",
        help=(
            "Carbon normalization grouping. The default pools all xB rows "
            "within each -tprime column, separately by period and stage."
        ),
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        carbon_phase1_table_dir = carbon_phase1_dir / "tables"
        carbon_phase1_plot_dir = carbon_phase1_dir / "plots"
        for path in (
            output_dir,
            table_dir,
            plot_dir,
            ...
            pull_diagnostic_dir,
            carbon_phase1_dir,
            carbon_phase1_table_dir,
            carbon_phase1_plot_dir,
        ):
        ...
        csv_path = table_dir / "mx2_peak_fit_results_v11.csv"
        json_path = table_dir / "mx2_peak_fit_results_v11.json"
        latex_path = table_dir / "mx2_peak_nominal_corrected_tables_v11.tex"
        status_path = table_dir / "mx2_peak_fit_status_v11.txt"
        ...
        if not args.disable_carbon_phase1:
            carbon_diagnostics = build_carbon_phase1_diagnostics(
                nh3_jobs=jobs,
                carbon_jobs=carbon_jobs,
                grouping=args.carbon_normalization_grouping,
            ...
            carbon_diagnostics["configuration"][
                "carbon_loading_metadata"
            ] = carbon_loading_metadata
            carbon_diagnostics["configuration"]["carbon_inputs"] = {
                period: {
                    stage: str(path)
                    for stage, path in stage_map.items()
                }
            ...
            carbon_products = write_carbon_phase1_products(
                diagnostics=carbon_diagnostics,
                table_dir=carbon_phase1_table_dir,
                plot_dir=carbon_phase1_plot_dir,
            )
        # endif
        ...
        if carbon_products is not None:
            ...
            print(
                ...
                f"{args.carbon_normalization_grouping}, "
