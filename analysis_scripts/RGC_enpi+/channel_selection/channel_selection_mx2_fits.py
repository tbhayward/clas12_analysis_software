#!/usr/bin/env python3
"""
channel_selection_mx2_fits_v18.py
...
otherwise unresolved corrected bins. A candidate constrained-carbon method is also evaluated without replacing the
original production fits. It uses a period-wide carbon scale as a Gaussian-
constrained nuisance parameter, compares linear and quadratic residual
backgrounds, and performs fit-range and control-region stability checks.
...
def execute_fit_jobs(
    ...
    print("Running channel_selection_mx2_fits_v18.py", flush=True)
...
def write_latex_nominal_tables(frame: pd.DataFrame, output_path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("% Auto-generated by channel_selection_mx2_fits_v18.py")
...
def plot_carbon_scale_scans(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            fig.tight_layout()
            path = output_dir / (
                f"carbon_phase1_scale_scan_{period}_{stage}_v18.png"
            )
...
def plot_carbon_group_scales(
    ...
    for stage in ("before", "after"):
        ...
        ax.legend()
        fig.tight_layout()
        path = output_dir / f"carbon_phase1_group_scales_{stage}_v18.png"
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
    # endfor
...
def plot_carbon_negative_bin_fraction(
    ...
    for stage in ("before", "after"):
        ...
        fig.tight_layout()
        path = output_dir / (
            f"carbon_phase1_negative_bin_fraction_{stage}_v18.png"
        )
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
...
def plot_problematic_carbon_subtractions(
    ...
    path = output_dir / "carbon_phase1_problematic_bins_after_v18.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def write_carbon_phase1_products(
    ...
    nominal_group_csv = (
        tables_dir / "carbon_phase1_period_scales_v18.csv"
    )
    column_group_csv = (
        tables_dir / "carbon_phase1_tprime_column_scales_v18.csv"
    )
    nominal_scan_csv = (
        tables_dir / "carbon_phase1_period_scale_scan_v18.csv"
    )
    nominal_bin_csv = (
        tables_dir / "carbon_phase1_bin_diagnostics_v18.csv"
    )
    json_path = tables_dir / "carbon_phase1_diagnostics_v18.json"
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            for mode, directory in destinations.items():
                path = directory / (
                    f"carbon_phase1_{mode}_{period}_{stage}_v18.png"
                )
...
def carbon_residual_background(
    ...
    """Evaluate a residual polynomial about the neutron-mass-squared point."""
    dx = np.asarray(x, dtype=float) - NEUTRON_MASS2_GEV2
...
def fit_one_carbon_candidate(
    ...
    """Fit one bin with shared mu, constrained alphas, and P1 or P2 residuals."""
    ...
    ncoef=residual_order+1
    # shared mu; then alpha, yield, sigma, residual coefficients for each period
    ...
    for period in periods:
        ...
        side=~peak
        c0=float(np.median(residual[side])) if np.any(side) else 0.0
        initial.extend([item['alpha0'],y0,0.055,c0]+[0.0]*residual_order)
        lower.extend([0.0,0.0,0.015]+[-1.0e7]*ncoef)
        upper.extend([max(10.0*item['alpha0'],1.0),1.0e10,0.20]+[1.0e7]*ncoef)
    # endfor
    ...
    for idx,period in enumerate(periods):
        ...
        rec={
            ...
            'model':f'Gaussian + constrained carbon + P{residual_order}',
...
def plot_carbon_template_fit_canvases(results: list[dict[str, Any]], output_dir: Path) -> list[Path]:
    """Draw selected carbon-template fits with protected title/legend spacing."""
    ensure_directory(output_dir)
    lookup={(r['stage'],r['x_index'],r['t_index']):r for r in results}; outputs=[]
    for period in ('su22','fa22','sp23'):
        for stage in ('before','after'):
            fig,axes=plt.subplots(len(XB_BINS),len(MINUS_TPRIME_BINS_GEV2),figsize=(24,15),sharex=True,sharey=False)
            for xi in range(len(XB_BINS)):
                for ti in range(len(MINUS_TPRIME_BINS_GEV2)):
                    ax=axes[xi,ti]; joint=lookup[(stage,xi,ti)]; p=joint['payloads'][period]; rec=p['record']; x=p['centers']
                    ax.errorbar(x,p['nh3'],yerr=p['error'],fmt='o',markersize=2,linewidth=.7,label='NH$_3$')
                    ax.step(x,p['carbon'],where='mid',linewidth=1,label='Constrained C')
                    ax.plot(x,p['carbon']+p['residual'],linestyle='--',linewidth=1,label=f"C + P{joint['selected_model']}")
                    ax.plot(x,p['total'],linewidth=1.4,label='Total fit')
                    ax.axvline(NEUTRON_MASS2_GEV2,linestyle=':',linewidth=.8)
                    ax.set_title(f"Bin {rec['bin_number']}: {joint['classification']}, $\\mu$={rec['shared_mean_gev2']:.4f}, $\\chi^2_\\nu$={rec['local_chi2_ndf']:.2f}",fontsize=8)
                    ax.grid(alpha=.22)
                    if xi==len(XB_BINS)-1: ax.set_xlabel('$M_x^2$ (GeV$^2$)')
                    if ti==0:
                        lo,hi=XB_BINS[xi]; ax.set_ylabel(f'{lo:.2f} < $x_B$ < {hi:.2f}\nCounts')
                # endfor
            # endfor
            handles, labels = axes[0, 0].get_legend_handles_labels()
            fig.suptitle(
            ...
            path=output_dir/f'carbon_template_fits_{period}_{stage}_v18.png'; fig.savefig(path,dpi=180); plt.close(fig); outputs.append(path)
        # endfor
    # endfor
    return outputs
...
def write_carbon_template_fit_products(results: list[dict[str, Any]], production_frame: pd.DataFrame, table_dir: Path, plot_dir: Path) -> dict[str, Any]:
    ...
    csv_path=table_dir/'carbon_template_fit_results_v18.csv'; json_path=table_dir/'carbon_template_fit_results_v18.json'
    comparison_path=table_dir/'carbon_vs_polynomial_comparison_v18.csv'
...
def build_final_carbon_cut_table(
    ...
    """
    Build tight, nominal, and loose Mx2 windows from the corrected carbon fits.

    A single shared mean is used in each kinematic bin. The common cut width is
    the maximum of the three period-specific fitted Gaussian widths.
    """
    rows: list[dict[str, Any]] = []
    for joint in _selected_corrected_carbon_results(results):
        ...
        sigma_values = {
            period: float(period_records[period]["sigma_gev2"])
            for period in ("su22", "fa22", "sp23")
        }
        sigma_cut = max(sigma_values.values())
        mean = float(joint["shared_mean_gev2"])
        payloads = joint["payloads"]

        row: dict[str, Any] = {
            "x_index": joint["x_index"],
            "t_index": joint["t_index"],
            "bin_number": combined_bin_number(
                joint["x_index"],
                joint["t_index"],
            ),
            ...
            "shared_mean_error_gev2": (
                joint["shared_mean_error_gev2"]
            ),
            "sigma_su22_gev2": sigma_values["su22"],
            "sigma_fa22_gev2": sigma_values["fa22"],
            "sigma_sp23_gev2": sigma_values["sp23"],
            "sigma_cut_gev2": sigma_cut,
            "sigma_cut_source_period": max(
                sigma_values,
                key=sigma_values.get,
            ),
            "selected_residual_order": joint["selected_model"],
            "classification": joint["classification"],
            "joint_chi2_ndf": joint["joint_chi2_ndf"],
            "maximum_local_chi2_ndf": (
                joint["maximum_local_chi2_ndf"]
            ),
            "fit_range_stability_gev2": (
                joint["maximum_abs_fit_range_delta_mean_gev2"]
            ),
            "control_endpoint_stability_gev2": (
                joint["maximum_abs_control_delta_mean_gev2"]
            ),
            "model_mean_difference_gev2": (
        ...
        for label, multiple in (
            ...
            row[f"{label}_sigma_multiple"] = multiple
            row[f"{label}_min_gev2"] = mean - multiple * sigma_cut
            row[f"{label}_max_gev2"] = mean + multiple * sigma_cut
        # endfor

        # Quantify component composition inside the nominal window.
        nominal_low = row["nominal_min_gev2"]
        nominal_high = row["nominal_max_gev2"]
        signal_total = 0.0
        carbon_total = 0.0
        residual_total = 0.0
        ...
        for period in ("su22", "fa22", "sp23"):
            ...
            mask = (
                (centers >= nominal_low)
                & (centers < nominal_high)
            )
            signal_total += float(np.sum(payload["signal"][mask]))
            carbon_total += float(np.sum(payload["carbon"][mask]))
            residual_total += float(np.sum(payload["residual"][mask]))
            total_prediction += float(np.sum(payload["total"][mask]))
        # endfor

        background_total = carbon_total + residual_total
        row["nominal_signal_integral"] = signal_total
        row["nominal_carbon_integral"] = carbon_total
        row["nominal_residual_background_integral"] = residual_total
        row["nominal_total_background_integral"] = background_total
        row["nominal_total_prediction_integral"] = total_prediction
        row["nominal_estimated_signal_fraction"] = (
            signal_total / total_prediction
            if total_prediction > 0.0
            else math.nan
        )
        row["nominal_estimated_background_fraction"] = (
            background_total / total_prediction
            if total_prediction > 0.0
            else math.nan
        )
        row["ideal_gaussian_containment_tight"] = math.erf(
            2.0 / math.sqrt(2.0)
        )
        row["ideal_gaussian_containment_nominal"] = math.erf(
            3.0 / math.sqrt(2.0)
        )
        row["ideal_gaussian_containment_loose"] = math.erf(
            4.0 / math.sqrt(2.0)
        )
        rows.append(row)
    # endfor
...
def write_python_cut_lookup(
    ...
    """Write a small importable Python lookup for the three cut variations."""
    lines = [
        '"""Auto-generated carbon-assisted Mx2 cut lookup."""',
        "",
        "MX2_CUTS_GEV2 = {",
    ]
    for row in cuts.itertuples(index=False):
        lines.extend(
            [
                ...
                f'        "tight": ({row.tight_min_gev2:.12g}, '
                f"{row.tight_max_gev2:.12g}),",
                f'        "nominal": ({row.nominal_min_gev2:.12g}, '
                f"{row.nominal_max_gev2:.12g}),",
                f'        "loose": ({row.loose_min_gev2:.12g}, '
                f"{row.loose_max_gev2:.12g}),",
                f'        "mu": {row.shared_mean_gev2:.12g},',
                f'        "sigma_cut": {row.sigma_cut_gev2:.12g},',
...
def write_cpp_cut_lookup(
    ...
    """Write a compact C++ header containing the final cut table."""
    lines = [
        "#pragma once",
        "",
        "#include <array>",
        ...
        "};",
        "",
        f"inline constexpr std::array<Mx2CutEntry, {len(cuts)}> "
        "kMx2CutsGeV2 = {{",
    ]
    for row in cuts.itertuples(index=False):
        lines.append(
            "    Mx2CutEntry{"
            f"{row.x_index}, {row.t_index}, "
            f"{row.tight_min_gev2:.12g}, {row.tight_max_gev2:.12g}, "
            f"{row.nominal_min_gev2:.12g}, {row.nominal_max_gev2:.12g}, "
            f"{row.loose_min_gev2:.12g}, {row.loose_max_gev2:.12g}, "
            f"{row.shared_mean_gev2:.12g}, {row.sigma_cut_gev2:.12g}"
            "},"
        )
    # endfor
    lines.extend(
...
def plot_final_cut_canvases(
    ...
    and residual polynomial are smooth curves. The total prediction is drawn
    as a smooth interpolation for display only; the underlying fit remains
    histogram based.
    """
    ensure_directory(output_dir)
    cut_lookup = {
    ...
    for period in ("su22", "fa22", "sp23"):
        ...
        for x_index in range(len(XB_BINS)):
            for t_index in range(len(MINUS_TPRIME_BINS_GEV2)):
                ...
                carbon_dense = np.interp(
                    dense_x,
                    centers,
                    np.asarray(payload["carbon"], dtype=float),
                )
                ...
                ax.plot(
                    ...
                    linewidth=1.0,
                    linestyle="--",
                    label=f"Residual $P_{joint['selected_model']}$",
                    zorder=3,
                )
                ax.plot(
                    dense_x,
                ...
                ax.axvspan(
                    cut.nominal_min_gev2,
                    cut.nominal_max_gev2,
                    alpha=0.12,
                    zorder=0,
                )
                ax.axvline(
                ...
                ax.set_title(
                    ...
                    f"$\\sigma_{{cut}}$={cut.sigma_cut_gev2:.4f}",
                    fontsize=8,
                )
                ax.grid(alpha=0.22)
                if x_index == len(XB_BINS) - 1:
        ...
        handles = [
            ...
            Line2D([0], [0], linestyle="-", linewidth=1.5, label="Total model"),
        ...
        output_path = output_dir / (
            f"final_carbon_assisted_cuts_{period}_v18.png"
        )
        fig.savefig(output_path, dpi=180)
        plt.close(fig)
...
def plot_master_cut_summary(
    ...
    """Plot fitted centers, cut widths, boundaries, and nominal composition."""
    ...
    ax_bounds.fill_between(
        bins,
        frame["tight_min_gev2"],
        frame["tight_max_gev2"],
        alpha=0.10,
        label="Tight (2$\\sigma$)",
    )
    ax_bounds.fill_between(
        bins,
        frame["nominal_min_gev2"],
        frame["nominal_max_gev2"],
        alpha=0.12,
        label="Nominal (3$\\sigma$)",
    )
    ax_bounds.plot(
        bins,
        frame["loose_min_gev2"],
        linestyle="-.",
        linewidth=0.9,
        label="Loose lower/upper (4$\\sigma$)",
    )
    ax_bounds.plot(
        bins,
        frame["loose_max_gev2"],
        linestyle="-.",
        linewidth=0.9,
    )
    ax_bounds.axhline(
    ...
    ax_bounds.set_ylabel("$M_x^2$ cut position (GeV$^2$)")
    ax_bounds.set_title(
        "Final carbon-assisted missing-neutron cut summary"
    )
    ax_bounds.grid(alpha=0.25)
    ax_bounds.legend(ncol=4, fontsize=9)

    ax_width = axes[1]
    ax_width.plot(
        bins,
        frame["sigma_su22_gev2"],
        marker="o",
        linewidth=1.0,
        label="Su22 $\\sigma$",
    )
    ...
    ax_width.plot(
        bins,
        frame["sigma_cut_gev2"],
        marker="s",
        linestyle="--",
        linewidth=1.2,
        label="$\\sigma_{cut}=\\max_r\\sigma_r$",
    )
    ax_width.set_ylabel("Fitted width (GeV$^2$)")
    ax_width.grid(alpha=0.25)
    ax_width.legend(ncol=4, fontsize=9)

    ax_fraction = axes[2]
    ax_fraction.plot(
        bins,
        frame["nominal_estimated_signal_fraction"],
        marker="o",
        linewidth=1.1,
        label="Estimated signal fraction",
    )
    ax_fraction.plot(
        bins,
        frame["nominal_estimated_background_fraction"],
        marker="s",
        linestyle="--",
        linewidth=1.1,
        label="Estimated background fraction",
    )
    ax_fraction.set_xlabel("Combined kinematic-bin number")
    ax_fraction.set_ylabel("Fraction inside nominal window")
    ax_fraction.set_ylim(0.0, 1.05)
    ax_fraction.set_xticks(bins)
    ax_fraction.grid(alpha=0.25)
    ax_fraction.legend(ncol=2, fontsize=9)
...
def write_final_carbon_cut_products(
    ...
    csv_path = tables_dir / "final_carbon_assisted_mx2_cuts_v18.csv"
    json_path = tables_dir / "final_carbon_assisted_mx2_cuts_v18.json"
    python_path = exports_dir / "final_carbon_assisted_mx2_cuts_v18.py"
    cpp_path = exports_dir / "final_carbon_assisted_mx2_cuts_v18.h"
    comparison_plot = (
        plots_dir / "carbon_vs_polynomial_mu_sigma_v18.png"
    )
    master_summary_plot = (
        plots_dir / "final_carbon_cut_master_summary_v18.png"
    )
    ...
    json_path.write_text(
        json.dumps(
            json_safe(
                {
                    "method": (
                        "Shared carbon-assisted mean with the maximum "
                        "period-specific width in each kinematic bin."
                    ),
                    "tight_sigma_multiple": 2.0,
                    "nominal_sigma_multiple": 3.0,
                    "loose_sigma_multiple": 4.0,
                    "cuts": cuts.to_dict(orient="records"),
                }
            ),
            indent=2,
        )
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.20,
        help="Low-Mx2 control-region minimum in GeV^2 (default: 0.20).",
    )
    parser.add_argument(
        "--carbon-control-max",
        type=float,
...
def main() -> int:
    ...
    try:
        ...
        carbon_input_paths: dict[str, dict[str, Path]] | None = None
        ...
        if not args.disable_carbon_phase1:
            carbon_input_paths = {
                ...
                for period, stage_map in DEFAULT_CARBON_INPUTS.items()
            }
            for period, stage, path in args.carbon_input:
                carbon_input_paths[period][stage] = path
            # endfor
        ...
        csv_path = table_dir / "mx2_peak_fit_results_v18.csv"
        json_path = table_dir / "mx2_peak_fit_results_v18.json"
        latex_path = table_dir / "mx2_peak_nominal_corrected_tables_v18.tex"
        status_path = table_dir / "mx2_peak_fit_status_v18.txt"
...
# =============================================================================
# v17 plotting improvements (non-algorithmic)
# =============================================================================
#
# Recommended display updates implemented/queued:
#
# 1. Keep the statistical model unchanged. The fit still evaluates the measured
#    carbon template histogram directly.
#
# 2. For plotting only, draw the carbon template as a step histogram while the
#    Gaussian, residual polynomial, and total prediction are rendered as smooth
#    curves. If a smooth display of the carbon contribution is later desired,
#    interpolate the template only for visualization, never for fitting.
#
# 3. Replace six vertical cut lines with:
#       - translucent shaded nominal (3σ) acceptance band;
#       - darker edges for the nominal boundaries;
#       - lighter dashed tight/loose boundary markers.
#
# 4. Produce a dedicated summary figure:
#       Top:    shared μ (carbon vs polynomial)
#       Middle: Δμ = μ_carbon − μ_polynomial
#       Bottom: σ comparison for each period
#    with resolved/marginal bins visually distinguished.
#
# These changes affect presentation only and do not modify the fitted
# parameters or cut definitions.
