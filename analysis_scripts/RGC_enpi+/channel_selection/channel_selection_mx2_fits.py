#!/usr/bin/env python3
"""
channel_selection_mx2_fits_v14.py
...
def execute_fit_jobs(
    ...
    print("Running channel_selection_mx2_fits_v14.py", flush=True)
...
def write_latex_nominal_tables(frame: pd.DataFrame, output_path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("% Auto-generated by channel_selection_mx2_fits_v14.py")
...
def plot_carbon_scale_scans(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            fig.tight_layout()
            path = output_dir / (
                f"carbon_phase1_scale_scan_{period}_{stage}_v14.png"
            )
...
def plot_carbon_group_scales(
    ...
    for stage in ("before", "after"):
        ...
        ax.legend()
        fig.tight_layout()
        path = output_dir / f"carbon_phase1_group_scales_{stage}_v14.png"
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
    # endfor
...
def plot_carbon_negative_bin_fraction(
    ...
    for stage in ("before", "after"):
        ...
        fig.tight_layout()
        path = output_dir / (
            f"carbon_phase1_negative_bin_fraction_{stage}_v14.png"
        )
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
...
def plot_problematic_carbon_subtractions(
    ...
    path = output_dir / "carbon_phase1_problematic_bins_after_v14.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def write_carbon_phase1_products(
    ...
    nominal_group_csv = (
        tables_dir / "carbon_phase1_period_scales_v14.csv"
    )
    column_group_csv = (
        tables_dir / "carbon_phase1_tprime_column_scales_v14.csv"
    )
    nominal_scan_csv = (
        tables_dir / "carbon_phase1_period_scale_scan_v14.csv"
    )
    nominal_bin_csv = (
        tables_dir / "carbon_phase1_bin_diagnostics_v14.csv"
    )
    json_path = tables_dir / "carbon_phase1_diagnostics_v14.json"
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            for mode, directory in destinations.items():
                path = directory / (
                    f"carbon_phase1_{mode}_{period}_{stage}_v14.png"
                )
...
def plot_carbon_template_fit_canvases(results: list[dict[str, Any]], output_dir: Path) -> list[Path]:
    ...
    for period in ('su22','fa22','sp23'):
        for stage in ('before','after'):
            ...
            # endfor
            handles,labels=axes[0,0].get_legend_handles_labels()
            fig.legend(handles,labels,loc='upper center',bbox_to_anchor=(0.5,0.975),ncol=4,frameon=True)
            fig.suptitle(f"{PERIOD_LABELS[period]}: {STAGE_LABELS[stage]}\nCandidate Gaussian + constrained carbon + residual-polynomial fits",fontsize=15,y=0.995)
            fig.tight_layout(rect=(0.0,0.0,1.0,0.91))
            path=output_dir/f'carbon_template_fits_{period}_{stage}_v14.png'; fig.savefig(path,dpi=180); plt.close(fig); outputs.append(path)
        # endfor
    # endfor
    return outputs
...
def write_carbon_template_fit_products(results: list[dict[str, Any]], production_frame: pd.DataFrame, table_dir: Path, plot_dir: Path) -> dict[str, Any]:
    ...
    csv_path=table_dir/'carbon_template_fit_results_v14.csv'; json_path=table_dir/'carbon_template_fit_results_v14.json'
    comparison_path=table_dir/'carbon_vs_polynomial_comparison_v14.csv'
...
def main() -> int:
    ...
    try:
        ...
        csv_path = table_dir / "mx2_peak_fit_results_v14.csv"
        json_path = table_dir / "mx2_peak_fit_results_v14.json"
        latex_path = table_dir / "mx2_peak_nominal_corrected_tables_v14.tex"
        status_path = table_dir / "mx2_peak_fit_status_v14.txt"
