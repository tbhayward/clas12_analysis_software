#!/usr/bin/env python3
"""
channel_selection_mx2_fits_v19.py
...
def execute_fit_jobs(
    ...
    print("Running channel_selection_mx2_fits_v19.py", flush=True)
...
def write_latex_nominal_tables(frame: pd.DataFrame, output_path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("% Auto-generated by channel_selection_mx2_fits_v19.py")
...
def plot_carbon_scale_scans(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            fig.tight_layout()
            path = output_dir / (
                f"carbon_phase1_scale_scan_{period}_{stage}_v19.png"
            )
...
def plot_carbon_group_scales(
    ...
    for stage in ("before", "after"):
        ...
        ax.legend()
        fig.tight_layout()
        path = output_dir / f"carbon_phase1_group_scales_{stage}_v19.png"
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
    # endfor
...
def plot_carbon_negative_bin_fraction(
    ...
    for stage in ("before", "after"):
        ...
        fig.tight_layout()
        path = output_dir / (
            f"carbon_phase1_negative_bin_fraction_{stage}_v19.png"
        )
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
...
def plot_problematic_carbon_subtractions(
    ...
    path = output_dir / "carbon_phase1_problematic_bins_after_v19.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def write_carbon_phase1_products(
    ...
    nominal_group_csv = (
        tables_dir / "carbon_phase1_period_scales_v19.csv"
    )
    column_group_csv = (
        tables_dir / "carbon_phase1_tprime_column_scales_v19.csv"
    )
    nominal_scan_csv = (
        tables_dir / "carbon_phase1_period_scale_scan_v19.csv"
    )
    nominal_bin_csv = (
        tables_dir / "carbon_phase1_bin_diagnostics_v19.csv"
    )
    json_path = tables_dir / "carbon_phase1_diagnostics_v19.json"
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            for mode, directory in destinations.items():
                path = directory / (
                    f"carbon_phase1_{mode}_{period}_{stage}_v19.png"
                )
...
def plot_carbon_template_fit_canvases(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            path = output_dir / (
                f"carbon_template_fits_{period}_{stage}_v19.png"
            )
            fig.savefig(path, dpi=180)
            plt.close(fig)
            outputs.append(path)
...
def write_carbon_template_fit_products(results: list[dict[str, Any]], production_frame: pd.DataFrame, table_dir: Path, plot_dir: Path) -> dict[str, Any]:
    ...
    csv_path=table_dir/'carbon_template_fit_results_v19.csv'; json_path=table_dir/'carbon_template_fit_results_v19.json'
    comparison_path=table_dir/'carbon_vs_polynomial_comparison_v19.csv'
...
def plot_final_cut_canvases(
    ...
    for period in ("su22", "fa22", "sp23"):
        ...
        output_path = output_dir / (
            f"final_carbon_assisted_cuts_{period}_v19.png"
        )
        fig.savefig(output_path, dpi=180)
        plt.close(fig)
...
def write_final_carbon_cut_products(
    ...
    csv_path = tables_dir / "final_carbon_assisted_mx2_cuts_v19.csv"
    json_path = tables_dir / "final_carbon_assisted_mx2_cuts_v19.json"
    python_path = exports_dir / "final_carbon_assisted_mx2_cuts_v19.py"
    cpp_path = exports_dir / "final_carbon_assisted_mx2_cuts_v19.h"
    comparison_plot = (
        plots_dir / "carbon_vs_polynomial_mu_sigma_v19.png"
    )
    master_summary_plot = (
        plots_dir / "final_carbon_cut_master_summary_v19.png"
    )
...
def main() -> int:
    ...
    try:
        ...
        csv_path = table_dir / "mx2_peak_fit_results_v19.csv"
        json_path = table_dir / "mx2_peak_fit_results_v19.json"
        latex_path = table_dir / "mx2_peak_nominal_corrected_tables_v19.tex"
        status_path = table_dir / "mx2_peak_fit_status_v19.txt"
