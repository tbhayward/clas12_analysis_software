#!/usr/bin/env python3
"""
channel_selection_mx2_fits_v16.py
...
def execute_fit_jobs(
    ...
    print("Running channel_selection_mx2_fits_v16.py", flush=True)
...
def write_latex_nominal_tables(frame: pd.DataFrame, output_path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("% Auto-generated by channel_selection_mx2_fits_v16.py")
...
def plot_carbon_scale_scans(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            fig.tight_layout()
            path = output_dir / (
                f"carbon_phase1_scale_scan_{period}_{stage}_v16.png"
            )
...
def plot_carbon_group_scales(
    ...
    for stage in ("before", "after"):
        ...
        ax.legend()
        fig.tight_layout()
        path = output_dir / f"carbon_phase1_group_scales_{stage}_v16.png"
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
    # endfor
...
def plot_carbon_negative_bin_fraction(
    ...
    for stage in ("before", "after"):
        ...
        fig.tight_layout()
        path = output_dir / (
            f"carbon_phase1_negative_bin_fraction_{stage}_v16.png"
        )
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
...
def plot_problematic_carbon_subtractions(
    ...
    path = output_dir / "carbon_phase1_problematic_bins_after_v16.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def write_carbon_phase1_products(
    ...
    nominal_group_csv = (
        tables_dir / "carbon_phase1_period_scales_v16.csv"
    )
    column_group_csv = (
        tables_dir / "carbon_phase1_tprime_column_scales_v16.csv"
    )
    nominal_scan_csv = (
        tables_dir / "carbon_phase1_period_scale_scan_v16.csv"
    )
    nominal_bin_csv = (
        tables_dir / "carbon_phase1_bin_diagnostics_v16.csv"
    )
    json_path = tables_dir / "carbon_phase1_diagnostics_v16.json"
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            for mode, directory in destinations.items():
                path = directory / (
                    f"carbon_phase1_{mode}_{period}_{stage}_v16.png"
                )
...
def plot_carbon_template_fit_canvases(results: list[dict[str, Any]], output_dir: Path) -> list[Path]:
    ...
    for period in ('su22','fa22','sp23'):
        for stage in ('before','after'):
            ...
            path=output_dir/f'carbon_template_fits_{period}_{stage}_v16.png'; fig.savefig(path,dpi=180); plt.close(fig); outputs.append(path)
        # endfor
    # endfor
    return outputs
...
def write_carbon_template_fit_products(results: list[dict[str, Any]], production_frame: pd.DataFrame, table_dir: Path, plot_dir: Path) -> dict[str, Any]:
    ...
    csv_path=table_dir/'carbon_template_fit_results_v16.csv'; json_path=table_dir/'carbon_template_fit_results_v16.json'
    comparison_path=table_dir/'carbon_vs_polynomial_comparison_v16.csv'
...
def plot_final_cut_canvases(
    ...
    """Plot corrected carbon fits with tight, nominal, and loose cut windows."""
    ensure_directory(output_dir)
    cut_lookup = {
    ...
    for period in ("su22", "fa22", "sp23"):
        ...
        for x_index in range(len(XB_BINS)):
            for t_index in range(len(MINUS_TPRIME_BINS_GEV2)):
                ...
                ax.plot(
                    centers,
                    payload["carbon"] + payload["residual"],
                    linestyle="--",
                    linewidth=1.0,
                    label=(
                        f"C + P{joint['selected_model']}"
                    ),
                )
                ax.plot(
                    centers,
                ...
                for low, high, style, label in (
                    ...
                    ax.axvline(low, linestyle=style, linewidth=0.9)
                    ax.axvline(high, linestyle=style, linewidth=0.9)
                # endfor
        ...
        handles, labels = axes[0, 0].get_legend_handles_labels()
        # Add one proxy entry for each cut style.
        from matplotlib.lines import Line2D
        handles.extend(
            [
                Line2D([0], [0], linestyle=":", label="Tight (2$\\sigma$)"),
                Line2D([0], [0], linestyle="--", label="Nominal (3$\\sigma$)"),
        ...
        output_path = output_dir / (
            f"final_carbon_assisted_cuts_{period}_v16.png"
        )
        fig.savefig(output_path, dpi=180)
        plt.close(fig)
...
def plot_carbon_vs_polynomial_mu_sigma(
    ...
    """
    Compare fitted shared means and period-specific widths for both methods.

    Top panel: shared mu from carbon-assisted and polynomial fits.
    Bottom panel: sigma for each period, with solid carbon-assisted curves and
    dashed polynomial curves.
    """
    ...
    for bin_number in bins:
        ...
        for period in ("su22", "fa22", "sp23"):
            carbon_sigma[period].append(
                float(
                    carbon_period_records[period]["sigma_gev2"]
                )
            )
...
def write_final_carbon_cut_products(
    ...
    csv_path = tables_dir / "final_carbon_assisted_mx2_cuts_v16.csv"
    json_path = tables_dir / "final_carbon_assisted_mx2_cuts_v16.json"
    python_path = exports_dir / "final_carbon_assisted_mx2_cuts_v16.py"
    cpp_path = exports_dir / "final_carbon_assisted_mx2_cuts_v16.h"
    comparison_plot = (
        plots_dir / "carbon_vs_polynomial_mu_sigma_v16.png"
    )
...
def main() -> int:
    ...
    try:
        ...
        csv_path = table_dir / "mx2_peak_fit_results_v16.csv"
        json_path = table_dir / "mx2_peak_fit_results_v16.json"
        latex_path = table_dir / "mx2_peak_nominal_corrected_tables_v16.tex"
        status_path = table_dir / "mx2_peak_fit_status_v16.txt"
