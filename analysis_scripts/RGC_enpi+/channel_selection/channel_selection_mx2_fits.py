#!/usr/bin/env python3
"""
channel_selection_mx2_fits_v12.py
...
def execute_fit_jobs(
    ...
    print("Running channel_selection_mx2_fits_v12.py", flush=True)
...
def write_latex_nominal_tables(frame: pd.DataFrame, output_path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("% Auto-generated by channel_selection_mx2_fits_v12.py")
...
def plot_carbon_scale_scans(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            fig.tight_layout()
            path = output_dir / (
                f"carbon_phase1_scale_scan_{period}_{stage}_v12.png"
            )
...
def plot_carbon_group_scales(
    ...
    for stage in ("before", "after"):
        ...
        ax.legend()
        fig.tight_layout()
        path = output_dir / f"carbon_phase1_group_scales_{stage}_v12.png"
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
    # endfor
...
def plot_carbon_negative_bin_fraction(
    ...
    for stage in ("before", "after"):
        ...
        fig.tight_layout()
        path = output_dir / (
            f"carbon_phase1_negative_bin_fraction_{stage}_v12.png"
        )
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
...
def plot_problematic_carbon_subtractions(
    ...
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.955))
    path = output_dir / "carbon_phase1_problematic_bins_after_v12.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def write_carbon_phase1_products(
    ...
    nominal_group_csv = (
        tables_dir / "carbon_phase1_period_scales_v12.csv"
    )
    column_group_csv = (
        tables_dir / "carbon_phase1_tprime_column_scales_v12.csv"
    )
    nominal_scan_csv = (
        tables_dir / "carbon_phase1_period_scale_scan_v12.csv"
    )
    nominal_bin_csv = (
        tables_dir / "carbon_phase1_bin_diagnostics_v12.csv"
    )
    json_path = tables_dir / "carbon_phase1_diagnostics_v12.json"
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            for mode, directory in destinations.items():
                path = directory / (
                    f"carbon_phase1_{mode}_{period}_{stage}_v12.png"
                )
...
# =============================================================================
# Exploratory carbon-template fits
# =============================================================================

def fit_one_joint_carbon_template_bin(
    nh3_jobs: list[FitJob],
    carbon_jobs: list[FitJob],
    period_scales: dict[tuple[str, str], dict[str, float]],
    stage: str,
    x_index: int,
    t_index: int,
    fit_min_gev2: float,
    ...
    """
    Fit one kinematic bin simultaneously across periods.

    The shared signal mean is free. Each period has an independent Gaussian
    yield and width plus a linear residual background. The measured carbon
    template is multiplied by the fixed period-wide Phase-1 normalization.
    This exploratory result does not alter the original production fit.
    """
    ...
    for period in periods:
        ...
        nh3_job = nh3_lookup[key]
        carbon_job = carbon_lookup[key]
        edges = np.linspace(
            nh3_job.histogram_min_gev2,
            nh3_job.histogram_max_gev2,
            nh3_job.histogram_bins + 1,
        )
        centers = 0.5 * (edges[:-1] + edges[1:])
        ...
        scale_info = period_scales[(period, stage)]
        scale = float(scale_info["carbon_scale"])
        scale_error = float(scale_info["carbon_scale_uncertainty"])
        fixed_carbon = scale * carbon_counts
        variance = (
            np.maximum(nh3_counts, 1.0)
            + np.maximum(scale**2 * carbon_counts, 0.0)
            + np.maximum((scale_error * carbon_counts) ** 2, 0.0)
        )
        prepared[period] = {
    ...
    # Parameter order: shared mean, then yield, sigma, c0, c1 for each period.
    initial = [NEUTRON_MASS2_GEV2]
    lower = [0.82]
    upper = [0.95]
    for period in periods:
        item = prepared[period]
        mask = item["fit_mask"]
        residual_counts = np.maximum(
            item["nh3"][mask] - item["fixed_carbon"][mask],
            0.0,
        )
        peak_mask = (
            item["centers"][mask] >= 0.72
        ) & (
            item["centers"][mask] <= 1.04
        )
        yield_guess = max(float(np.sum(residual_counts[peak_mask])), 10.0)
        side_mask = ~peak_mask
        c0_guess = (
            float(np.median(residual_counts[side_mask]))
            if np.any(side_mask)
            else 0.0
        )
        initial.extend([yield_guess, 0.055, c0_guess, 0.0])
        lower.extend([0.0, 0.015, -1.0e6, -1.0e6])
        upper.extend([1.0e9, 0.20, 1.0e6, 1.0e6])
    # endfor

    def unpack(parameters: np.ndarray, period_index: int) -> tuple[float, ...]:
        offset = 1 + 4 * period_index
        return tuple(float(v) for v in parameters[offset:offset + 4])
    # enddef
    ...
    def residual_function(parameters: np.ndarray) -> np.ndarray:
        ...
        for period_index, period in enumerate(periods):
            item = prepared[period]
            mask = item["fit_mask"]
            signal_yield, sigma, c0, c1 = unpack(
                parameters,
                period_index,
            )
            ...
            residual_background = linear_background(
                item["centers"],
                c0,
                c1,
            )
            ...
            blocks.append(
                (item["nh3"][mask] - prediction[mask])
                / np.sqrt(item["variance"][mask])
            )
        # endfor
    ...
    ndf = max(n_points - n_parameters, 1)
    chi2 = float(np.sum(residuals**2))
    covariance = np.full((n_parameters, n_parameters), np.nan)
    if result.jac.size and n_points > n_parameters:
        try:
            jtj_inverse = np.linalg.pinv(result.jac.T @ result.jac)
            covariance = jtj_inverse * (chi2 / ndf)
    ...
    for period_index, period in enumerate(periods):
        item = prepared[period]
        signal_yield, sigma, c0, c1 = unpack(parameters, period_index)
        signal = gaussian_signal_from_yield(
            item["centers"],
            signal_yield,
            mean,
        ...
        total = item["fixed_carbon"] + signal + residual_background
        offset = 1 + 4 * period_index
        errors = []
        for local_index in range(4):
            variance_value = covariance[
                offset + local_index,
                offset + local_index,
            ]
            errors.append(
                math.sqrt(max(float(variance_value), 0.0))
        ...
        mask = item["fit_mask"]
        local_pull = (
            item["nh3"][mask] - total[mask]
        ) / np.sqrt(item["variance"][mask])
        local_chi2 = float(np.sum(local_pull**2))
        local_ndf = max(int(np.count_nonzero(mask)) - 4, 1)
        record = {
            "period": period,
            "stage": stage,
            "x_index": x_index,
            ...
            "bin_id": bin_identifier(x_index, t_index),
            "success": bool(result.success),
            "message": str(result.message),
            "shared_mean_gev2": mean,
            "shared_mean_error_gev2": mean_error,
            "signal_yield": signal_yield,
            "signal_yield_error": errors[0],
            "sigma_gev2": sigma,
            "sigma_error_gev2": errors[1],
            "linear_c0": c0,
            "linear_c0_error": errors[2],
            "linear_c1": c1,
            "linear_c1_error": errors[3],
            "carbon_scale": item["scale"],
            "carbon_scale_uncertainty": item["scale_error"],
            "local_chi2": local_chi2,
            "local_ndf": local_ndf,
            "local_chi2_ndf": local_chi2 / local_ndf,
            "joint_chi2": chi2,
            "joint_ndf": ndf,
            "joint_chi2_ndf": chi2 / ndf,
            "fit_min_gev2": fit_min_gev2,
            "fit_max_gev2": fit_max_gev2,
            "model": "Gaussian + fixed scaled carbon + linear residual",
            "production_fit_changed": False,
        }
        ...
        payloads[period] = {
            ...
            "nh3": item["nh3"],
            "error": np.sqrt(item["variance"]),
            "fixed_carbon": item["fixed_carbon"],
            "signal": signal,
            "linear": residual_background,
            "total": total,
            "fit_mask": mask,
        }
    # endfor
...
def run_carbon_template_fits(
    ...
    """Run all exploratory joint carbon-template fits."""
    period_scales: dict[tuple[str, str], dict[str, float]] = {}
    for row in period_diagnostics["group_rows"]:
        period_scales[(row["period"], row["stage"])] = row
    # endfor
    ...
    for stage in ("before", "after"):
        for x_index in range(len(XB_BINS)):
            for t_index in range(len(MINUS_TPRIME_BINS_GEV2)):
                results.append(
                    fit_one_joint_carbon_template_bin(
                        nh3_jobs=nh3_jobs,
...
def plot_carbon_template_fit_canvases(
    ...
    """Draw exploratory fit canvases in a dedicated directory."""
        ...
        for stage in ("before", "after"):
            ...
            for x_index in range(len(XB_BINS)):
                for t_index in range(len(MINUS_TPRIME_BINS_GEV2)):
                    ...
                    ax.step(
                        x,
                        payload["fixed_carbon"],
                        where="mid",
                        linewidth=1.0,
                        label="Fixed scaled C",
                    )
                    ax.plot(
                        x,
                        payload["fixed_carbon"] + payload["linear"],
                        linewidth=1.0,
                        linestyle="--",
                        label="C + linear residual",
                    )
                    ax.plot(
                        x,
                        payload["total"],
                    ...
                    # endif
                    if t_index == 0:
                        low, high = XB_BINS[x_index]
                        ax.set_ylabel(
                            f"{low:.2f} < $x_B$ < {high:.2f}\nCounts"
                        )
                    # endif
            # endfor
            handles, labels = axes[0, 0].get_legend_handles_labels()
            ...
            fig.suptitle(
                f"{PERIOD_LABELS[period]}: {STAGE_LABELS[stage]}\n"
                "Exploratory Gaussian + fixed scaled carbon + "
                "linear-residual fits",
                fontsize=15,
                y=0.995,
            )
            fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.955))
            path = output_dir / (
                f"carbon_template_fits_{period}_{stage}_v12.png"
            )
            fig.savefig(path, dpi=180)
            plt.close(fig)
            outputs.append(path)
...
def write_carbon_template_fit_products(
    results: list[dict[str, Any]],
    table_dir: Path,
    plot_dir: Path,
) -> dict[str, Any]:
    """Write exploratory carbon-template fit products."""
    ...
    csv_path = table_dir / "carbon_template_fit_results_v12.csv"
    json_path = table_dir / "carbon_template_fit_results_v12.json"
    pd.DataFrame(rows).to_csv(csv_path, index=False)
    compact = {
        "description": (
            "Exploratory joint fits with a shared free Gaussian mean, "
            "period-specific Gaussian yields and widths, fixed period-wide "
            "carbon normalizations, and period-specific linear residual "
            "backgrounds. These results do not replace production fits."
        ),
        "results": rows,
    }
    json_path.write_text(
...
def main() -> int:
    ...
    try:
        ...
        csv_path = table_dir / "mx2_peak_fit_results_v12.csv"
        json_path = table_dir / "mx2_peak_fit_results_v12.json"
        latex_path = table_dir / "mx2_peak_nominal_corrected_tables_v12.tex"
        status_path = table_dir / "mx2_peak_fit_status_v12.txt"
