...
def plot_model_variations(frame: pd.DataFrame, output_dir: Path) -> None:
    """Plot corrected alternative-model shifts relative to the nominal model."""
    ensure_directory(output_dir)
    selected = frame[
        (frame["stage"] == "after")
    ...
    for period in ("su22", "fa22", "sp23"):
        ...
        for model_name in ("linear", "exponential"):
            ...
            ax.plot(
                model_frame["bin_number"],
                model_frame["delta_mean_from_nominal_gev2"],
                marker="o",
                markersize=4,
                linewidth=1.0,
                label=BACKGROUND_LABELS[model_name],
        ...
        for model_name in ("linear", "exponential"):
            ...
            ax.plot(
                model_frame["bin_number"],
                model_frame["delta_sigma_from_nominal_gev2"],
                marker="o",
                markersize=4,
                linewidth=1.0,
                label=BACKGROUND_LABELS[model_name],
