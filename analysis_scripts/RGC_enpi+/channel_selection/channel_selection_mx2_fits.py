#!/usr/bin/env python3
"""
channel_selection_mx2_fits_v25.py
...
For each of the three RGC run periods, the complete workflow is run independently for nominal and ISR samples before and after finalized momentum corrections. Uncorrected ISR inputs use *_ISR_2.root and corrected ISR inputs use *_ISR_mom_corrections.root. For each variant, the script:
...
def execute_fit_jobs(
    ...
    print("Running channel_selection_mx2_fits_v25.py", flush=True)
...
def write_latex_nominal_tables(frame: pd.DataFrame, output_path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("% Auto-generated by channel_selection_mx2_fits_v25.py")
...
def plot_carbon_scale_scans(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            fig.tight_layout()
            path = output_dir / (
                f"carbon_phase1_scale_scan_{period}_{stage}_v25.png"
            )
...
def plot_carbon_group_scales(
    ...
    for stage in ("before", "after"):
        ...
        ax.legend()
        fig.tight_layout()
        path = output_dir / f"carbon_phase1_group_scales_{stage}_v25.png"
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
    # endfor
...
def plot_carbon_negative_bin_fraction(
    ...
    for stage in ("before", "after"):
        ...
        fig.tight_layout()
        path = output_dir / (
            f"carbon_phase1_negative_bin_fraction_{stage}_v25.png"
        )
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
...
def plot_problematic_carbon_subtractions(
    ...
    path = output_dir / "carbon_phase1_problematic_bins_after_v25.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def write_carbon_phase1_products(
    ...
    nominal_group_csv = (
        tables_dir / "carbon_phase1_period_scales_v25.csv"
    )
    column_group_csv = (
        tables_dir / "carbon_phase1_tprime_column_scales_v25.csv"
    )
    nominal_scan_csv = (
        tables_dir / "carbon_phase1_period_scale_scan_v25.csv"
    )
    nominal_bin_csv = (
        tables_dir / "carbon_phase1_bin_diagnostics_v25.csv"
    )
    json_path = tables_dir / "carbon_phase1_diagnostics_v25.json"
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            for mode, directory in destinations.items():
                path = directory / (
                    f"carbon_phase1_{mode}_{period}_{stage}_v25.png"
                )
...
def plot_carbon_template_fit_canvases(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            path = output_dir / (
                f"carbon_template_fits_{period}_{stage}_v25.png"
            )
            fig.savefig(path, dpi=180)
            plt.close(fig)
            outputs.append(path)
...
def write_carbon_template_fit_products(results: list[dict[str, Any]], production_frame: pd.DataFrame, table_dir: Path, plot_dir: Path) -> dict[str, Any]:
    ...
    csv_path=table_dir/'carbon_template_fit_results_v25.csv'; json_path=table_dir/'carbon_template_fit_results_v25.json'
    comparison_path=table_dir/'carbon_vs_polynomial_comparison_v25.csv'
...
def plot_final_cut_canvases(
    ...
    for period in ("su22", "fa22", "sp23"):
        ...
        output_path = output_dir / (
            f"final_carbon_assisted_cuts_{period}_v25.png"
        )
        fig.savefig(output_path, dpi=180)
        plt.close(fig)
...
def write_final_carbon_cut_products(
    ...
    csv_path = tables_dir / "final_carbon_assisted_mx2_cuts_v25.csv"
    json_path = tables_dir / "final_carbon_assisted_mx2_cuts_v25.json"
    python_path = exports_dir / "final_carbon_assisted_mx2_cuts_v25.py"
    cpp_path = exports_dir / "final_carbon_assisted_mx2_cuts_v25.h"
    metadata_path = exports_dir / "final_carbon_assisted_mx2_cuts_metadata_v25.json"
    comparison_plot = (
        plots_dir / "carbon_vs_polynomial_mu_sigma_v25.png"
    )
    master_summary_plot = (
        plots_dir / "final_carbon_cut_master_summary_v25.png"
    )
            ...
            json_safe(
                {
                    "method": (
                        ...
                        "Gaussian widths and cut windows."
                    ),
        ...
        json.dumps(
            {
                ...
                "fit_method": (
                    ...
                    "carbon template; adaptive P1/P2 residual; independent "
...
def write_nominal_isr_comparison_products(
    ...
    csv_path = tables_dir / "nominal_vs_isr_fit_comparison_v25.csv"
    json_path = tables_dir / "nominal_vs_isr_fit_comparison_v25.json"
    mu_sigma_plot = plots_dir / "nominal_vs_isr_mu_sigma_v25.png"
    difference_plot = plots_dir / "nominal_vs_isr_parameter_differences_v25.png"
...
def run_analysis_variant(
    ...
    csv_path = table_dir / "mx2_peak_fit_results_v25.csv"
    json_path = table_dir / "mx2_peak_fit_results_v25.json"
    latex_path = table_dir / "mx2_peak_nominal_corrected_tables_v25.tex"
    status_path = table_dir / "mx2_peak_fit_status_v25.txt"
    ...
    variant_manifest_path = output_dir / "analysis_variant_manifest_v25.json"
    variant_manifest = {
        "schema_version": 1,
...
def main() -> int:
    """Run nominal and ISR workflows and write comparison products."""
    ...
    try:
        ...
        nominal_output = root_output / "nominal"
        isr_output = root_output / "isr"
        comparison_output = root_output / "comparisons"
        ...
        nominal_result = run_analysis_variant(
            args, "nominal", nominal_inputs,
            nominal_carbon_inputs, nominal_output,
        )
        ...
        if not args.disable_isr:
            ...
            isr_result = run_analysis_variant(
                args, "isr", isr_inputs,
                isr_carbon_inputs, isr_output,
            )
            if (
                nominal_result["final_cut_csv"] is not None
                and isr_result["final_cut_csv"] is not None
            ):
        ...
        manifest_path = root_output / "nominal_isr_analysis_manifest_v25.json"
        manifest = {
            "schema_version": 1,
            "analysis": "Parallel nominal and ISR channel selection",
            ...
            "import_contract": {
                "nominal_cut_json": nominal_result["final_cut_json"],
                "nominal_python_lookup": nominal_result["python_cut_lookup"],
                "isr_cut_json": (
                    isr_result["final_cut_json"]
                    if isr_result is not None else None
                ),
                "isr_python_lookup": (
                    isr_result["python_cut_lookup"]
                    if isr_result is not None else None
                ),
            },
        }
        ...
        print("")
        print("Nominal/ISR channel-selection analysis complete.")
        print(f"Nominal outputs:    {nominal_output}")
        if isr_result is not None:
            print(f"ISR outputs:        {isr_output}")
        # endif
        if comparison_products is not None:
            print(f"Comparison outputs: {comparison_output}")
        # endif
