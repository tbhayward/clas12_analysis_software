#!/usr/bin/env python3
"""
channel_selection_mx2_fits_v26.py
...
cross-check and never enter the exported production cut table.
...
The public output is intentionally compact:
  ...
  output/channel_selection_mx2_fit_stability/
    final_carbon_assisted_cuts/
      tables/final_carbon_assisted_mx2_cuts_v26.json
      tables/final_carbon_assisted_mx2_cuts_v26.csv
      exports/final_carbon_assisted_mx2_cuts_v26.py
      exports/final_carbon_assisted_mx2_cuts_v26.h
    diagnostics/
      plots/nominal_vs_isr_mu_sigma_v26.png
      plots/nominal_vs_isr_parameter_differences_v26.png
      tables/nominal_vs_isr_fit_comparison_v26.csv
      tables/nominal_vs_isr_fit_comparison_v26.json
    channel_selection_manifest_v26.json
...
def execute_fit_jobs(
    ...
    print("Running channel_selection_mx2_fits_v26.py", flush=True)
...
def write_latex_nominal_tables(frame: pd.DataFrame, output_path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("% Auto-generated by channel_selection_mx2_fits_v26.py")
...
def plot_carbon_scale_scans(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            fig.tight_layout()
            path = output_dir / (
                f"carbon_phase1_scale_scan_{period}_{stage}_v26.png"
            )
...
def plot_carbon_group_scales(
    ...
    for stage in ("before", "after"):
        ...
        ax.legend()
        fig.tight_layout()
        path = output_dir / f"carbon_phase1_group_scales_{stage}_v26.png"
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
    # endfor
...
def plot_carbon_negative_bin_fraction(
    ...
    for stage in ("before", "after"):
        ...
        fig.tight_layout()
        path = output_dir / (
            f"carbon_phase1_negative_bin_fraction_{stage}_v26.png"
        )
        fig.savefig(path, dpi=180)
        plt.close(fig)
        outputs.append(path)
...
def plot_problematic_carbon_subtractions(
    ...
    path = output_dir / "carbon_phase1_problematic_bins_after_v26.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def write_carbon_phase1_products(
    ...
    nominal_group_csv = (
        tables_dir / "carbon_phase1_period_scales_v26.csv"
    )
    column_group_csv = (
        tables_dir / "carbon_phase1_tprime_column_scales_v26.csv"
    )
    nominal_scan_csv = (
        tables_dir / "carbon_phase1_period_scale_scan_v26.csv"
    )
    nominal_bin_csv = (
        tables_dir / "carbon_phase1_bin_diagnostics_v26.csv"
    )
    json_path = tables_dir / "carbon_phase1_diagnostics_v26.json"
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            for mode, directory in destinations.items():
                path = directory / (
                    f"carbon_phase1_{mode}_{period}_{stage}_v26.png"
                )
...
def plot_carbon_template_fit_canvases(
    ...
    for period in ("su22", "fa22", "sp23"):
        for stage in ("before", "after"):
            ...
            path = output_dir / (
                f"carbon_template_fits_{period}_{stage}_v26.png"
            )
            fig.savefig(path, dpi=180)
            plt.close(fig)
            outputs.append(path)
...
def write_carbon_template_fit_products(results: list[dict[str, Any]], production_frame: pd.DataFrame, table_dir: Path, plot_dir: Path) -> dict[str, Any]:
    ...
    csv_path=table_dir/'carbon_template_fit_results_v26.csv'; json_path=table_dir/'carbon_template_fit_results_v26.json'
    comparison_path=table_dir/'carbon_vs_polynomial_comparison_v26.csv'
...
def plot_final_cut_canvases(
    ...
    for period in ("su22", "fa22", "sp23"):
        ...
        output_path = output_dir / (
            f"final_carbon_assisted_cuts_{period}_v26.png"
        )
        fig.savefig(output_path, dpi=180)
        plt.close(fig)
...
def write_final_carbon_cut_products(
    ...
    csv_path = tables_dir / "final_carbon_assisted_mx2_cuts_v26.csv"
    json_path = tables_dir / "final_carbon_assisted_mx2_cuts_v26.json"
    python_path = exports_dir / "final_carbon_assisted_mx2_cuts_v26.py"
    cpp_path = exports_dir / "final_carbon_assisted_mx2_cuts_v26.h"
    metadata_path = exports_dir / "final_carbon_assisted_mx2_cuts_metadata_v26.json"
    comparison_plot = (
        plots_dir / "carbon_vs_polynomial_mu_sigma_v26.png"
    )
    master_summary_plot = (
        plots_dir / "final_carbon_cut_master_summary_v26.png"
    )
...
def write_nominal_isr_comparison_products(
    ...
    csv_path = tables_dir / "nominal_vs_isr_fit_comparison_v26.csv"
    json_path = tables_dir / "nominal_vs_isr_fit_comparison_v26.json"
    mu_sigma_plot = plots_dir / "nominal_vs_isr_mu_sigma_v26.png"
    difference_plot = plots_dir / "nominal_vs_isr_parameter_differences_v26.png"
...
def run_analysis_variant(
    ...
    csv_path = table_dir / "mx2_peak_fit_results_v26.csv"
    json_path = table_dir / "mx2_peak_fit_results_v26.json"
    latex_path = table_dir / "mx2_peak_nominal_corrected_tables_v26.tex"
    status_path = table_dir / "mx2_peak_fit_status_v26.txt"
    ...
    variant_manifest_path = output_dir / "analysis_variant_manifest_v26.json"
    variant_manifest = {
        "schema_version": 1,
...
def copy_nominal_production_products(
    ...
    """Publish only the nominal cuts needed by downstream analyses."""
    source_root = Path(nominal_result["output_dir"]) / "final_carbon_assisted_cuts"
    ...
    source_csv = Path(nominal_result["final_cut_csv"])
    source_json = Path(nominal_result["final_cut_json"])
    source_python = Path(nominal_result["python_cut_lookup"])
    source_cpp = Path(nominal_result["cpp_cut_lookup"])

    canonical_csv = tables_dir / "final_carbon_assisted_mx2_cuts_v26.csv"
    canonical_json = tables_dir / "final_carbon_assisted_mx2_cuts_v26.json"
    canonical_python = exports_dir / "final_carbon_assisted_mx2_cuts_v26.py"
    canonical_cpp = exports_dir / "final_carbon_assisted_mx2_cuts_v26.h"
    ...
    # Compatibility aliases intentionally preserve the current default path in
    # determine_dilution_factor.py. The contents are the new nominal v26 cuts.
    compatibility_json = tables_dir / "final_carbon_assisted_mx2_cuts_v24.json"
    compatibility_csv = tables_dir / "final_carbon_assisted_mx2_cuts_v24.csv"
    shutil.copy2(canonical_json, compatibility_json)
    shutil.copy2(canonical_csv, compatibility_csv)
    ...
    return {
        ...
        "dilution_compatibility_json": str(compatibility_json),
    }
...
def main() -> int:
    ...
    try:
        ...
        work_root = root_output / ".fit_work"
        nominal_work = work_root / "nominal"
        isr_work = work_root / "isr"
        ...
        manifest_path = root_output / "channel_selection_manifest_v26.json"
        manifest = {
            "schema_version": 2,
            "production_policy": (
            ...
            "dilution_factor_contract": {
                ...
                "default_json_expected_by_determine_dilution_factor": (
                    production_products["dilution_compatibility_json"]
                ),
        ...
        print(
            "Dilution-compatible alias: "
            f"{production_products['dilution_compatibility_json']}"
        )
