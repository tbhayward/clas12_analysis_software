...
// Raw event counters (per-bin)
long long Npp = 0, Npm = 0, Nmp = 0, Nmm = 0;
...
void performChi2Fits_GeneralExclusive(const char* output_file,
  ...
  for (size_t i = 0; i < numBins; ++i) {
    ...
    minuit.DefineParameter(0,  "ALU_offset",      0.00,  0.01,  -0.1,  0.1);
    minuit.DefineParameter(1,  "AUL_offset",      0.00,  0.00,  -0.1,  0.1);
    minuit.DefineParameter(2,  "F_LU_sin/F_UU",   0.00,  0.001,  -1.0,  1.0);
    minuit.DefineParameter(3,  "F_UL_sin/F_UU",   0.00,  0.00,  -1.0,  1.0);
    minuit.DefineParameter(4,  "F_UL_sin2/F_UU",  0.00,  0.00,  -1.0,  1.0);
    minuit.DefineParameter(5,  "F_LL/F_UU",       0.00,  0.00,  -1.0,  1.0);
    minuit.DefineParameter(6,  "F_LL_cos/F_UU",   0.00,  0.00,  -1.0,  1.0);
    minuit.DefineParameter(7,  "F_UU_cos/F_UU",   0.00,  0.0,  -1.0,  1.0);
    minuit.DefineParameter(8,  "F_UU_cos2/F_UU",  0.00,  0.0,  -1.0,  1.0);
    minuit.DefineParameter(9,  "A_T_UL",          0.00,  0.0,  -1.0,  1.0);
    minuit.DefineParameter(10, "A_T_LL",          0.00,  0.0,  -1.0,  1.0);
    ...
    // Extended kinematics2_ table row: same + -t, W, DepB/DepA (unchanged)
    kinLatex2 << (i+1) << " ~&~ "
              << TripleHelper::triple(q2min,  meanQ2,  q2max)   << " ~&~ "
              << TripleHelper::triple(xmin,   meanx,   xmax)    << " ~&~ "
              << TripleHelper::triple(ymin,   meany,   ymax)    << " ~&~ "
    ...
    if (g_ge_write_bin_export && binExport) {
      // Convert fitted ratios -> physical asymmetry amplitudes
      ...
      // DSA: apply depolarization ratios analogous to single-spin case
  ...
  // Extended kinematics2 table: write to sibling file "kinematics2_..." (unchanged)
