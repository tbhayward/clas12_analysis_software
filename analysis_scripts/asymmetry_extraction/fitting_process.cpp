...
void performMLMFits_single_hadron(const char* output_file, const char* kinematic_file,
  ...
  for (size_t i = 0; i < numBins; ++i) {
    ...
    minuit.DefineParameter(3, "ALL", 0.35, 0.01, -1, 1);
...
void performChi2Fits_single_hadron(const char* output_file, const char* kinematic_file,
  ...
  for (size_t i = 0; i < numBins; ++i) {
    ...
    if (binNames[currentFits] == "epipluspiminus" || 
        ...
        xF = new TTreeReaderValue<double>(dataReader, "xF2");
        pT = new TTreeReaderValue<double>(dataReader, "pT2");
    } else {
        // Assign the reader values using one set of branches
        z = new TTreeReaderValue<double>(dataReader, "z");
        zeta = new TTreeReaderValue<double>(dataReader, "zeta");
        t = new TTreeReaderValue<double>(dataReader, "t");
        xF = new TTreeReaderValue<double>(dataReader, "xF");
        pT = new TTreeReaderValue<double>(dataReader, "pT");
    } 
