...
void createIntegratedKinematicPlots() {
    ...
    for (Int_t i = 0; i < branches->GetEntries(); ++i) {
        ...
        leg->AddEntry(dataHist, (std::string("ccdb (") + std::to_string((int)dataHist->GetEntries()) + " counts)").c_str(), "l");
        leg->AddEntry(mcHist, (std::string("sqlite (") + std::to_string((int)mcHist->GetEntries()) + " counts)").c_str(), "l");
